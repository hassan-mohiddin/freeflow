...
import { operationIdentity, reduceSemanticPacket } from "../../../skills/evaluate-skill/scripts/lib/semantic-evidence.mjs";
...
test("semantic reducer preserves every fixed criterion and selected turn in compact evidence", async () => {
  for (const name of names) {
    ...
    assert.deepEqual(reduced.parity.criterion_ids, source.evidence.criteria.map((item) => item.id));
...
test("semantic reducer emits typed exact-lineage omissions and separates savings from omitted bytes", async () => {
  const reduced = reduceSemanticPacket(await packet("wfi-002-candidate"), { bundle: "wfi-002-candidate", sourcePath: "packet.json" });
  const decoded = decodeCev1(reduced.rendered.content);
...
test("operation identities change with implementation content", () => {
  const first = operationIdentity("example", [function reducer() { return 1; }]);
  const second = operationIdentity("example", [function reducer() { return 2; }]);
