...
import {
	...
	inspectSkill,
	validateSkill,
} from "../../../skills/write-skill/scripts/lib/skill-author-core.mjs";
...
test("init creates one minimal draft skill file", async (t) => {
	...
	assert.doesNotMatch(
		await readFile(result.skill_path, "utf8"),
		/^\s*(?:>\s*)?(?:Status|Readiness)\s*:/im,
	);
...
test("validate accepts project-contained skill and runtime dependencies", async (t) => {
	...
	await writeFile(
		resolve(projectRoot, "skills", "other-skill", "SKILL.md"),
		"# Other\n",
	);
	...
	assert.deepEqual(result.linked_dependencies, [
	...
	async (t) => {
		...
		assert.ok(
			result.errors.some((item) =>
				item.includes("resolves outside project root"),
			),
		);
	},
);
...
test("inspect remains advisory and does not claim behavioral success", async (t) => {
	...
	assert.ok(
		inspection.signals.some((item) => item.code === "activation-boundary"),
	);
	assert.ok(
		!inspection.signals.some((item) => item.code === "status-unlabeled"),
	);
});
...
test("inspect warns when readiness metadata appears in the active body", async (t) => {
	...
	assert.ok(
		inspection.signals.some((item) => item.code === "status-in-active-body"),
	);
});
