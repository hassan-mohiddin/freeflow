...
const legacyComparisonTest = (name, fn) =>
  test(name, { skip: "Legacy product-surface comparison is deferred until Phase 7 archival." }, fn);
...
legacyComparisonTest(
  "enabled Context Virtualization registers, projects, archives, restores, and reports through commands",
  async () => {
...
legacyComparisonTest("enabled Conversation History exposes only search and retrieve operations", async () => {
...
legacyComparisonTest("Freeflow Context exposes both operation families when both features are enabled", async () => {
...
legacyComparisonTest(
  "Conversation History searches hidden active-branch entries and retrieves selected content",
  async () => {
...
legacyComparisonTest("mixed assistant sources retain ordinary content and exclude Freeflow tool calls", async () => {
...
legacyComparisonTest("archived Context Virtualization sources return to hidden Conversation History", async () => {
...
legacyComparisonTest("visible retrieved sources are excluded until the retrieval result is archived", async () => {
...
legacyComparisonTest("Conversation History rejects visible-only searches and bounds oversized retrieval", async () => {
...
legacyComparisonTest("Conversation History reports partial coverage for invalid eligible sources", async () => {
...
legacyComparisonTest("visible invalid sources do not make an empty hidden corpus partial", async () => {
...
legacyComparisonTest("disabled Context Virtualization hides the tool and leaves tool results unchanged", async () => {
