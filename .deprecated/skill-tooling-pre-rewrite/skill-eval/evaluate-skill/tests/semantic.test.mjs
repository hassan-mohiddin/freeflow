...
} from "../../../skills/evaluate-skill/scripts/lib/semantic.mjs";
...
test("semantic prompt uses opaque identity and fixed criteria", async (t) => {
  ...
  await writeFile(
    ...
    JSON.stringify({ variant: "candidate", changed_paths: ["answer.md"] }),
  );
  await writeFile(
    resolve(run, "inputs", "case.json"),
    JSON.stringify({
      prompt: "Answer the question.",
      assertions: [{ id: "quality", type: "semantic", rubric: "Answer directly and accurately." }],
    }),
  );
  await writeFile(
    resolve(run, "objective-grade.json"),
    JSON.stringify({ objective_pass: true, assertions: [{ id: "path", type: "path_exists", state: "pass" }] }),
  );
...
test("semantic packet records upstream byte caps as explicit omissions", async (t) => {
  ...
  await writeFile(
    resolve(run, "inputs", "case.json"),
    JSON.stringify({ prompt: "x", assertions: [{ id: "quality", type: "semantic", rubric: "x" }] }),
  );
  ...
  assert.equal(
    details.some((item) => item.reason === "UC" && item.omittedBytes === 1000),
    true,
  );
});
...
test("multi-turn semantic packet exposes only one shared declared turn scope", async (t) => {
  ...
  await writeFile(
    resolve(run, "metadata.json"),
    JSON.stringify({ variant: "candidate", execution_mode: "rpc-scripted", changed_paths: [] }),
  );
  await writeFile(
    resolve(run, "inputs", "case.json"),
    JSON.stringify({
  ...
  await writeFile(
    ...
    JSON.stringify({
      turns: [
        ...
        {
          id: "turn-2",
          final_text: "undeclared secret answer",
          workspace: { changed_paths: ["secret"], diff: "SECRET DIFF" },
        },
  ...
  assert.deepEqual(
    built.evidence.turns.map((turn) => turn.id),
    ["turn-1", "turn-3"],
  );
...
test("semantic packet represents deleted and file-count-capped changed paths without crashing", async (t) => {
  ...
  const changedPaths = [
    "deleted.txt",
    ...Array.from({ length: 19 }, (_, index) => `kept-${index}.txt`),
    "after-cap.txt",
  ];
  ...
  await writeFile(
    resolve(run, "inputs", "case.json"),
    JSON.stringify({
      prompt: "Inspect all changed paths.",
      assertions: [{ id: "quality", type: "semantic", rubric: "Account for all changed paths." }],
    }),
  );
  await writeFile(resolve(run, "objective-grade.json"), JSON.stringify({ objective_pass: true, assertions: [] }));
...
test("semantic evidence reads reject changed-path traversal", async (t) => {
  ...
  await writeFile(
    resolve(run, "metadata.json"),
    JSON.stringify({ variant: "subject", changed_paths: ["../secret.txt"] }),
  );
  await writeFile(
    resolve(run, "inputs", "case.json"),
    JSON.stringify({ prompt: "x", assertions: [{ id: "quality", type: "semantic", rubric: "x" }] }),
...
test("semantic protocol rejects extra, missing, duplicate, or inconsistent assertions", () => {
  const valid = {
    verdict: "pass",
    assertions: [{ id: "quality", verdict: "pass", evidence: ["Observed fact"] }],
    uncertainty: null,
  };
  ...
  assert.throws(
    () =>
      validateSemanticResult(
        { ...valid, assertions: [...valid.assertions, { id: "objective-check", verdict: "pass", evidence: ["x"] }] },
        ["quality"],
      ),
    /do not match/,
  );
  ...
  assert.throws(
    () => validateSemanticResult({ ...valid, assertions: [...valid.assertions, ...valid.assertions] }, ["quality"]),
    /duplicate/,
  );
...
test("semantic post-process and cleanup failures preserve settled execution", async (t) => {
  ...
  await writeFile(resolve(run, "diff"), "");
  const outcome = await gradeSemanticRun(
    run,
    {
      provider: "p",
      model: "m",
    ...
    },
    {
      runSubject: async () => ({
        ...
        parsed: {
          ...
          final_text: JSON.stringify({
            verdict: "pass",
            assertions: [{ id: "quality", verdict: "pass", evidence: ["answer"] }],
            uncertainty: null,
          }),
      ...
      persistEvidence: async () => {
        throw new Error("semantic evidence write failed");
      },
      cleanup: async (path) => {
        await rm(path, { recursive: true, force: true });
        throw new Error("semantic cleanup failed");
      },
    },
  );
