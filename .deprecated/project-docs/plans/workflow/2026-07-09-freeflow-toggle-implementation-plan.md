...
- `enabled: false`: suppress workflow skills, output-router tools/context/hooks, and observed/native routing. Keep `/freeflow enable` available.
...
- `outputRouter.enabled` remains an independent layer toggle, but is effective only when top-level Freeflow is enabled.
...
2. Dynamic Pi skill exposure
   ...
   - Exclude `output-router` from model skill exposure; it stays runtime capability context only.
...
3. Runtime context and hooks
   ...
   - Gate output-router context on its effective layer state.
...
4. Commands and settings
   ...
   - Keep `/output-router` as a compatibility command.
...
5. Setup/docs/evidence
   ...
   - Add Pi extension tests for missing setup, master off, skills off, router-only, and settings row inactivity.
...
- Enabling output-router, observed routing, native safety net, or script transform by default.
