...
export const EXPECTED_BASELINE_FAILURE_KEYS = ["outside-file-boundary/freeflow:run-cat-host-shell"];
export async function runContextModeRealDeepBenchmark(options = {}) {
  ...
  try {
    ...
    async function ffTransform(sourceOutputId, stream, operation) {
      const start = performance.now();
...
function batchQueryAnswerText(result) {
