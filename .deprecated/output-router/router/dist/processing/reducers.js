...
function buildOutputFacts(details) {
  const facts = [{ name: "build", value: `${details.errorCount} errors, ${details.warningCount} warnings` }];
...
function schemaRequiredProperties(schema) {
  return Array.isArray(schema.required)
    ? schema.required.filter((property) => typeof property === "string" && isMcpParameterName(property))
    : [];
}
