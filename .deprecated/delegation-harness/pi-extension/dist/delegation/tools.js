...
export function registerDelegationTools(pi) {
  ...
  const toolDefinitions = [
    ...
    delegationTool(
      ...
      ROUTE_PARAMETERS,
      (params, _signal, ctx) => executeRoute(params, ctx),
      "parent-control",
    ),
    delegationTool(
      ...
      (params, _signal, ctx) => executeRequestExecutionAuthorization(params, ctx),
      "parent-control",
    ),
    delegationTool(
      ...
      (params, signal, ctx) => executeApplyRoute(pi, params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_spawn",
      ...
      SPAWN_PARAMETERS,
      (params, signal, ctx) => executeSpawn(pi, params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_status",
      ...
      STATUS_PARAMETERS,
      (params, signal, ctx) => executeStatus(pi, params, signal, ctx),
      "read-recovery",
    ),
    delegationTool(
      "delegate_wait",
      ...
      WAIT_PARAMETERS,
      (params, signal, ctx) => executeWait(params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_result",
      ...
      TARGET_PARAMETERS,
      (params, signal, ctx) => executeResult(pi, params, signal, ctx),
      "read-recovery",
    ),
    delegationTool(
      "delegate_send",
      ...
      SEND_PARAMETERS,
      (params, signal, ctx) => executeSend(pi, params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_capture",
      ...
      CAPTURE_PARAMETERS,
      (params, signal, ctx) => executeCapture(pi, params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_cancel",
      ...
      CLOSE_PARAMETERS,
      (params, signal, ctx) => executeCancel(pi, params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_close",
      ...
      CLOSE_PARAMETERS,
      (params, signal, ctx) => executeClose(pi, params, signal, ctx),
      "parent-control",
    ),
    delegationTool(
      ...
      REPORT_PARAMETERS,
      (params, _signal, ctx) => executeRecordReport(params, ctx),
      "parent-control",
    ),
    delegationTool(
      "delegate_finish",
      ...
      FINISH_PARAMETERS,
      (params, _signal, ctx) => executeFinish(params, ctx),
      "child-lifecycle",
    ),
    delegationTool(
      "delegate_attention",
      ...
      (params, _signal, ctx) => executeAttention(params, ctx),
      "child-lifecycle",
    ),
    delegationTool(
      "delegate_progress",
      ...
      PROGRESS_PARAMETERS,
      (params, _signal, ctx) => executeProgress(params, ctx),
      "child-lifecycle",
    ),
    delegationTool(
      "delegate_inbox",
      ...
      INBOX_PARAMETERS,
      (params, _signal, ctx) => executeInbox(params, ctx),
      "read-recovery",
    ),
    delegationTool(
      "delegate_ack_alert",
      ...
      (params, _signal, ctx) => executeAckAlert(params, ctx),
      "read-recovery",
    ),
    delegationTool(
      "delegate_ack_all",
      ...
      ACK_ALL_PARAMETERS,
      (params, _signal, ctx) => executeAckAll(params, ctx),
      "read-recovery",
    ),
    delegationTool(
      ...
      (params, _signal, ctx) => executeUserAttention(params, ctx),
      "read-recovery",
    ),
    delegationTool(
      ...
      (params, _signal, ctx) => executeUpdateExecutionMap(params, ctx),
      "parent-control",
    ),
  ];
...
async function executeAckAll(params, ctx) {
  ...
  const read = await store.markParentAlertsRead(
    taskId,
    candidates.map((alert) => alert.alertId),
  );
  return {
    toolStatus: "ok",
...
async function executeRecordReport(params, ctx) {
  ...
  if (!parsed.ok || report === undefined) {
    ...
    const alert = await store.queueParentAlert(taskId, {
      outcome: state === "failed" ? "failed" : "attention",
      state: state,
    ...
    packages: Array.isArray(executionMap.packages)
      ? executionMap.packages.map((pkg) => ({
          ...
          commitCheckpoints: Array.isArray(pkg.commitCheckpoints)
            ? pkg.commitCheckpoints.map((checkpoint) => ({
...
function firstErrorMessage(compact) {
  return Array.isArray(compact.errors)
    ? compact.errors.find((error) => typeof error?.message === "string")?.message
    : undefined;
}
...
function compactRows(rows) {
  return Array.isArray(rows)
    ? rows.map((row) => ({ tag: row.tag, fields: row.fields, lineNumber: row.lineNumber }))
    : [];
}
...
function compactErrors(errors) {
  return Array.isArray(errors)
    ? errors.map((error) => ({ lineNumber: error.lineNumber, message: error.message, blockKind: error.blockKind }))
    : [];
}
...
function attentionAgentFromTarget(target) {
  return Array.isArray(target?.agents)
    ? target.agents.find((agent) => agent.state === "attention" || agent.state === "waiting_for_parent")
    : undefined;
}
...
function verifierResultPassing(compact) {
  ...
  if (checks.length > 0) return checks.every((check) => check?.status === "pass" || check?.status === "skipped");
  const first = compact?.results?.[0];
...
async function layoutAllocationForAlreadyAppliedRoute(store, taskId, routeId, decision, application) {
  ...
  const allocation = layoutState.allocations.find(
    (candidate) => candidate.allocationId === application.layoutAllocationId,
  );
...
function childRouteSourcePointers(request) {
  return (request.targetFiles ?? []).map((path) => ({
    kind: "target_file",
    path,
...
async function routedWorkerCommandAuthority(store, taskId, agentId, request, cwd) {
  ...
  let candidates = (executionMap.packages ?? []).filter((pkg) => {
    ...
    if (!requestScopes.every((scope) => scopeCompatibleWithPackage(scope, packageScopes, cwd))) return false;
...
function createPiCmuxRunner(pi, signal) {
  return {
    async run(command, options = {}) {
      ...
      const executionStatus = signal?.aborted
        ? "cancelled"
        : result?.killed
          ? "timed_out"
          : exitCode === 0
...
async function parentDescendantReconciliationBlock(store, taskId, agentId, manifest, operation) {
  ...
  const activeDescendants = descendants.filter(
    (descendant) => !["closed", "cancelled", "completed"].includes(descendant.state),
  );
  const unconsumedCompleted = descendants.filter(
    (descendant) => descendant.state === "completed" && unread.some((alert) => alert.agentId === descendant.agentId),
  );
  ...
  return typedError(
    ...
    "parent close/cancel requires descendant close, cancel, adopt, or park decisions before the parent pane disappears",
    {
      ...
      unreadAlertIds: unread
        .filter((alert) => descendants.some((descendant) => descendant.agentId === alert.agentId))
...
function compactCheckFields(check) {
  if (Array.isArray(check?.fields)) return check.fields.map((field) => truncateLine(String(field), 180));
  ...
  return [check?.name, check?.status, outputId, check?.evidence ?? check?.notes]
    .filter(Boolean)
    .map((field) => truncateLine(String(field), 180));
}
...
function compactEvidenceFields(item) {
  if (Array.isArray(item?.fields)) return item.fields.map((field) => truncateLine(String(field), 180));
  return [
    item?.label,
...
function compactItemText(item) {
  if (Array.isArray(item?.fields)) return item.fields.map((field) => String(field)).join("|");
