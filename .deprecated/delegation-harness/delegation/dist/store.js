...
export class DelegationStore {
  ...
  async initTask(input) {
    ...
    const existing = (await fileExists(paths.taskJson)) ? await readJson(paths.taskJson) : undefined;
    const task = {
      taskId,
  ...
  async publishPlanningReport(taskId, input) {
    ...
    return this.withExecutionAuthorizationLock(safeTaskId, async () => {
      ...
      if (stored.errors !== undefined) eventData.errors = stored.errors;
  ...
  async readRouteApplications(taskId) {
    const safeTaskId = validateSafeId(taskId, "task id");
    return (await readJsonLines(taskPaths(this.root, safeTaskId).routeApplicationsJsonl)).map((application) =>
  ...
  async readLeaseEvents(taskId) {
    const safeTaskId = validateSafeId(taskId, "task id");
    const events = (await readJsonLines(taskPaths(this.root, safeTaskId).leasesJsonl)).map((event) =>
      normalizeLeaseEvent(safeTaskId, event),
    );
  ...
  async readWakeAttempts(taskId) {
    const safeTaskId = validateSafeId(taskId, "task id");
    return (await readJsonLines(taskPaths(this.root, safeTaskId).wakeAttemptsJsonl)).map((attempt) =>
      normalizeWakeAttempt(safeTaskId, attempt),
    );
  }
  pathsForTask(taskId) {
  ...
  async queueParentAlertRecord(taskId, input) {
    ...
    return this.withParentAlertLock(safeTaskId, async () => {
      ...
      if (input.evidence !== undefined) candidate.evidence = stripUndefined(input.evidence);
  ...
  async maybeEscalateClosedParent(sourceAlert) {
    ...
    try {
      ...
      const sourceLink = {
        ...
        evidence: sourceAlert.evidence === undefined ? null : stripUndefined(sourceAlert.evidence),
      };
