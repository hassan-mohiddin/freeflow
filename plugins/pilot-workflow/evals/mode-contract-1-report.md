# Pilot Workflow Mode Contract Eval Report - 1
...
- `MODE-002` "Make strict-workflow the default for this repo" should update only `.pilot-workflow/config.json`.

Both run against `tiny-post-setup-source-conflict-app`, which has the compact always-on `AGENTS.md` block and `.pilot-workflow/config.json`.
...
Updated `plugins/pilot-workflow/skills/mode-contract/SKILL.md` with a compact command/default section:
...
- reads `.pilot-workflow/config.json` when repo default matters
...
- Baseline passed.
- With skill passed.
- Both changed only `.pilot-workflow/config.json`.
...
The always-on core already points agents at `.pilot-workflow/config.json`, so these evals did not produce a baseline failure.
