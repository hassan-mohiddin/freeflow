...
claude_args=(
  ...
  --permission-mode "${PILOT_WORKFLOW_CLAUDE_PERMISSION_MODE:-dontAsk}"
  --tools "${PILOT_WORKFLOW_CLAUDE_TOOLS:-Read,Edit,Bash}"
  --add-dir "$run_dir"
)

if [ "${PILOT_WORKFLOW_CLAUDE_BARE:-1}" = "1" ]; then
  claude_args+=(--bare)
fi
...
claude_plugin_dir="${PILOT_WORKFLOW_CLAUDE_PLUGIN_DIR:-$default_plugin_dir}"
