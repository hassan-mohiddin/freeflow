...
```text
plugins/pilot-workflow/evals/scripts/run-claude-fixture-eval.sh
```
...
plugins/pilot-workflow/evals/fixture-eval-runbook.md
```
...
- `with-skill`: `--plugin-dir plugins/pilot-workflow`.
- portable evals: `--bare`, `--no-session-persistence`, `--permission-mode dontAsk`, `--tools Read,Edit,Bash`, `--add-dir "$run_dir"`.
- host-memory evals: set `PILOT_WORKFLOW_CLAUDE_BARE=0` so `CLAUDE.md` and imports are discoverable.
...
```text
bare=1
plugin_dir=.../plugins/pilot-workflow
...
Then run one host-memory eval with `PILOT_WORKFLOW_CLAUDE_BARE=0`, preferably `AON-001`, to verify `CLAUDE.md` import behavior.
