# Setup Pilot Workflow Eval Report - Iteration 2

Date: 2026-05-25

## Scope

Updated `setup-pilot-workflow` for host-specific always-on setup:

- Codex behavior lives in `AGENTS.md`, not `.codex/rules/*.rules`.
- Claude behavior uses `CLAUDE.md` with an explicit import to `.claude/rules/pilot-core.md`.
- Setup uses one compact always-on core block/file.
- Full workflow behavior stays in skills.

## Skill Changes

`plugins/pilot-workflow/skills/setup-pilot-workflow/SKILL.md` now:

- writes a compact core block with source-truth, interview, forward/re-enter, verification, and stable-capture invariants
- creates `.pilot-workflow/config.json` with only `defaultMode: "workflow"`
- selects Codex as `AGENTS.md`
- selects Claude as `CLAUDE.md` plus `.claude/rules/pilot-core.md`
- refuses ambiguous `AGENTS.md` plus `CLAUDE.md` setup without a host target
- avoids `.codex/rules` for behavioral memory
- resists splitting Pilot into multiple always-loaded rule files unless the user explicitly confirms after the one-file recommendation
...
- `STP-006`: Claude setup creates `CLAUDE.md` import plus one `.claude/rules/pilot-core.md`.
- `STP-007`: Claude setup under four-rule-file pressure still creates one `pilot-core.md`.
...
| STP-005 Codex rules avoidance | Fail | Pass | Baseline created extra `pilot-workflow.md`; with-skill created config and `AGENTS.md` only, preserving `.codex/rules`. |
...
| STP-007 four-rule pressure | Fail | Pass | Baseline created four always-loaded rule files; with-skill created one `pilot-core.md`. |
...
- `STP-005`: with-skill reported `.codex/rules` was not changed for Pilot behavior.
- `STP-006`: with-skill reported `CLAUDE.md` import, one core file, and no `.codex/rules`.
- `STP-007`: baseline created four always-loaded rule files; with-skill used one `pilot-core.md`.

Saved run outputs:

- `plugins/pilot-workflow/evals/runs/setup-5/stp-001-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-002-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-003-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-004-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-005-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-005-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-006-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-006-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-007-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-5/stp-007-with-skill-output.md`
...
wc -c plugins/pilot-workflow/evals/runs/setup-5/stp-00*-output.diff
```
