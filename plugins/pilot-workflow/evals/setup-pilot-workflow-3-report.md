# Setup Pilot Workflow Eval Report - Iteration 3

Date: 2026-05-26

## Scope

Updated `setup-pilot-workflow` for setup profiles and explicit persisted defaults.

Owned paths:

- `plugins/pilot-workflow/skills/setup-pilot-workflow/`
...
`setup-pilot-workflow/SKILL.md` now:
...
- allows `.pilot-workflow/config.json` to use an explicitly requested valid repo default: `conversation`, `workflow`, or `strict-workflow`
...
- update `AGENTS.md` with the compact Pilot block
- create `.pilot-workflow/config.json` with only `defaultMode: "strict-workflow"`
...
- Did not create `.pilot-workflow/config.json`.
...
- Created `.pilot-workflow/config.json`.
- Wrote `defaultMode: "workflow"` despite the explicit `strict-workflow` default request.

`STP-008` after revision: pass.

- Created compact `AGENTS.md` activation.
- Created `.pilot-workflow/config.json` with only `defaultMode: "strict-workflow"`.
...
- `STP-006`: pass, Claude setup still uses `CLAUDE.md` import plus one `pilot-core.md`.
...
The nested agents verified config JSON and activation counts in their final responses. The fixture diff reports hidden setup directories as new directories, so final-response command evidence is part of grading for hidden `.pilot-workflow` and `.claude` file contents.
...
wc -l plugins/pilot-workflow/skills/setup-pilot-workflow/SKILL.md plugins/pilot-workflow/skills/setup-pilot-workflow/references/host-setup.md
git diff --check
```
