# Setup Pilot Workflow Eval Report - Iteration 1

Date: 2026-05-25

## Scope

Added `setup-pilot-workflow` and focused fixture evals for first-run setup:

- `STP-001` clean repo with `AGENTS.md` only
- `STP-002` repo with both `AGENTS.md` and `CLAUDE.md`
- `STP-003` repo instruction conflict
- `STP-004` existing stale `## Pilot Workflow` block

Compared:

- Baseline: no Pilot Workflow skill files
- With skill: `plugins/pilot-workflow/skills/setup-pilot-workflow/SKILL.md`
...
- create only `.pilot-workflow/config.json` with `defaultMode: "workflow"`
- add only the minimal `## Pilot Workflow` activation block
- ask before choosing between `AGENTS.md` and `CLAUDE.md`
- stop before setup when repo rules conflict with Pilot's ask/verify behavior
...
- `plugins/pilot-workflow/evals/fixtures/tiny-setup-agents-app/`
- `plugins/pilot-workflow/evals/fixtures/tiny-setup-both-app/`
- `plugins/pilot-workflow/evals/fixtures/tiny-setup-conflict-app/`
- `plugins/pilot-workflow/evals/fixtures/tiny-setup-existing-block-app/`

Added prompts:

- `plugins/pilot-workflow/evals/prompts/stp-001.txt`
- `plugins/pilot-workflow/evals/prompts/stp-002.txt`
- `plugins/pilot-workflow/evals/prompts/stp-003.txt`
- `plugins/pilot-workflow/evals/prompts/stp-004.txt`
...
|---|---:|---:|
| Creates `.pilot-workflow/config.json` with exactly `defaultMode: "workflow"` | 0 | 2 |
| Adds exactly one `## Pilot Workflow` block to `AGENTS.md` | 1 | 2 |
...
|---|---:|---:|
| Creates `.pilot-workflow/config.json` with exactly `defaultMode: "workflow"` | 0 | 2 |
| Leaves exactly one `## Pilot Workflow` block in `AGENTS.md` | 2 | 2 |
...
- `STP-004`: `replaced the old Pilot block with the required activation block`

Saved run outputs:

- `plugins/pilot-workflow/evals/runs/setup-1/stp-001-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-2/stp-001-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-1/stp-002-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-2/stp-002-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-1/stp-003-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-2/stp-003-with-skill-output.md`
- `plugins/pilot-workflow/evals/runs/setup-4/stp-004-baseline-output.md`
- `plugins/pilot-workflow/evals/runs/setup-4/stp-004-with-skill-output.md`
...
- `STP-003`: installed Pilot as advisory despite conflicting repo instructions.
...
After review, compressed `setup-pilot-workflow/SKILL.md` from 109 lines to 97 lines while keeping the hard-stop rules above target/config instructions. Reran all three with-skill evals as `setup-3`; behavior stayed intact.
...
bash -n plugins/pilot-workflow/evals/scripts/run-codex-fixture-eval.sh
plugins/pilot-workflow/evals/scripts/run-codex-fixture-eval.sh ...
PILOT_WORKFLOW_REQUIRE_EMPTY_DIFF=1 plugins/pilot-workflow/evals/scripts/run-codex-fixture-eval.sh ...
wc -c plugins/pilot-workflow/evals/runs/setup-*/stp-*-output.diff
wc -l plugins/pilot-workflow/skills/setup-pilot-workflow/SKILL.md
```
...
Keep `setup-pilot-workflow` as the next developer meta skill.

Next useful setup eval later:

- existing `.pilot-workflow/config.json` with an explicit persisted default should be preserved or repaired only after asking
