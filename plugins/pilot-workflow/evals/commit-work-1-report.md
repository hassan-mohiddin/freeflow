...
`plugins/pilot-workflow/skills/commit-work/SKILL.md`

## Research Summary

- Pilot runtime says `commit-work` follows successful execution, review, and verification.
...
- Old Orchestra commit prior art is useful for diff discipline and provenance, but Pilot should not copy its hook machinery, canon-frozen rules, mandatory refs policy, or heavy governance.
...
- `bash -n plugins/pilot-workflow/evals/fixtures/tiny-commit-app/.pilot-eval-setup.sh`
- `wc -l plugins/pilot-workflow/skills/commit-work/SKILL.md` => 93

Fixture setup sanity check passed: copied fixture starts with dirty `src/settings-page.ts` and `src/billing.ts` after `.pilot-eval-setup.sh`.
...
- final response: `plugins/pilot-workflow/evals/runs/commit-work-2/cmt-001-with-skill-output.md`
