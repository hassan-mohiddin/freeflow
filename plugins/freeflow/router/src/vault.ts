...
const sessionIndexWriteLocks = new Map<string, Promise<void>>();
...
export async function storeMetadataOutput(
  vault: VaultHandle,
  options: StoreMetadataOutputOptions,
  ...
  const record: MetadataOutputRecord = {
    kind: "metadata",
    outputId,
    recordId,
    objectId,
...
function assertStorablePersistence(persistence: EvidencePersistence | undefined): void {
...
async function indexVaultRecordAfterAppend(
  vault: VaultHandle,
  sessionId: string,
  record: VaultRecord,
...
function sessionIndexLockPath(vault: VaultHandle, sessionId: string): string {
...
function recordIdFromHash(hash: string): string {
  return `ffrec_${hash.slice(0, 24)}`;
}

function exactVaultPersistence(outputId: string): EvidencePersistence {
  return {
    status: "vaulted",
...
function metadataOnlyPersistence(outputId?: string): EvidencePersistence {
  return {
...
function producerForTextSourceKind(sourceKind: TextOutputRecord["sourceKind"]): ProducerDescriptor {
