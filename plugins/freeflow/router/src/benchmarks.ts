...
import type { EvidencePacket, RetrievalRoutedResult } from "./types.js";
...
export function renderRouterBenchmarkReport(report: RouterBenchmarkReport): string {
  ...
  for (const fixture of report.fixtures) {
    for (const result of fixture.results) {
      ...
      lines.push(
        `| ${escapeTable(fixture.id)} | ${escapeTable(result.mode)} | ${correctness} | ${escapeTable(result.actualPath ?? "-")} | ${escapeTable(
          result.actualLines ?? "-",
        )} | ${result.rawBytes}/${result.rawTokensApprox} | ${result.routedBytes}/${result.routedTokensApprox} | ${result.latencyMs.p50.toFixed(
          2,
        )}/${result.latencyMs.p95.toFixed(2)} | ${escapeTable(recovery)} | ${escapeTable(notes)} |`,
      );
    }
  }
...
function improvedRetrieveObservation(
...
function parseCliArgs(argv: readonly string[]): { iterations?: number; reportPath: string } {
