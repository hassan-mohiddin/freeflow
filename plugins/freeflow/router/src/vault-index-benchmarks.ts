...
import { createVault, readOutputText, storeCommandOutput, storeMetadataOutput, storeTextOutput, type VaultHandle } from "./vault.js";
import type { MetadataOutputRecord, OutputStream, TextOutputRecord, VaultRecord } from "./types.js";
...
const REPORT_PATH = "plugins/freeflow/evals/reports/runtime/vault-index-storage-spike-1-report.md";
...
export async function runVaultIndexStorageBenchmark(options: RunVaultIndexStorageBenchmarkOptions = {}): Promise<VaultIndexStorageBenchmarkReport> {
  ...
  const scanQueryLatencies: number[] = [];
  ...
  const sqliteStatus = "not-run" as const;
  const candidates: VaultIndexStorageCandidateResult[] = [
    {
...
async function createBenchmarkFixtures(vault: VaultHandle, iteration: number): Promise<BenchmarkFixture[]> {
  const commandText = [
...
export function renderVaultIndexStorageBenchmarkReport(report: VaultIndexStorageBenchmarkReport): string {
...
export async function writeVaultIndexStorageBenchmarkReports(
