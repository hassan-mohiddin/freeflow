...
import { freeflowBatch } from "./batch.js";
import { freeflowRetrieve } from "./retrieve.js";
import { freeflowRun, type HostCommandRunResult, type HostCommandRunner } from "./run.js";
import type { BatchRoutedResult, ImportantLine, RetrievalRoutedResult, RoutedResult } from "./types.js";
...
type FixtureRun = (env: BenchmarkEnvironment) => Promise<FixtureObservation>;

interface ContextModeFixtureDefinition {
  id: string;
  title: string;
  category: ContextModeFixtureCategory;
  rawBytes: number;
  expectedFacts: string[];
  modes: Record<ContextModeMode, FixtureRun>;
}
...
export async function runContextModeNormalizedBenchmarks(
  options: RunContextModeNormalizedBenchmarksOptions = {},
): Promise<ContextModeNormalizedBenchmarkReport> {
  ...
  try {
    ...
    const fixtureResults: ContextModeFixtureResult[] = [];
    for (const fixture of fixtures) {
      const results: ContextModeModeResult[] = [];
...
export function renderContextModeNormalizedBenchmarkReport(report: ContextModeNormalizedBenchmarkReport): string {
...
export async function writeContextModeNormalizedBenchmarkReports(
  report: ContextModeNormalizedBenchmarkReport,
  markdownReportPath: string,
  options: WriteContextModeNormalizedBenchmarkReportsOptions = {},
...
function contextModeFixtureDefinitions(env: BenchmarkEnvironment): ContextModeFixtureDefinition[] {
  return [
    commandFixture(env),
    docsFixture(env),
    logsFixture(env),
...
function commandFixture(env: BenchmarkEnvironment): ContextModeFixtureDefinition {
...
function docsFixture(env: BenchmarkEnvironment): ContextModeFixtureDefinition {
...
function logsFixture(env: BenchmarkEnvironment): ContextModeFixtureDefinition {
...
function jsonTableFixture(env: BenchmarkEnvironment): ContextModeFixtureDefinition {
...
function repoSearchFixture(env: BenchmarkEnvironment): ContextModeFixtureDefinition {
...
function batchFixture(env: BenchmarkEnvironment): ContextModeFixtureDefinition {
...
async function runFixtureMode(
  fixture: ContextModeFixtureDefinition,
  mode: ContextModeMode,
  env: BenchmarkEnvironment,
  iterations: number,
): Promise<ContextModeModeResult> {
  const latencies: number[] = [];
  let observation: FixtureObservation | undefined;
...
async function freeflowRetrieveObservation(
  env: BenchmarkEnvironment,
  options: { rawBytes: number; expectedFacts: readonly string[]; query: string },
): Promise<FixtureObservation> {
  const before = await directoryByteSize(env.vaultRoot);
...
async function freeflowBatchObservation(
  env: BenchmarkEnvironment,
  options: { rawBytes: number; expectedFacts: readonly string[] },
): Promise<FixtureObservation> {
  const before = await directoryByteSize(env.vaultRoot);
...
function summarizeContextModeReport(fixtures: readonly ContextModeFixtureResult[]): ContextModeNormalizedSummary {
...
function summarizeMode(fixtures: readonly ContextModeFixtureResult[], mode: ContextModeMode): ContextModeModeSummary {
  const results = fixtures
    .map((fixture) => fixture.results.find((result) => result.mode === mode))
    .filter((result): result is ContextModeModeResult => Boolean(result));
...
async function setupRepo(repoRoot: string): Promise<void> {
  await mkdir(join(repoRoot, "docs"), { recursive: true });
  await mkdir(join(repoRoot, "src"), { recursive: true });
  await mkdir(join(repoRoot, "graphify-out"), { recursive: true });
  await writeFile(join(repoRoot, "docs/output-router.md"), DOC_TEXT, "utf8");
...
function fixtureCommandRunner(): HostCommandRunner {
  return {
    async run(request): Promise<HostCommandRunResult> {
...
function relevantSnippet(raw: string, facts: readonly string[], capBytes: number): string {
...
function truncateToBytes(value: string, capBytes: number): string {
...
async function readDirectoryRecursive(path: string): Promise<string[]> {
...
function contextModeModes(): ContextModeMode[] {
  return ["context-mode-normalized-proxy", "freeflow-owned-tools"];
}

function contextOutputId(id: string, raw: string): string {
...
function defaultReportPath(): string {
  return resolve(process.cwd(), "plugins/freeflow/evals/reports/runtime/context-mode-normalized-benchmark-1-report.md");
}
