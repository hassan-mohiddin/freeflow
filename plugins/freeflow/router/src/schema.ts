...
function requireString(record: Record<string, unknown>, key: string, path: string, issues: MutableIssues) {
  if (typeof record[key] !== "string" || record[key] === "") {
    issues.push({ path: `${path}.${key}`, message: "Expected a non-empty string." });
  }
}

function requireBoolean(record: Record<string, unknown>, key: string, path: string, issues: MutableIssues) {
...
function requireInteger(record: Record<string, unknown>, key: string, path: string, issues: MutableIssues) {
...
function validateSourceRef(value: unknown, path: string, issues: MutableIssues) {
  if (!isRecord(value)) {
...
function validateImportantLines(value: unknown, path: string, issues: MutableIssues) {
  if (value === undefined) {
    return;
  }
...
function validateImportantLine(value: unknown, path: string, issues: MutableIssues) {
  if (!isRecord(value)) {
...
function validLineRange(value: string): boolean {
...
function validateEvidenceArray(value: unknown, path: string, issues: MutableIssues) {
  if (value === undefined) {
    return;
  }
...
export function validateEvidencePacket(value: unknown): ValidationResult<EvidencePacket> {
...
export function validateRoutedResult(value: unknown): ValidationResult<RoutedResult> {
  const issues: ValidationIssue[] = [];
...
export function validateRouterConfig(value: unknown): ValidationResult<RouterConfig> {
  const issues: ValidationIssue[] = [];
