...
import { createLocalVaultIndex, type VaultIndexMatch, type VaultIndexQueryFilters } from "./vault-index.js";
...
function resolveVaultStream(record: VaultRecord, requested?: OutputStream): OutputStream {
...
function vaultRecordResultFields(record: VaultRecord): Partial<RetrievalRoutedResult> {
  const fields: Partial<RetrievalRoutedResult> = {
    producer: producerForVaultRecord(record),
    persistence: persistenceForVaultRecord(record),
  };
  const recordId = (record as { recordId?: unknown }).recordId;
...
function producerForVaultRecord(record: VaultRecord): ProducerDescriptor {
  const producer = (record as { producer?: ProducerDescriptor }).producer;
  if (producer?.kind) {
    return producer;
  }
  if (record.kind === "command") {
...
function persistenceForVaultRecord(record: VaultRecord): EvidencePersistence {
  const persistence = (record as { persistence?: EvidencePersistence }).persistence;
...
function vaultRecordDetails(record: VaultRecord): string {
...
function vaultRecordRecovery(
  record: VaultRecord,
  requestedStream?: OutputStream,
): { reason: string; hint: NonNullable<RetrievalRoutedResult["recovery"]> } {
  const persistence = persistenceForVaultRecord(record);
