...
import { byteLength, countLines } from "./evidence.js";
import { reduceObservedOutput } from "./reducers.js";
import { createVault, storeMetadataOutput, storeTextOutput } from "./vault.js";
import type {
  EvidencePacket,
  EvidencePersistence,
  ObservedHostDescriptor,
  ObservedOutputNormalization,
  ObservedProducerRisk,
...
interface NormalizedObservedOutput {
  text: string;
  shape: ObservedOutputNormalization["shape"];
  mediaType?: string;
}

interface StoredObservedOutput {
  outputId?: string;
  recordId?: string;
  persistence: EvidencePersistence;
  evidenceSource: EvidencePacket["source"];
}

export async function routeObservedToolOutput(options: RouteObservedToolOutputOptions): Promise<ObservedRoutedResult> {
  const preserve = options.preserve ?? "important";
  const thresholds = { ...DEFAULT_ROUTER_THRESHOLDS, ...options.thresholds };
  let normalizedOutput: NormalizedObservedOutput;
  let normalized: ObservedOutputNormalization;
  ...
  try {
    ...
    const fallbackOutput: NormalizedObservedOutput = {
  ...
  let stored: StoredObservedOutput;
  try {
  ...
  const result: ObservedRoutedResult = {
    toolStatus: "ok",
    decisionId,
    preserve,
    host: options.host,
...
function observedNormalization(normalizedOutput: NormalizedObservedOutput): ObservedOutputNormalization {
  return {
...
function normalizeObservedOutput(value: unknown, mediaType?: string): NormalizedObservedOutput {
...
function normalizeContentBlocks(value: unknown): string | undefined {
...
function recoveryHint(persistence: EvidencePersistence) {
  ...
  if (persistence.recoverability === "exact" && recoveryOutputId !== undefined) {
    return {
      how: `Use freeflow_retrieve with source.kind=vault and outputId=${recoveryOutputId}, stream=raw, and an exact lineRange to recover exact observed output.`,
...
function decisionIdFromSeed(seed: string): string {
...
function safePassthroughText(value: unknown): string {
  if (typeof value === "string") {
    return value;
  }
  try {
