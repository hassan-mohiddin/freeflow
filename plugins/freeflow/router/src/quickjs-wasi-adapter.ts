...
interface QuickJsRunResult {
  status: "success" | "failed" | "timed_out" | "output_limit_exceeded";
  stdout: string;
  stderr: string;
  stdoutBytes: number;
  stderrBytes: number;
...
function unavailableQuickJsAdapter(reason: string): ScriptSandboxAdapter {
  return {
    id: "quickjs-wasi",
    version: "unavailable",
    languages: ["javascript"],
...
async function loadQuickJsRuntime(packageRoot: string): Promise<QuickJsRuntime> {
...
async function runQuickJs(runtime: QuickJsRuntime, options: QuickJsRunOptions): Promise<QuickJsRunResult> {
  ...
  let stderr = "";
  let truncated = false;
  const appendOutput = (stream: "stdout" | "stderr", value: unknown) => {
...
function installHostApi(
  vm: any,
  appendOutput: (stream: "stdout" | "stderr", value: unknown) => void,
  inputs: Record<string, string>,
) {
  const readText = vm.newFunction("readText", (...args: any[]) => {
  ...
  const writeText = vm.newFunction("writeText", (...args: any[]) => {
  ...
  const consoleLog = vm.newFunction("console.log", (...args: any[]) => {
    appendOutput("stdout", `${formatHostArgs(vm, args)}\n`);
    return vm.undefined;
  });
  const consoleError = vm.newFunction("console.error", (...args: any[]) => {
...
function disposeHandle(value: any) {
