...
} from "../dist/context-saving-baseline.js";
...
test("context-saving baseline measures current model-visible sizes and guardrails", async () => {
  ...
  assert.ok(exactRetrieve.evidenceLocations.some((location) => location.includes("docs/output-router.md")));

  assert.ok(report.guardrails.some((guardrail) => guardrail.path === "plugins/freeflow/router/tests/retrieve.test.js"));
  assert.ok(report.guardrails.some((guardrail) => guardrail.path === "plugins/freeflow/router/tests/pi-extension-derive.test.js"));
  ...
  assert.match(rendered, /plugins\/freeflow\/router\/tests\/retrieve.test.js/);
});
