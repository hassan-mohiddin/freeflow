...
import { freeflowTransform, TRANSFORM_ENGINE_IMPLEMENTATION } from "../dist/transform.js";
...
test("run script filters call the shared transform engine, not the derive facade", async () => {
  const runSource = await readFile("plugins/freeflow/router/src/run.ts", "utf8");
  assert.match(runSource, /from "\.\/transform\.js"/);
