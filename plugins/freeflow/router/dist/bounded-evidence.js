...
export function buildBoundedExcerpt(options) {
...
function overCapEdgeRanges(lines, range, caps) {
    const lineCount = range.end - range.start + 1;
    const chunkLineCount = Math.min(10, Math.max(1, Math.floor(lineCount / 2)));
    const headRange = shrinkRangeToMaxBytes(lines, { start: range.start, end: Math.min(range.end, range.start + chunkLineCount - 1) }, "head", caps);
    const tailRange = shrinkRangeToMaxBytes(lines, { start: Math.max(range.start, range.end - chunkLineCount + 1), end: range.end }, "tail", caps);
...
function shrinkRangeToMaxBytes(lines, range, edge, caps) {
    let current = range;
...
function linesForRange(lines, range) {
    return lines.slice(range.start - 1, range.end).join("\n");
}
function capLineRangeForWindow(range, window, caps) {
...
function maxLinesForWindow(window, caps) {
    if (window === "lines_30") {
...
function maxExcerptBytesForWindow(window, caps) {
...
function truncateHeadToUtf8Bytes(text, maxBytes) {
    if (maxBytes <= 0) {
        return "";
    }
    if (byteLength(text) <= maxBytes) {
...
function truncateTailToUtf8BytesContent(text, maxBytes) {
    if (maxBytes <= 0) {
        return "";
    }
    if (byteLength(text) <= maxBytes) {
...
function moveToCodePointStartBoundary(text, index) {
    if (index > 0 && index < text.length && isLowSurrogate(text.charCodeAt(index))) {
        return index + 1;
    }
    return index;
}
function moveToCodePointEndBoundary(text, index) {
