...
export function createDefaultObservedReducerRegistry() {
    const reducers = [webSearchReducer(), fetchReducer(), codeSearchReducer(), mcpReducer(), jsonReducer(), genericTextReducer()];
    return {
        names() {
...
export function reduceObservedOutput(input) {
    return createDefaultObservedReducerRegistry().reduce(input);
}
export function webSearchReducer() {
    return {
        name: "web-search",
        supports(input) {
...
export function codeSearchReducer() {
    return {
        name: "code-search",
        supports(input) {
...
export function genericTextReducer() {
    return {
        name: "generic-text",
        supports() {
            return true;
...
function producerTextFallback(input, reducer, reason) {
    const result = genericTextReducer().reduce(input);
    return {
        ...result,
        reducer,
...
function extractRows(value, keys) {
    if (Array.isArray(value)) {
        return value;
    }
    if (!isRecord(value)) {
...
function reduceMcpJson(input, value) {
...
function reduceMcpJsonArray(input, key, rows) {
...
function reduceMcpJsonObject(input, value) {
...
function reduceMcpMixed(input, payload) {
...
function compactMcpTextBlock(text) {
...
function mcpGenericTextFallback(input, reason) {
    const result = genericTextReducer().reduce(input);
    return {
        ...result,
        reducer: "mcp",
...
function extractMcpPayload(input) {
    const textBlocks = [];
    const jsonValues = [];
...
function collectMcpPayload(value, textBlocks, jsonValues) {
    if (!isRecord(value)) {
        if (typeof value === "string" && value.length > 0) {
...
function collectMcpBlock(block, textBlocks, jsonValues) {
...
function collectMcpText(text, textBlocks, jsonValues) {
...
function unwrapCommonArray(value) {
    if (!isRecord(value)) {
        return undefined;
    }
...
function reduceJsonObject(input, value) {
    const fields = importantJsonFields(value).slice(0, MAX_JSON_OBJECT_FIELDS);
    const compact = fields.length > 0 ? Object.fromEntries(fields) : compactJsonValue(value);
    const excerpt = stableJson({
        type: "object",
...
function importantJsonFields(value, prefix = "", depth = 0) {
    const fields = [];
...
function textCaps(preserve, thresholds) {
