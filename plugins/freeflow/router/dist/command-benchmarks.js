...
const DEFAULT_SKIPPED_EXTERNAL_TOOLS = [
    {
        name: "RTK",
...
export function renderCommandBenchmarkReport(report) {
    ...
    lines.push("", "## Regression Status", "", report.summary.improved.failed === 0
        ...
        : `Improved Freeflow command routing failed ${report.summary.improved.failed} gated command-output benchmark fixture(s).`, "", `Failed command facts preserved: ${report.summary.failedCommandFactsPreserved ? "yes" : "no"}.`, "");
    return lines.join("\n");
}
export async function writeCommandBenchmarkReport(report, reportPath) {
    await mkdir(dirname(reportPath), { recursive: true });
    await writeFile(reportPath, renderCommandBenchmarkReport(report), "utf8");
}
export async function writeCommandBenchmarkJsonReport(report, reportPath) {
    await mkdir(dirname(reportPath), { recursive: true });
...
function summarizeCommandMode(fixtures, mode) {
...
function isCommandGatedPass(result, mode) {
...
function createCommandBenchmarkFixtures(vaultRoot, externalComparators) {
    return commandFixtureDefinitions().map((definition) => commandFixture(definition, vaultRoot, externalComparators));
}
function commandFixtureDefinitions() {
    const noisySuccess = [
    ...
    const gitOutput = [
        " M plugins/freeflow/router/src/run.ts",
        "?? plugins/freeflow/router/src/command-benchmarks.ts",
        " plugins/freeflow/router/src/run.ts | 12 +++++++++---",
    ...
    return [
        ...
        },
        {
            ...
            stderr: "",
            combined: gitOutput,
            requiredFacts: ["M plugins/freeflow/router/src/run.ts", "1 file changed"],
...
function commandFixture(definition, vaultRoot, externalComparators) {
    const expected = {
...
async function nativeCommandObservation(fixture) {
    return {
...
async function improvedCommandObservation(fixture, vaultRoot) {
...
async function externalComparatorObservation(fixture, comparator) {
    const startedAt = performance.now();
...
function fixedCommandRunner(fixture) {
    return {
        async run() {
            const result = {
                stdout: fixture.stdout,
...
async function verifyCommandRecovery(vaultRoot, sessionId, outputId, expectedCombined, requiredFacts) {
    if (!outputId) {
...
function scoreCommandCorrectness(expected, observation) {
