...
import { freeflowDerive } from "./derive.js";
...
const GUARDED_FIXTURES = [
    {
        path: "plugins/freeflow/router/tests/run.test.js",
        behaviors: [
    ...
    {
        path: "plugins/freeflow/router/tests/retrieve.test.js",
        behaviors: [
    ...
    {
        path: "plugins/freeflow/router/tests/derive.test.js",
        behaviors: [
    ...
    {
        path: "plugins/freeflow/router/tests/pi-extension.test.js",
        behaviors: [
    ...
    {
        path: "plugins/freeflow/router/tests/pi-extension-derive.test.js",
        behaviors: [
    ...
    {
        path: "plugins/freeflow/router/tests/regression-fixtures.test.js",
        behaviors: [
    ...
    {
        path: "plugins/freeflow/router/tests/vault-index.test.js",
        behaviors: [
...
export async function runContextSavingBaseline(options = {}) {
    ...
    try {
        ...
        await writeFile(join(repoRoot, "docs/output-router.md"), REPO_DOC_TEXT, "utf8");
        ...
        const repoExactResult = await freeflowRetrieve({
            action: "retrieve",
            source: { kind: "repo", root: repoRoot, path: "docs/output-router.md" },
...
export function renderContextSavingBaselineReport(report) {
    ...
    for (const observation of report.observations) {
        lines.push(`| ${escapeTable(observation.id)} | ${observation.tool} | ${escapeTable(observation.action)} | ${observation.rawBytes} | ${observation.modelVisibleBytes} | ${observation.detailsPayloadBytes} | ${formatPercent(observation.modelVisibleReductionPercent)} | ${observation.factsPreserved ? "yes" : "no"} | ${observation.recoveryAvailable ? "yes" : "no"} | ${escapeTable(observation.status)} |`);
    }
...
export async function writeContextSavingBaselineReport(report, markdownReportPath, options = {}) {
...
function currentPiModelVisibleText(result) {
...
function resultOutputId(result) {
    const value = asRecord(result);
...
function resultRecoveryHint(result) {
    const value = asRecord(result);
    return stringAt(asRecord(value?.recovery), "how") ?? "";
}
function resultEvidenceLocations(result) {
    const value = asRecord(result);
    const locations = [];
...
function defaultReportPath() {
    return resolve(process.cwd(), "plugins/freeflow/evals/reports/runtime/context-saving-native-tools-baseline-1-report.md");
}
async function runCli() {
    ...
    console.log(`Freeflow context-saving baseline ${shortId}: ${report.summary.factsPreserved}/${report.summary.fixtures} facts preserved, ${report.summary.recoveryAvailable}/${report.summary.fixtures} recoverable`);
