import { DEFAULT_MAX_IMPORTANT_LINES, assembleImportantLines, lineEntries, nonEmptyLineEntries, splitLines, } from "./evidence.js";
...
export function parseCommandOutput(input) {
...
function parseTestRunnerOutput(input) {
...
function parseTypeScriptOrLintOutput(input) {
    const command = commandText(input.command);
    ...
    const streams = [
        ...
        { stream: "combined", text: input.combined },
    ];
...
function parseGenericOutput(input) {
...
function selectGenericImportantLines(input) {
...
function firstFailureBlock(text, stream) {
    const entries = lineEntries(text);
    const firstFailureIndex = entries.findIndex((entry) => /^\s*(FAIL|FAILED|FAILURES)(?:\s|$)/.test(entry.line) || /\b(AssertionError|expected|Expected|Received|stack|Traceback|Error:)\b/.test(entry.line));
...
function testSummaryEvidence(text) {
    const matches = lineEntries(text).filter((entry) => /\b(Test Suites?:|Tests?:|FAILURES)/i.test(entry.line) ||
...
function selectEvidenceLines(evidenceBlocks, maxImportantLines) {
...
function emptyEvidence(sourceText) {
    return {
        importantLines: [],
        fidelity: "exact",
        compressed: false,
...
function isGitEvidenceLine(line) {
    return (/\b(On branch|Your branch|Changes to be committed|Changes not staged|Untracked files|modified:|new file:|deleted:|files? changed|insertions?\(\+\)|deletions?\(-\))\b/i.test(line) ||
        /^([ MARCUD?!]{2})\s+/.test(line) ||
        /\|\s+\d+\s+[+\-]+/.test(line));
}
function gitCounts(text) {
    const counts = {};
...
function testSummaryText(counts, status, exitCode) {
    const parts = [];
...
function diagnosticMatches(entries) {
    const matches = [];
    let headerEntry;
    for (const entry of entries) {
        ...
        if (stylish && headerEntry) {
            const reference = {
                ...
                severity: stylish[3]?.toLowerCase() ?? "error",
                message: stylish[4] ?? "",
            };
...
function diagnosticImportantEntries(matches) {
    const selected = [];
    const seen = new Set();
...
function diagnosticReference(line) {
    ...
    if (tsc) {
        return {
            ...
            column: Number(tsc[3]),
            code: tsc[5] ?? "",
            severity: tsc[4]?.toLowerCase() ?? "error",
            message: tsc[6] ?? "",
        };
    }
    ...
    if (lint) {
        const reference = {
            ...
            severity: lint[4]?.toLowerCase() ?? "error",
            message: lint[5] ?? "",
        };
        if (lint[6] !== undefined) {
...
function diagnosticCounts(references) {
    return {
...
function commandLooksLikeTestRunner(command) {
    return (/^\s*(?:npm|pnpm|yarn)\s+(?:run\s+)?(?:test|tests)\b/i.test(command) ||
...
function commandLooksLikeTypeScriptOrLint(command) {
    return (/\b(?:tsc|typecheck|eslint|biome|lint)\b/i.test(command) ||
        /^\s*(?:npm|pnpm|yarn)\s+(?:run\s+)?(?:build|typecheck|lint)\b/i.test(command));
}
function commandLooksLikeBuildTool(command) {
    return (/^\s*(?:npm|pnpm|yarn)\s+(?:run\s+)?(?:build|install|ci)\b/i.test(command) ||
