...
export async function runCodexQaBenchmarks(options = {}) {
    const iterations = normalizeIterations(options.iterations, 1);
    const fixture = await createSandboxPermissionsFixture();
    try {
        const results = [];
        const modeResults = [];
        for (const mode of ["native-broad-search-proxy", "improved-freeflow-router"]) {
...
export async function writeCodexQaBenchmarkReports(report, markdownReportPath, options = {}) {
...
export function renderCodexQaBenchmarkReport(report) {
    const lines = [
...
async function runModeIterations(fixture, mode, iterations) {
    const latencies = [];
...
async function nativeBroadSearchObservation(fixture) {
...
function gradeObservation(fixture, observation) {
...
function summarizeCodexQaReport(fixtures) {
...
async function createSandboxPermissionsFixture() {
    ...
    await writeTextFile(join(root, GENERATED_GRAPH_PATH), [
        "<html><body>",
...
function answerFromEvidence(evidence, text) {
...
async function readRepoTextFiles(root) {
    const files = [];
...
function defaultReportPath() {
    return resolve(process.cwd(), "plugins/freeflow/evals/reports/runtime/output-router-codex-qa-benchmark-1-report.md");
}
async function main() {
    ...
    console.log(`Freeflow Codex Q&A benchmark: improved ${report.summary.improved.passed}/${report.summary.fixtures} pass; native ${report.summary.nativeBaseline.passed}/${report.summary.fixtures} pass`);
