...
Pi uses an extension instead of `hooks/hooks.json`. The built Pi extension registers direct commands, reads `workflow/SKILL.md`, `workflow/references/workflow-map.md`, `interview-gate/SKILL.md`, `output-router/SKILL.md`, and `output-router/references/safety-policy.md`, refreshes that context on `session_start` and `session_compact`, and injects it during Pi's `before_agent_start` lifecycle event. The same extension registers `freeflow_status`, `freeflow_retrieve`, `freeflow_run`, and `freeflow_derive` and gives them compact/expanded TUI renderers so collapsed tool rows stay readable while `ctrl+o` shows structured status, evidence, recovery details, and non-destructive migration recommendations. `freeflow_derive operation.kind="script"` stays disabled by default; Pi discovers optional JavaScript/Python/jq adapters only from explicit `FREEFLOW_QUICKJS_WASI_ROOT`, `FREEFLOW_ERYX_ROOT`, and `FREEFLOW_JQ_WASM_ROOT` package roots and does not install or download script runtimes during execution. The Eryx Python adapter also requires Node `--experimental-wasm-jspi`. When configured, its Pi `tool_result` hook observed-routes MCP, web, fetch, and code-search outputs after direct host execution; host permissions and execution remain owned by Pi. It also injects compact provider summaries for available/configured producer providers instead of raw provider docs. Pi `/workflow` commands set a session-scoped current-mode override, while `.freeflow/config.json` remains the default-mode source. It follows the same boundary as the Codex/Claude hooks: context loading and output routing only, no enforcement.
