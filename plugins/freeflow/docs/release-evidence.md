...
- Script-sandbox proof gate: router exposes a sandbox adapter/probe interface, required adversarial proofs, rejected unsafe mechanisms, and status-visible candidate-unproven mechanisms; no unsandboxed fallback exists. JavaScript product execution is implemented only for explicitly configured/provided QuickJS roots and remains disabled by default; Python is unavailable and jq is proof-backed but not product-enabled.
