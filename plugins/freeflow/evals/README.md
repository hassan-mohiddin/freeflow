...
Baseline fixture runs set `FREEFLOW_DISABLE_RUNTIME_CONTEXT=1` before launching nested agents so installed lifecycle hooks do not inject Freeflow workflow context into the baseline.
...
Use `scripts/check-runtime-context-hook.sh` after changing plugin-bundled workflow context hooks.
