...
check_marketplace_locality() {
  ...
  same_value "$check" "Codex marketplace source path" "./plugins/freeflow" "$codex_path" || ok=0
  same_value "$check" "Claude marketplace source" "./plugins/freeflow" "$claude_source" || ok=0
  ...
  if [ "$ok" = "1" ]; then
    record_check "$check" "pass" "Root marketplace files point at ./plugins/freeflow."
  fi
}
...
check_release_boundary() {
  ...
  contains_fixed "$release_boundary_adr" "one plugin runtime under \`plugins/freeflow/\`" || {
...
check_package_cleanliness() {
  ...
  contains_fixed "$gitignore" "plugins/freeflow/evals/runs/" || {
    record_check "$check" "fail" ".gitignore does not ignore generated eval runs."
    ok=0
  }

  if git -C "$repo_root" ls-files plugins/freeflow/evals/runs | rg -q .; then
    record_check "$check" "fail" "Generated eval run output is tracked."
    ok=0
  fi

  if find "$repo_root" -path "$repo_root/.git" -prune -o -path "$repo_root/plugins/freeflow" -prune -o -path '*/.codex-plugin/plugin.json' -print | rg -q .; then
    record_check "$check" "fail" "Found another Codex plugin manifest outside plugins/freeflow."
    ok=0
  fi

  if find "$repo_root" -path "$repo_root/.git" -prune -o -path "$repo_root/plugins/freeflow" -prune -o -path "$repo_root/.claude-plugin/marketplace.json" -prune -o -path '*/.claude-plugin/plugin.json' -print | rg -q .; then
    record_check "$check" "fail" "Found another Claude plugin manifest outside plugins/freeflow."
    ok=0
  fi
...
check_docs_drift() {
  ...
  contains_fixed "$release_evidence" "13 direct workflow skill calls" || {
    record_check "$check" "fail" "Release evidence no longer lists 13 direct workflow skill calls."
    ok=0
  }

  contains_fixed "$release_evidence" "2 contributor skill calls" || {
    record_check "$check" "fail" "Release evidence no longer lists 2 contributor skill calls."
    ok=0
  }
