# Freeflow Command Surface Eval Report - 1
...
The question was whether Freeflow already has all plugin slash commands and whether they work when users call them.

## Result

Freeflow has the documented command surface as model-routed skill calls, not native host slash-command handlers.

Added `plugins/freeflow/command-surface.json` as the canonical command map:
...
Added `plugins/freeflow/evals/scripts/audit-command-surface.sh` to verify:
