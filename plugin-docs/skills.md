...
Freeflow ships a small workflow skill pack. Natural language is the preferred interface. In Codex/Claude, slash-style phrases work as model-routed skill hints; in Pi, the extension registers direct Freeflow commands and exposes model skills only after `/setup-freeflow` creates `.freeflow/config.json` and skills are enabled.
