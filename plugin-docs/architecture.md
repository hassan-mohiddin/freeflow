...
The hooks load the existing `mode-contract`, `workflow`, `interview-gate`, and `output-router` skills plus discovery-light runtime guidance:
...
Setup handles the same-session case directly: after successful setup verification, it reads the mode-contract, workflow, interview-gate, and output-router skills, applies the discovery-light runtime rule, and only then says that context is loaded.
