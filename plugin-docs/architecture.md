...
The hooks stay inert until `.freeflow/config.json` exists, parses, and matches the supported setup config shape. Once configured and enabled, they load `skills/decision-gate/references/runtime-kernel.md` plus any independently enabled capability context:

- at session start, including startup, resume, clear, and compact

`enabled: false` suppresses all Freeflow hook context; `skills.enabled: false` suppresses the runtime kernel and workflow skill exposure while leaving enabled capabilities available. Full `mode-contract`, `workflow`, `decision-gate`, `discover`, and lifecycle skill bodies remain available on demand. Hooks do not run after edit/write tools, block tools, grant permissions, enforce mode policy, replace repo instructions, or use host instruction files as activation markers.

Setup handles the same-session case directly: after successful setup verification, it reads and applies the canonical kernel plus any capability skill effective after setup and only then says that current-session context is loaded.
