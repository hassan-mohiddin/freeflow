	...
	for (const line of match[1].split("\n")) {
		...
		if (rawValue.includes(": ") && !/^["'|>]/.test(rawValue)) {
			frontmatterErrors.push(
				`frontmatter.${key} contains ': ' in a YAML plain scalar; quote the value`,
			);
		}
...
export async function validateSkill(input) {
	...
	const [resolvedSkillRoot, resolvedProjectRoot] = await Promise.all([
		realpath(skillRoot),
	...
	if (name && basename(skillRoot) !== name)
		warnings.push(
			`directory name '${basename(skillRoot)}' differs from skill name '${name}'`,
		);
	if (!/^#\s+\S/m.test(body))
		errors.push("SKILL.md body needs a top-level heading");
...
export async function inspectSkill(input) {
	...
	const foundWeak = weakPhrases.filter((phrase) =>
		body.toLowerCase().includes(phrase),
	);
	if (foundWeak.length)
		signals.push({ level: "advisory", code: "weak-verbs", detail: foundWeak });
	if (
		frontmatter?.description &&
		!/\bwhen\b|\buse\b/i.test(frontmatter.description)
	) {
		signals.push({
			level: "advisory",
	...
	if (
		/^\s*(?:>\s*)?(?:\*\*)?(?:Status|Readiness)(?::\*\*|\*\*:|:)\s*/im.test(
			body,
		)
	) {
		signals.push({
	...
	return {
		...
		signals,
		claim:
			"Static inspection reports structure and wording signals only; it does not prove activation or behavior.",
	};
}
