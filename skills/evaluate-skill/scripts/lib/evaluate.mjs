...
import { createManifest, captureGitEvidenceNonMutating, copyDirectory, initializeFixtureGit, makeWritable, materializeCompositionVariant, materializeSkillVariant, removeWritableTree } from "./materialize.mjs";
...
export function validateCompositionSkillReads(composition, skillReads, role) {
  ...
  if (JSON.stringify(actualNames) !== JSON.stringify(expectedNames)) throw new Error(`Composition activation evidence is missing or ambiguous for ${role}`);
}
...
export async function validateCompositionRuntimeImplementation(repoRoot, runtime, role) {
  for (const identity of Object.values(runtime.implementation_identity)) {
    ...
    if (actual !== identity.sha256) throw new Error(`Composition runtime implementation changed after approval for ${role}: ${identity.path}`);
  }
}

export function validateCompositionRuntimeEvidence(runtime, expected, records, expectedDeliveries, role) {
  if (!Array.isArray(records) || records.length !== expectedDeliveries) throw new Error(`Composition runtime delivery count is invalid for ${role}`);
...
async function executeVariant(workspace, plan, variant, evidenceDir, id, dependencies) {
  ...
  try {
    ...
    const assertWorkingSource = async (source, label) => {
      ...
      if (currentSourceHash !== source.snapshot_hash) throw new Error(`${label} source changed after preflight for ${variant.role}`);
    };
    ...
    if (plan.composition) {
      for (const component of plan.composition.base_stack) await assertWorkingSource(component, `Composition ${component.name}`);
      if (plan.composition.runtime) await assertWorkingSource(plan.composition.runtime, "Composition runtime");
      const materialized = await materializeCompositionVariant(workspace.repoRoot, plan.composition, variant, compositionRoot);
      ...
      for (const snapshot of skillSnapshots) {
        const source = snapshot.name === plan.composition.target_name
          ? variant
          : plan.composition.base_stack.find((component) => component.name === snapshot.name);
        const hash = await hashDirectory(snapshot.path);
        if (hash !== source.snapshot_hash) throw new Error(`Materialized composition skill differs from approved resources: ${snapshot.name}`);
    ...
    if (selectedHost !== "none") {
      ...
      subject = await subjectRunner({
        ...
        readRoots: [fixtureRoot, ...(skillSnapshots?.map((item) => item.path) ?? [snapshotRoot]), ...(runtimeDelivery ? [runtimeDelivery.materialized.runtime.path] : [])],
        ...
        onTurnSettled: rpcMode ? async () => {
          ...
          if (skillSnapshots) {
            for (const snapshot of skillSnapshots) {
              const hash = await hashDirectory(snapshot.path);
              if (hash !== compositionHashesBefore[snapshot.name]) throw new Error(`Composition skill mutated during ${variant.role}: ${snapshot.name}`);
              compositionHashes[snapshot.name] = hash;
            }
            if (runtimeDelivery) {
              await validateCompositionRuntimeImplementation(workspace.repoRoot, plan.composition.runtime, variant.role);
              const hash = await hashDirectory(runtimeDelivery.materialized.runtime.path);
              if (hash !== compositionHashesBefore.runtime) throw new Error(`Composition runtime mutated during ${variant.role}`);
    ...
    if (subject !== null && plan.composition) validateCompositionSkillReads(plan.composition, subject.parsed.skill_reads, variant.role);
    if (subject !== null && runtimeDelivery) {
      ...
      validateCompositionRuntimeEvidence(plan.composition.runtime, runtimeDelivery.expected, runtimeEvidence, expectedDeliveries, variant.role);
    }
    ...
    const metadata = {
      ...
      composition: plan.summary.composition ? {
        ...
        runtime: plan.summary.composition.runtime_profile ? { profile: plan.summary.composition.runtime_profile } : null,
      ...
      activation: {
        ...
        skill_reads: subject?.parsed.skill_reads ?? (skillSnapshots ? Object.fromEntries(skillSnapshots.map((item) => [item.name, false])) : null),
      },
      changed_paths: git.changedPaths,
    ...
    if (dependencies.persistVariantEvidence) {
      ...
      if (runtimeDelivery) {
        await cp(runtimeDelivery.environment.FREEFLOW_EVAL_RUNTIME_EVIDENCE, resolve(evidenceDir, "runtime-delivery.jsonl")).catch(() => {});
      }
    }
