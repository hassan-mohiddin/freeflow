...
function resourceIdentity(entries) {
  const ordered = [...entries].sort((left, right) => left[0].localeCompare(right[0]));
  return { entries: ordered, aggregate_sha256: sha256(stableJson(ordered)) };
}

export async function declaredResourceIdentity(root, resources) {
  return resourceIdentity(await filesystemEntries(root, [...resources].sort()));
}

export async function hashDeclaredResources(root, resources) {
  return (await declaredResourceIdentity(root, resources)).aggregate_sha256;
}

export function gitResourceIdentity(repoRoot, revision, rootPath, resources) {
  const root = rootPath.replace(/\/$/, "") === "." ? "" : rootPath.replace(/\/$/, "");
  const entries = [];
  ...
  return resourceIdentity(entries);
}

export function hashGitResources(repoRoot, revision, rootPath, resources) {
  return gitResourceIdentity(repoRoot, revision, rootPath, resources).aggregate_sha256;
}
