...
export function buildCodexInvocation({ workspace, model, thinking, skillName, prompt }) {
  ...
  return {
    ...
    args: [
      ...
      "-c",
      `model_reasoning_effort=${tomlString(thinking)}`,
      "-c",
...
export function compactCodexJsonLine(line, context = {}) {
  ...
  if (!record || typeof record !== "object" || Array.isArray(record) || typeof record.type !== "string")
    throw new Error("Malformed Codex JSONL record");
...
export function parseCodexJsonl(stdout) {
  ...
  for (const [index, line] of stdout.split("\n").entries()) {
    ...
    try {
      ...
      if (!record || typeof record !== "object" || Array.isArray(record) || typeof record.type !== "string")
        throw new Error("record must be an object with type");
      records.push(record);
    } catch (error) {
  ...
  if (threadStarted.length !== 1)
    parseErrors.push({ line: null, error: `Expected one thread.started; got ${threadStarted.length}` });
  if (turnStarted.length !== 1)
    parseErrors.push({ line: null, error: `Expected one turn.started; got ${turnStarted.length}` });
  if (turnCompleted.length !== 1)
    parseErrors.push({ line: null, error: `Expected one turn.completed; got ${turnCompleted.length}` });
  ...
  if (threadIndex !== 0)
    parseErrors.push({ line: null, error: "Codex thread.started must be the first retained record" });
  ...
  if (terminalIndex < 0 || terminalIndex !== records.length - 1)
    parseErrors.push({
      line: null,
      error: "Codex turn.completed must be the terminal retained record with no evidence after it",
    });
  if (byType("turn.failed").length > 0 || byType("error").length > 0)
    parseErrors.push({ line: null, error: "Codex reported failed terminal evidence" });
  ...
  const usage =
    usageRecord &&
    ["input_tokens", "cached_input_tokens", "output_tokens", "reasoning_output_tokens"].every((key) =>
      Number.isFinite(usageRecord[key]),
    )
      ? {
      ...
      : null;
  const toolEvents = records.filter(
    (record) =>
      ["item.started", "item.updated", "item.completed"].includes(record.type) &&
      ["command_execution", "file_change", "mcp_tool_call", "web_search"].includes(record.item?.type),
  );
...
export async function runCodexSubject({
  ...
  if (JSON.stringify(tools) !== JSON.stringify(["read", "write"]))
    throw new Error("Codex diagnostic tools must be exactly read, write");
  ...
  try {
    ...
    return {
      ...
      parsed: {
        events: [],
        parse_errors: [{ line: null, error: failure }],
        final_text: "",
        usage: null,
        tool_events: [],
        skill_read: false,
