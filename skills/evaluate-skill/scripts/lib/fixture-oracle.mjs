...
export async function inspectFixtureOracle(fixtureRoot, declaration) {
  ...
  for (const check of declaration.checks) {
    ...
    if (!isWithin(fixtureReal, await realpath(path)))
      throw new Error(`Fixture oracle path escapes fixture root: ${check.path}`);
    ...
    if (check.sha256 !== undefined && observation.sha256 !== check.sha256)
      failures.push({
        path: check.path,
    ...
    for (const value of check.contains ?? [])
      if (!content.includes(value)) failures.push({ path: check.path, reason: "missing-literal", value });
    for (const value of check.not_contains ?? [])
      if (content.includes(value)) failures.push({ path: check.path, reason: "forbidden-literal", value });
  }
