...
export default function registerSkillEvalGuard(pi) {
  ...
  pi.on("tool_call", (event, ctx) => {
    ...
    if (event.toolName !== "read" || typeof event.input?.path !== "string") return undefined;

    const requested = event.input.path.startsWith("@") ? event.input.path.slice(1) : event.input.path;
    let canonical;
    try {
      canonical = realpathSync(path.resolve(ctx.cwd, requested));
    } catch {
      return { block: true, reason: `Read path cannot be resolved inside the evaluation environment: ${requested}` };
    }
    if (!allowedRoots.some((root) => isContained(root, canonical))) {
      return { block: true, reason: `Read path escapes the evaluation environment: ${requested}` };
    }
    return undefined;
  });
}
