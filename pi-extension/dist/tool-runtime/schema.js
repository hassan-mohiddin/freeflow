...
const COMMON_KEYS = ["type", "description", "title", "enum", "const"];
...
function validateSchema(schema, path = "$", depth = 0) {
...
function check(value, schema, path) {
  ...
  if (schema.enum && !schema.enum.some((candidate) => equalJson(value, candidate))) return `${path} is not in enum.`;
  switch (schema.type) {
    case "null":
  ...
  return Object.freeze({
    ...
    validate(value) {
      try {
        ...
        const error = check(frozen, schema, "$input");
        return error ? { ok: false, error } : { ok: true, value: frozen };
      } catch (error) {
