...
export function validateProgramRequest(value, defaultTimeoutMs = PROGRAM_LIMITS.defaultTimeoutMs) {
  ...
  if (
    ...
    new Set(operations.map((key) => JSON.stringify([key.id, key.revision]))).size !== operations.length
  )
  ...
  if (captures.length > 32 || !captures.every((id) => identity(id, 256)) || new Set(captures).size !== captures.length)
  ...
  return Object.freeze({
    ...
    operations: Object.freeze(operations.map((key) => Object.freeze({ ...key }))),
