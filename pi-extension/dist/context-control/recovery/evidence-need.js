import { EVIDENCE_CARDINALITY_KINDS, EVIDENCE_ROLES, EVIDENCE_TEMPORAL_SCOPES } from "../core/types.js";
import { EVIDENCE_SCOPE_TIERS } from "./scope-catalog.js";
export const EVIDENCE_NEED_VERSION = "0.1";
...
function normalizeList(value, field, maxItems, maxLength, caseInsensitive) {
  ...
  for (const item of value) {
    const normalized = normalizeText(item, field, maxLength);
...
function normalizeScope(value) {
  ...
  const session = input.session === undefined ? undefined : enumValue(input.session, ["current"], "scope.session");
  const branch = input.branch === undefined ? undefined : enumValue(input.branch, ["active"], "scope.branch");
  const maxTier =
...
function normalizeIntent(value, role, temporal) {
...
function normalizeCardinality(value) {
  ...
  if (!Number.isSafeInteger(input.maxSources) || input.maxSources < 1 || input.maxSources > 8) {
    throw new EvidenceNeedError("invalid-cardinality", "set/comparison maxSources must be an integer from 1 through 8");
  }
  return Object.freeze({ kind, maxSources: input.maxSources });
}
export function normalizeEvidenceNeed(value) {
  const input = record(value);
  ...
  const text = normalizeText(input.text, "EvidenceNeed.text", MAX_EVIDENCE_NEED_TEXT);
  ...
  const exactRequired = input.exactRequired === undefined ? undefined : input.exactRequired;
  const expectedEvidence =
    input.expectedEvidence === undefined
      ? undefined
      : normalizeText(input.expectedEvidence, "EvidenceNeed.expectedEvidence", MAX_EVIDENCE_NEED_TEXT);
...
export function validateEvidenceNeed(value) {
