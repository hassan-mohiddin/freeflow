...
import { rankMatchingPassages } from "../search/ranking.js";
...
const GENERIC_KINDS = ["user", "assistant", "toolResult", "summary"];
const DEFAULT_GENERIC_KINDS = ["user", "assistant", "summary"];
...
const EXACT_MATCH_CLASSES = new Set(["exact", "strong-structured"]);
function sourceToolName(source) {
...
function sourceMatchesScope(source, need, currentSessionId, activeBranchId, configuredMaxTier) {
...
function matchedIdentifiers(source, need) {
...
function candidateFor(source, need) {
...
function selectSingle(need, ranked) {
...
export function resolveGenericRecovery(needValue, sources, currentSessionId, activeBranchId, configuredMaxTier) {
  const normalized = normalizeEvidenceNeed(needValue);
