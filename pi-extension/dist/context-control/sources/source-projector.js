import { contextRefForEntry, isContextControlToolName } from "./types.js";
...
function toolCallParts(content) {
  ...
  for (const block of content) {
    ...
    if (isContextControlToolName(name)) continue;
...
export function projectContextSource(source, options = {}) {
  ...
  if (source.kind === "summary") {
    ...
    if (isContextControlToolName(message?.toolName) && options.includeContextControlResults !== true) {
      return { status: "excluded", reason: "context_control_source" };
    }
