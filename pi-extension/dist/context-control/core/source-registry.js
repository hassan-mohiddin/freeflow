...
export function isContextControlGeneratedTool(toolName) {
  return CONTEXT_CONTROL_TOOL_PATTERN.test(normalizedToolName(toolName));
}
export function isSensitiveRecoverySource(source) {
...
export function isRecoveryEligibleSource(source) {
  return !isSensitiveRecoverySource(source);
}
function sourceCategory(toolName, content) {
...
function sourceRole(toolName, kind) {
...
function sourcePrivacy(category) {
  return category === "ordinary" ? "allowed" : "unknown";
}
function sourceIntegrity(content, contentHash) {
...
function sourceIdentity(sessionId, entryId, toolCallId, toolName) {
  return {
    sessionId,
    entryId,
...
function entryIdSet(entries) {
  return new Set(entries.map((entry) => stringValue(record(entry).id)).filter((id) => id !== undefined));
}
function sessionHeader(entries) {
  return entries.find((entry) => record(entry).type === "session") ?? {};
}
...
function sessionFileCandidates(ctx) {
...
function jsonlFiles(root, limit = 512) {
...
function refFor(entryId, sessionId, includeSessionInRef) {
...
function buildSources(entries, options) {
  const calls = new Map();
  const duplicateCalls = new Set();
  const historicallyConsumed = new Set();
  const parentById = new Map();
  for (const entryValue of entries) {
    const entry = record(entryValue);
    const id = stringValue(entry.id);
...
export function repositoryIdentityForCwd(cwd) {
  return repositoryIdentity(cwd);
}
export function isSessionPath(value) {
