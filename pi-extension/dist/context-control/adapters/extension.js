...
import { CONTEXT_CONTROL_RUNTIME_MODES } from "../core/types.js";
export const CONTEXT_CONTROL_EXTENSION_VERSION = "0.2";
function validMode(value) {
  if (typeof value === "string" && CONTEXT_CONTROL_RUNTIME_MODES.includes(value)) {
    return value;
  }
  throw new Error(`FREEFLOW_CONTEXT_CONTROL_EXPERIMENT must be one of: ${CONTEXT_CONTROL_RUNTIME_MODES.join(", ")}`);
}
function sidecarRoot(ctx, outputDir) {
...
async function configuredContextControl(ctx, pi) {
...
export function createContextControlExtension(pi, options = {}) {
  ...
  state.start = async (ctx) => bind(ctx);
  state.reload = async (ctx) => {
    if (runtime !== undefined) await runtime.shutdown("configuration-reload");
    runtime = undefined;
    return bind(ctx);
  };
  state.purge = async (ctx) => {
  ...
  state.setPrompt = (prompt) => runtime?.setPrompt(prompt);
  state.project = async (messages, ctx) => {
  ...
  state.messageEnd = async (message) => (runtime ? runtime.messageEnd(message) : undefined);
  ...
  if (options.registerLifecycle !== false) {
    ...
    pi.on("message_end", async (event) => state.messageEnd(event?.message));
    pi.on("agent_end", async () => undefined);
    pi.on("agent_settled", async () => state.settled());
  }
  return state;
}
export default function contextControlExtension(pi) {
