...
export function compactRunToolText(result) {
    ...
    if (result?.routing?.status)
        header.push(`route=${result.routing.status}`);
    if (result?.parser?.name)
        header.push(`parser=${result.parser.name}`);
    ...
    if (result?.scriptFilter?.outputId)
        header.push(`derived=${result.scriptFilter.outputId}`);
...
function compactProcessingToolText(result) {
    ...
    if (Array.isArray(result?.facts) && result.facts.length > 0)
        lines.push(row("facts", result.facts.length));
    if (result?.recovery?.outputId)
        lines.push(row("rec", "vault", result.recovery.outputId));
...
function compactBatchStepLine(step) {
    ...
    if (result?.routing?.status)
        parts.push(`route=${result.routing.status}`);
    if (step?.durationMs !== undefined)
        parts.push(`${step.durationMs}ms`);
    if (outputId)
        parts.push(`out=${outputId}`);
    if (Array.isArray(result?.evidence))
        parts.push(`e=${result.evidence.length}`);
    if (Array.isArray(result?.importantLines))
        parts.push(`p=${result.importantLines.length}`);
    if (result?.execution?.status)
        parts.push(`exec=${result.execution.status}`);
    const message = step?.error ?? result?.failure?.message ?? result?.summary;
    if (message)
        parts.push(truncateRawLine(message, 160));
    return row(...parts);
}
