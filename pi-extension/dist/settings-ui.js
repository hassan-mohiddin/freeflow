...
function matchesKeybinding(keybindings, data, keybinding, fallback) {
    try {
        if (keybindings?.matches?.(data, keybinding))
            return true;
    }
    catch {
...
function walkSettingsItems(items, visitor) {
    for (const item of items) {
        visitor(item);
        if (item.children)
            walkSettingsItems(item.children, visitor);
    }
}
...
class FreeflowSettingsComponent {
    ...
    handleInput(data) {
        ...
        if (this.matches(data, "tui.select.up", isUp)) {
            if (frame.items.length > 0)
                frame.selected = frame.selected === 0 ? frame.items.length - 1 : frame.selected - 1;
        }
        else if (this.matches(data, "tui.select.down", isDown)) {
            if (frame.items.length > 0)
                frame.selected = frame.selected === frame.items.length - 1 ? 0 : frame.selected + 1;
        }
    ...
    styleLine(line, item, selected) {
        if (item.inactive)
            return this.theme.fg?.("dim", line) ?? line;
        if (selected)
            return this.theme.fg?.("accent", line) ?? line;
        return line;
    }
    activateSelected() {
