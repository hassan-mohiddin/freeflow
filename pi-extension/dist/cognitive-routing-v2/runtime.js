...
export class RoutingRuntime {
  ...
  observationScope() {
    ...
    const profile = runtime.activeProfile ?? "solo";
    const control = String(runtime.controlMode).startsWith("manual-")
      ? "manual"
  ...
  admitOperation(scope, effect, _operation, ctx = this.ctx) {
    try {
  ...
  admitProgram(scope, ctx = this.ctx) {
    try {
  ...
  prepared(profile, input, handoffId, restoring = false) {
    ...
    return prepareView({
      ...
      tools: (this.pi.getAllTools?.() ?? [])
        .filter((tool) => this.pi.getActiveTools?.().includes(tool.name))
        .map((tool) =>
  ...
  budgetNotice(messages, ctx) {
    ...
    const tools = (this.pi.getAllTools?.() ?? []).filter((tool) => this.pi.getActiveTools?.().includes(tool.name));
  ...
  async unit(input, callId, op) {
    ...
    if (input.operation === "recover") {
      ...
      const requestedResults = [...(input.results ?? [])];
