...
import { CmuxAdapter, assertLeafProfilesDoNotIncludeDelegationTools, compileTaskPacket, createDelegationStore, delegationRootForRepo, resolveProfileForRole, shellQuote, validateSafeId, } from "../../../delegation/dist/index.js";
...
export function registerDelegationTools(pi) {
    ...
    const toolDefinitions = [
        ...
        delegationTool("delegate_wait", "Delegate Wait", "P4 lifecycle surface placeholder for explicit bounded watch mode.", TARGET_PARAMETERS, (params, _signal, ctx) => lifecyclePending("delegate_wait", params, ctx, "P4 owns bounded watch mode, timeout heartbeat, and retry caps.")),
        delegationTool("delegate_result", "Delegate Result", "P4 lifecycle surface placeholder for compact parsed result retrieval.", TARGET_PARAMETERS, (params, _signal, ctx) => lifecyclePending("delegate_result", params, ctx, "P4 owns result/report retrieval lifecycle beyond stored pointers.")),
        ...
        delegationTool("delegate_cancel", "Delegate Cancel", "P4 lifecycle surface placeholder for cancellation semantics.", CLOSE_PARAMETERS, (params, _signal, ctx) => lifecyclePending("delegate_cancel", params, ctx, "P4 owns complete cancellation semantics; no pane action was attempted.")),
        ...
        delegationTool("delegate_record_report", "Delegate Record Report", "P4 lifecycle surface placeholder for parent report recording.", REPORT_PARAMETERS, (params, _signal, ctx) => lifecyclePending("delegate_record_report", params, ctx, "P4 owns parent report lifecycle and alerting.")),
    ];
...
async function lifecyclePending(operation, params, ctx, reason) {
    const store = createStore(ctx);
    const taskId = stringOrUndefined(params.taskId);
    const agentId = stringOrUndefined(params.agentId);
    return {
