...
async function executeFinish(params, ctx) {
    ...
    const status = await store.writeAgentStatus(target.taskId, target.agentId, { state: state, message: validation.summary, reason: validation.status === "blocked" || validation.status === "failed" ? validation.summary : undefined });
    const event = await store.appendAgentEvent(target.taskId, target.agentId, { type: "agent-result", state: state, message: validation.summary, data: { resultStatus: validation.status, jsonPath: paths.jsonPath, filesChanged: payload.filesChanged, findings: payload.findings, checks: payload.checks } });
    await store.appendTaskEvent(target.taskId, { type: "agent-result", state: state, message: validation.summary, data: { agentId: target.agentId, resultStatus: validation.status, jsonPath: paths.jsonPath, filesChanged: payload.filesChanged, findings: payload.findings, checks: payload.checks } });
    const alert = await store.queueParentAlert(target.taskId, { agentId: target.agentId, outcome: alertOutcomeForResultStatus(validation.status), state: state, status: validation.status, eventType: "agent-result", sourceEventId: event.eventId, message: validation.summary, evidence: { jsonPath: paths.jsonPath }, data: { resultStatus: validation.status, filesChanged: payload.filesChanged, findings: payload.findings, checks: payload.checks } });
    return {
        toolStatus: "ok",
...
async function finishParentReport(params, validation, target, store, role) {
    ...
    const status = await store.writeAgentStatus(target.taskId, target.agentId, { state: state, message: validation.summary, reason: state === "blocked" || state === "failed" ? validation.summary : undefined });
    const event = await store.appendAgentEvent(target.taskId, target.agentId, { type: `task-${reportName}`, state: state, message: `${reportName} recorded via delegate_finish${report.status ? `: ${report.status}` : ""}`, data: { reportName, status: report.status, rawPath: reportPaths.rawPath, jsonPath: reportPaths.jsonPath, resultJsonPath: resultPaths.jsonPath } });
    await store.appendTaskEvent(target.taskId, { type: `task-${reportName}`, state: state, message: `${reportName} recorded via delegate_finish${report.status ? `: ${report.status}` : ""}`, data: { agentId: target.agentId, reportName, status: report.status, rawPath: reportPaths.rawPath, jsonPath: reportPaths.jsonPath, resultJsonPath: resultPaths.jsonPath } });
    const outcome = alertOutcomeForRecordedReport(reportName, report.status, state);
    const alert = outcome === undefined ? undefined : await store.queueParentAlert(target.taskId, { agentId: target.agentId, outcome: outcome, state: state, status: report.status, eventType: `task-${reportName}`, sourceEventId: event.eventId, message: validation.summary, evidence: { rawPath: reportPaths.rawPath, jsonPath: reportPaths.jsonPath }, data: { reportName, resultStatus: validation.status, reportStatus: report.status } });
    return {
        toolStatus: "ok",
        operation: "delegate_finish",
        status: "stored",
...
function stateForResultStatus(status) {
    if (status === "completed" || status === "completed_with_risks")
        return "completed";
    return status;
}
function alertOutcomeForResultStatus(status) {
...
function appendAlertRows(lines, alerts, label) {
    ...
    if (alerts.length > 5)
        lines.push(row(`${label}s_more`, alerts.length - 5));
}
