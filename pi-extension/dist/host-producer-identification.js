...
export function resolvePiObservedRoutingDecision(event, observedRoutingConfig) {
...
export function identifyPiMcpProducer(event) {
    ...
    const server = firstString(input.server, input.mcpServer, inputProducer.server, details.server, detailMcp.server, detailProducer.server, split?.server);
    const tool = firstString(input.tool, input.name, inputProducer.tool, details.tool, detailMcp.tool, detailProducer.tool, split?.tool);
    if (!server || !tool) {
        return undefined;
    }
...
export function identifyBuiltInPiProducer(event) {
    const toolName = typeof event?.toolName === "string" ? event.toolName : "";
    return BUILT_IN_PI_PRODUCERS[toolName];
}
export function classifyObservedProducerRisk(producer, event) {
...
function mcpAnnotationRisk(event) {
...
function matchesVerb(toolName, verbs) {
