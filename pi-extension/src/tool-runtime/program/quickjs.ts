...
import { PROGRAM_LIMITS, ProgramLimitError, type ProgramRequest } from "./limits.js";
...
export type GuestTerminal =
  { type: "finished"; detached: boolean } | { type: "failed"; code: string; message: string; detached: boolean };

export class QuickJSRunner {
  private runtime: any;
  private context: any;
  private pending = new Map<number, any>();
  private immediate = new Set<any>();
  private sequence = 0;
  private emitSequence = 0;
  private submitted = 0;
  private cancelled = false;
  private jobError: string | undefined;
  private pendingWaiters: (() => void)[] = [];

  constructor(
    private readonly setup: WorkerSetup,
    private readonly send: (frame: GuestFrame) => void,
  ) {}
  ...
  private async waitPending(): Promise<void> {
    if (this.pending.size === 0) return;
    await new Promise<void>((resolve) => this.pendingWaiters.push(resolve));
  }
  ...
  private errorText(handle: any): string {
    try {
  ...
  private immediateOutcome(json: string): any {
  ...
  private installBindings(): void {
    const invoke = this.context.newFunction("__ffInvoke", (kindHandle: any, idHandle: any, rawHandle: any) => {
    ...
    const emit = this.context.newFunction("__ffEmit", (rawHandle: any) => {
  ...
  reply(frame: Extract<HostFrame, { type: "reply" }>): void {
  ...
  async run(): Promise<GuestTerminal> {
    const QuickJS = await newQuickJSWASMModuleFromVariant(RELEASE_SYNC as any);
