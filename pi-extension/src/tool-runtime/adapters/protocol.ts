...
import { OperationRegistry, RegistryError } from "../registry.js";

export type AdapterBundleV1 = Readonly<{
  protocol: 1;
  id: string;
  revision: string;
  ...
  policy: Readonly<{
    ...
    settlement: "effect-aware";
  }>;
  operations: readonly Operation<Json, Json>[];
  dispose(): void;
}>;
...
type StoredBundle = {
  bundle: AdapterBundleV1;
  registration?: { dispose(): void };
};

function exact(value: any, required: readonly string[]): boolean {
  return (
    value &&
    typeof value === "object" &&
    !Array.isArray(value) &&
...
function boundedOutputSchema(schema: any, depth = 0): boolean {
...
function snapshotBundle(bundle: AdapterBundleV1): AdapterBundleV1 {
...
export class CooperatingAdapterRuntime {
  ...
  private readonly failures: { code: string; message: string }[] = [];
  ...
  private fail(code: string, message: string): AdapterRegistration {
  ...
  private validate(bundle: AdapterBundleV1): AdapterBundleV1 {
    if (
  ...
  private activate(stored: StoredBundle): void {
  ...
  register(bundle: AdapterBundleV1): AdapterRegistration {
    ...
    const stored: StoredBundle = { bundle: accepted };
  ...
  status(): {
    ...
    announced: readonly { id: string; revision: string; executionWorld: string; active: boolean }[];
...
export function publishCooperatingAdapterEndpoint(runtime: CooperatingAdapterRuntime): { dispose(): void } {
  ...
  (globalThis as any)[ENDPOINT] = endpoint;
  return {
    dispose() {
      if ((globalThis as any)[ENDPOINT] === endpoint) delete (globalThis as any)[ENDPOINT];
    },
  };
}

export function registerCooperatingAdapter(bundle: AdapterBundleV1): AdapterRegistration {
  const endpoint = (globalThis as any)[ENDPOINT] as Endpoint | undefined;
  return endpoint
