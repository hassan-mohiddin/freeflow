...
function rangeEnd(buffer: Buffer, start: number, maximum: number): number {
  let end = Math.min(buffer.length, start + maximum);
  while (end > start && end < buffer.length && continuation(buffer[end])) end -= 1;
  return end;
}

export function createReadTextOperation(state: () => ToolExecutionState | undefined): Operation<Json, Json> {
  return {
    ...
    async authorize(input, scope) {
      try {
        await resolveWorkspaceFile(state(), scope, (input as any).path);
        return { kind: "allowed" };
      } catch (error) {
        return {
          kind: "denied",
