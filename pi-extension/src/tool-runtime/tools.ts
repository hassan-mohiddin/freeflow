...
type ToolRuntimeToolName = (typeof TOOL_RUNTIME_TOOL_NAMES)[number];

const string = (maxLength: number, description?: string) => ({
  type: "string",
  minLength: 1,
  maxLength,
...
const branch = (properties: Record<string, unknown>, required: string[]) => ({
  type: "object",
  additionalProperties: false,
  properties,
  required,
...
export const TOOL_RUNTIME_SCHEMAS: Record<ToolRuntimeToolName, any> = {
  freeflow_tools: {
    type: "object",
    additionalProperties: false,
    properties: {
...
function json(value: unknown, seen = new Set<object>(), depth = 0): boolean {
  ...
  if (typeof value !== "object" || depth > 64 || seen.has(value as object)) return false;
  seen.add(value as object);
  const valid = Array.isArray(value)
    ? value.length <= 100_000 && value.every((item) => json(item, seen, depth + 1))
    : Object.keys(value as Record<string, unknown>).length <= 100_000 &&
      Object.entries(value as Record<string, unknown>).every(
        ([key, item]) => key.length <= 4096 && json(item, seen, depth + 1),
      );
  seen.delete(value as object);
  return valid;
}

function validOperationKey(value: unknown): boolean {
  return (
    record(value) &&
...
export function validToolRuntimeInput(name: ToolRuntimeToolName, value: unknown): boolean {
...
const descriptions: Record<ToolRuntimeToolName, string> = {
  freeflow_tools:
