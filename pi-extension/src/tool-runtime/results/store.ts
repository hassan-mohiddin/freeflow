...
function sessionFacts(ctx: any): { sessionFile: string; sessionId: string } {
...
function namespaceParts(ctx: any, originSessionId: string): string[] {
...
function namespaceRoot(ctx: any, originSessionId: string): string {
  return namespaceParts(ctx, originSessionId).at(-1)!;
}

async function verifiedNamespace(ctx: any, originSessionId: string, create: boolean): Promise<string> {
...
function capturePath(ctx: any, descriptor: CaptureDescriptorV1): string {
...
async function namespaceBytes(root: string): Promise<number> {
  let names: string[];
  try {
...
export class CaptureStore {
  ...
  async read(ctx: any, descriptor: CaptureDescriptorV1): Promise<Buffer> {
