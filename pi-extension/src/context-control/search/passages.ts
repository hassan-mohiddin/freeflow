...
export type ContextTextPassage = {
  text: string;
  start: number;
  end: number;
  tokens: string[];
...
export function splitTextIntoPassages(text: string, signal?: AbortSignal): ContextTextPassage[] {
  throwIfAborted(signal);
  if (text.length <= PASSAGE_TARGET_CHARACTERS) {
    const tokens = tokenize(text);
    return [{ text, start: 0, end: text.length, tokens, length: Math.max(1, tokens.length) }];
  }
  const result: ContextTextPassage[] = [];
  let start = 0;
...
export function snippet(text: string, phrase: string, terms: readonly string[]): string {
