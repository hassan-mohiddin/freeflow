...
import type { ContextSourceIdentity } from "../core/types.js";

export const CARRY_FORWARD_SCOPES = ["source", "activity", "artifact"] as const;
export const CARRY_FORWARD_OWNERS = ["model", "artifact", "user"] as const;
export const CARRY_FORWARD_STATES = ["active", "superseded", "retired"] as const;

export type CarryForwardScope = (typeof CARRY_FORWARD_SCOPES)[number];
export type CarryForwardOwner = (typeof CARRY_FORWARD_OWNERS)[number];
export type CarryForwardState = (typeof CARRY_FORWARD_STATES)[number];
...
export interface CarryForwardRecord {
  id: string;
  meaning: string;
  sources: readonly ContextSourceIdentity[];
  scope: CarryForwardScope;
  owner: CarryForwardOwner;
  state: CarryForwardState;
  checkpointId: string;
  supersededBy?: string;
  provenance?: readonly CarryForwardProvenance[];
}

export type CarryForwardView = Readonly<CarryForwardRecord>;

export type CarryForwardExactEvidenceResult =
  | {
...
export type CarryForwardWorkingStateResult =
  | { status: "available"; recordId: string; record: CarryForwardView }
  | { status: "unavailable"; recordId: string; reason: "missing" | "inactive" };

export interface CarryForwardChange {
  recordId: string;
  supersededBy?: string;
  checkpointId: string;
  reason: "owner-precedence" | "broader-scope" | "explicit-supersede" | "explicit-retire";
}
...
const OWNER_RANK: Readonly<Record<CarryForwardOwner, number>> = Object.freeze({ model: 0, artifact: 1, user: 2 });
const SCOPE_RANK: Readonly<Record<CarryForwardScope, number>> = Object.freeze({ source: 0, activity: 1, artifact: 2 });

function validText(value: unknown, max: number): value is string {
  return typeof value === "string" && value.trim() !== "" && value.length <= max;
}

function cloneIdentity(value: ContextSourceIdentity): ContextSourceIdentity {
  return Object.freeze({ ...value });
}

function cloneProvenance(value: CarryForwardProvenance): CarryForwardProvenance {
...
function cloneRecord(value: CarryForwardRecord): CarryForwardView {
  return Object.freeze({
    ...value,
...
function preference(left: CarryForwardRecord, right: CarryForwardRecord): number {
  return OWNER_RANK[left.owner] - OWNER_RANK[right.owner] || SCOPE_RANK[left.scope] - SCOPE_RANK[right.scope];
}

function sameMeaning(left: CarryForwardRecord, right: CarryForwardRecord): boolean {
  return left.meaning.trim().replace(/\s+/gu, " ") === right.meaning.trim().replace(/\s+/gu, " ");
}

export class CarryForwardRegistry {
  private readonly records = new Map<string, CarryForwardView>();
  private readonly changes: CarryForwardChange[] = [];

  constructor(private readonly sourceExists: (identity: ContextSourceIdentity) => boolean) {}

  register(input: CarryForwardRecord): CarryForwardView {
  ...
  supersede(recordId: string, replacement: CarryForwardRecord, checkpointId: string): CarryForwardView {
  ...
  retire(recordId: string, checkpointId: string): CarryForwardView {
    const current = this.records.get(recordId);
    if (current === undefined || current.state !== "active")
      throw new CarryForwardError("not-active", "Carry-forward record is not active");
    if (!validText(checkpointId, 256)) throw new CarryForwardError("invalid-checkpoint", "Checkpoint is invalid");
  ...
  get(recordId: string): CarryForwardView | undefined {
    const value = this.records.get(recordId);
    return value === undefined ? undefined : cloneRecord(value);
  }

  use(recordId: string, purpose: "working_state"): CarryForwardWorkingStateResult;
  use(recordId: string, purpose: "quotation" | "comparison"): CarryForwardExactEvidenceResult;
  use(
    recordId: string,
    purpose: "working_state" | "quotation" | "comparison",
  ): CarryForwardWorkingStateResult | CarryForwardExactEvidenceResult {
    const value = this.records.get(recordId);
  ...
  active(): readonly CarryForwardView[] {
    return Object.freeze(
      [...this.records.values()]
  ...
  changesView(): readonly CarryForwardChange[] {
    return Object.freeze(this.changes.map((change) => Object.freeze({ ...change })));
  }

  private validateActive(input: CarryForwardRecord): CarryForwardRecord {
