...
import type {
  ...
  ContextControlProjectionResult,
  ContextControlPinSnapshot,
  ContextControlProposal,
  ContextControlRecoveryScope,
  ContextControlRuntimeOptions,
  ContextControlSource,
  ContextControlSourceSnapshot,
  ContextControlStatus,
  ContextControlOperation,
...
function currentProjection(projection: ResidencyProjection | undefined): ContextProjection {
...
function sourceKey(identity: ContextSourceIdentity): string {
...
function hasToolCall(message: any): boolean {
  return (
    ...
    (Array.isArray(message?.content) &&
      message.content.some((block: any) => ["toolCall", "tool_use"].includes(block?.type)))
  );
}
...
function normalizeExactEvidence(value: string): string {
  return value.replace(/^\n+|\n+$/gu, "");
}

function candidateCardFromTiered(card: TieredSearchCandidateCard, handle?: string): ContextControlCandidateCard {
  return {
...
function cleanupProposalSourceIndexVersion(
  refs: readonly string[],
  snapshot: ContextControlSourceSnapshot,
  projections: ReadonlyMap<string, ResidencyProjection>,
): string {
  return sha256Text(
    stableJson(
      refs.map((ref) => {
...
function maxTierForScope(scope: ContextControlRecoveryScope): "active-branch" | "current-session" | "cross-session" {
  if (scope === "active-branch") return "active-branch";
  if (scope === "current-session") return "current-session";
  return "cross-session";
}

function listRemainsWithin(base: readonly string[] | undefined, requested: readonly string[] | undefined): boolean {
  return base === undefined || (requested !== undefined && requested.every((value) => base.includes(value)));
}

function recoveryNeedWidens(
  base: EvidenceNeed,
  requested: EvidenceNeed,
  policyMaxTier: "active-branch" | "current-session" | "cross-session",
): boolean {
...
class NullAuditSink implements ContextControlAuditSink {
  async record(_event: ContextControlAuditEvent): Promise<void> {
    return undefined;
  }
}
...
export class ContextControlRuntime {
  ...
  private readonly cleanupMode: "model-only" | "model-approval" | "automatic";
  private readonly recoveryMode: "model-only" | "model-approval" | "automatic";
  private readonly recoveryScope: ContextControlRecoveryScope;
  private readonly projections = new Map<string, ResidencyProjection>();
  private readonly carryForward: CarryForwardRegistry;
  private readonly pinPriorStates = new Map<string, ContextControlPinSnapshot>();
  private readonly leases = new Map<string, ContextControlEvidenceLease>();
  private readonly checkpointDetector = new CheckpointEvidenceNeedDetector();
  private readonly auditEvents: ContextControlAuditEvent[] = [];
  private auditFailureCount = 0;
  private pendingProposal: ContextControlProposal | undefined;
  private pendingProactiveRecoveryKey: string | undefined;
  private prompt: string | undefined;
  private proactiveRecoveryKey: string | undefined;
  private started = false;
  private state: ContextControlStatus["state"] = "disabled";
  private lastError: string | undefined;
  private sessionId: string | undefined;
  private branchId: string | undefined;
  private generation = 0;
  private checkpointId: string | undefined;
  private visibleToolCallIds = new Set<string>();
  ...
  readonly mode: ContextControlRuntimeOptions["mode"];

  async start(): Promise<{ status: "ready" | "disabled"; reason?: string }> {
    this.started = true;
  ...
  async project(messages: readonly any[]): Promise<ContextControlProjectionResult> {
    const unchanged: ContextControlProjectionResult = {
      messages,
      changed: false,
    ...
    let proposal: ContextControlProposal | undefined =
      this.pendingProposal !== undefined &&
  ...
  async cleanup(targets: unknown): Promise<any> {
    if (this.mode === "shadow") {
  ...
  async decideProposal(input: any): Promise<any> {
    ...
    let cleanupSnapshot: ContextControlSourceSnapshot | undefined;
    ...
    if (
      ...
      recoveryNeedWidens(
        proposal.need as EvidenceNeed,
        recoveryValidation.decision.need,
        maxTierForScope(this.recoveryScope),
      )
    ) {
    ...
    if (proposal.kind === "recovery") {
      ...
      if (decision.action === "modify") {
        ...
        recovered = await this.recover(proposal.need as EvidenceNeed, candidate.ref);
      }
    ...
    const dispositions =
      action === "approve"
        ? proposal.refs.map((ref) => ({ ref, state: "reference" as const }))
        : requested.map((change: any) => {
    ...
    if (
      ...
      !uniqueRefs(dispositions.map((change: any) => change.ref))
    ) {
      return { status: "rejected", operation: "decide", reason: "decision_out_of_scope" };
    }
    if (
      dispositions.some(
        (disposition: any) => disposition.state !== "full" && this.activeLeaseForSource(disposition.ref),
      )
    ) {
      return { status: "rejected", operation: "decide", reason: "decision_protected_by_exact_lease" };
    }
    const changes: ContextControlJournalChange[] = dispositions.flatMap((disposition: any) => {
  ...
  async recover(need: EvidenceNeed, expectedRef?: string): Promise<RecoveryResult> {
    ...
    let normalizedNeed: EvidenceNeed;
    try {
      normalizedNeed = normalizeEvidenceNeed(need) as EvidenceNeed;
    } catch {
      return { status: "unavailable", reason: "invalid-evidence-need" };
    }

    let snapshot: ContextControlSourceSnapshot;
    let searchResult: ReturnType<typeof searchTieredEvidence>;
    try {
      snapshot = this.snapshot();
      this.rebindIfNeeded(snapshot);
    ...
    let completeness: "complete" | "partial" = sourceCompleteness === "partial" ? "partial" : "complete";
  ...
  private async recoverSelectedSet(
    need: EvidenceNeed,
    snapshot: ContextControlSourceSnapshot,
    searchResult: ReturnType<typeof searchTieredEvidence>,
    resolution: Extract<TieredSearchResolution, { status: "selected-set" }>,
    ...
    const sources = selected as ContextControlSource[];
    ...
    const changes: ContextControlJournalChange[] = [];
    const leases: ContextControlEvidenceLease[] = [];
    const modes: Array<"none" | "restore" | "retrieve"> = [];
  ...
  async pin(refs: unknown): Promise<any> {
    return this.updatePins(refs, true);
  }

  async unpin(refs: unknown): Promise<any> {
  ...
  async useEvidence(input: any): Promise<any> {
  ...
  acknowledgeEvidenceUse(handle: unknown, excerpt: unknown): any {
  ...
  async messageEnd(message: any): Promise<any> {
  ...
  async purge(): Promise<{ status: "ok" | "unavailable"; operation: "purge"; reason?: string }> {
    try {
  ...
  status(): ContextControlStatus {
    const residency: Record<string, ResidencyState> = {};
  ...
  audit(): readonly ContextControlAuditEvent[] {
    return Object.freeze(
  ...
  private createAbstentionHandle(need: EvidenceNeed, resolution: "ambiguous" | "unavailable"): string {
  ...
  private proposalFingerprintFor(proposal: ContextControlProposal): string {
  ...
  private async persistProposalDisposition(proposal: ContextControlProposal): Promise<void> {
  ...
  private recoveryUnavailable(need: EvidenceNeed, reason: string): RecoveryResult {
    return {
      status: "unavailable",
      reason,
  ...
  private clearLeases(reason: string): void {
  ...
  private carryForwardDescriptor(
    sourceRef: string,
    meaning: string,
    checkpointId: string,
  ): ContextControlCarryForwardDescriptor {
  ...
  private carryForwardRecordFor(change: ContextControlJournalChange): CarryForwardRecord | undefined {
  ...
  private syncCarryForward(change: ContextControlJournalChange): void {
  ...
  private snapshot(): ContextControlSourceSnapshot {
  ...
  private rebindIfNeeded(snapshot: ContextControlSourceSnapshot): void {
  ...
  private replay(snapshot: ContextControlSourceSnapshot): void {
    this.projections.clear();
    this.carryForward.clear();
    this.pinPriorStates.clear();
    this.suppressedProposalFingerprints.clear();
  ...
  private proactiveRecovery(snapshot: ContextControlSourceSnapshot):
    | {
        need: EvidenceNeed;
        sourceRef: string;
        key: string;
  ...
  private ensureRecoveryProposal(
    need: EvidenceNeed,
    sourceRef: string,
    reason: string,
    proactiveKey: string,
    snapshot: ContextControlSourceSnapshot,
  ): ContextControlProposal | undefined {
    if (
    ...
    const alternativeHandles = Object.freeze(
      candidateCards
        ...
        .filter((handle): handle is string => handle !== undefined && !selectedHandles.includes(handle)),
    );
    const proposal: ContextControlProposal = {
      id,
      kind: "recovery",
  ...
  private recoveryProposalEnvelope(proposal: ContextControlProposal): RecoveryProposalEnvelope | undefined {
    if (
      proposal.kind !== "recovery" ||
    ...
    try {
      return buildRecoveryProposal({
        ...
        candidates: candidates as NonNullable<(typeof candidates)[number]>[],
      });
    } catch {
      return undefined;
    }
  ...
  private recoveryMessage(result: Extract<RecoveryResult, { status: "recovered" }>): any {
    return {
      role: "custom",
        ...
        return [
          {
            ...
            ref,
            ...(source.path === undefined ? {} : { locator: source.path }),
            tier: "active-branch" as const,
            relation: "active-branch" as const,
            role: source.role,
            temporal: source.temporal,
            characters: source.characters,
            completeness: source.completeness ?? "unknown",
            confidence: candidate?.confidence === "high" ? ("high" as const) : ("medium" as const),
            matchClass: "strong-structured" as const,
  ...
  private proposalMessage(proposal: ContextControlProposal, analysis: LifecycleAnalysis): any {
    const lines =
      proposal.kind === "recovery"
        ? [
  ...
  private async updatePins(refs: unknown, pin: boolean): Promise<any> {
    if (this.mode === "shadow") {
  ...
  private async record(event: Omit<ContextControlAuditEvent, "version">): Promise<void> {
    const value: ContextControlAuditEvent = { version: "0.1", ...event };
...
export function createDefaultContextControlJournal(path?: string): ContextControlJournal {
