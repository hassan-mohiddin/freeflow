...
export async function buildFreeflowStatusReport(params = {}, ctx) {
	...
	return {
		...
		vaultIndex,
		observedRouting: observedRoutingStatus(
			effectiveFreeflowConfig.observedRouting,
		),
		scriptTransform: scriptTransformStatus(
			effectiveFreeflowConfig.scriptTransform,
			scriptSandbox,
		),
...
async function inspectMissingVaultRoot(root) {
	...
	if (!ancestor) {
		return {
			...
			detail:
				"Vault root does not exist and no existing writable ancestor could be found. No directory was created.",
		};
	}
...
function observedRoutingStatus(config) {
	const servers = Object.entries(
		(config.mcp?.servers ?? {}) as Record<string, any>,
	).map(([id, server]) => ({
		id,
		...
		if (!TOP_LEVEL_CONFIG_KEYS.has(key)) {
			recommendations.push(
				recommendation(
					key,
					"review",
					"Unrecognized top-level Freeflow config key; review before migrating.",
				),
			);
		}
	}
	...
	if (rawConfig.observedRouting !== undefined) {
		recommendations.push(
			recommendation(
				"observedRouting",
				"move",
				"Move top-level observedRouting under outputRouter.observedRouting.",
			),
		);
		collectObservedRoutingRecommendations(
			rawConfig.observedRouting,
			recommendations,
			"observedRouting",
		);
	}
	if (rawConfig.scriptTransform !== undefined) {
		recommendations.push(
			recommendation(
				"scriptTransform",
				"move",
				"Move top-level scriptTransform under outputRouter.scriptTransform.",
			),
		);
		collectScriptTransformRecommendations(
			rawConfig.scriptTransform,
			recommendations,
			"scriptTransform",
		);
	}
...
function collectOutputRouterRecommendations(value, recommendations) {
	...
	if (!isRecord(value)) {
		recommendations.push(
			recommendation(
				"outputRouter",
				"fix",
				"Expected object; remove or rewrite invalid outputRouter config.",
			),
		);
		return;
	}
		...
		if (!OUTPUT_ROUTER_CONFIG_KEYS.has(key)) {
			recommendations.push(
				recommendation(
					`outputRouter.${key}`,
					"review",
					"Unrecognized outputRouter key; review before migrating.",
				),
			);
		}
	}
	...
	addDefaultRecommendation(
		recommendations,
		"outputRouter.enabled",
		value.enabled,
		DEFAULT_OUTPUT_ROUTER_ENABLED,
	);
	addDefaultRecommendation(
		recommendations,
		"outputRouter.profile",
		value.profile,
		DEFAULT_OUTPUT_ROUTER_PROFILE,
	);
	addDefaultRecommendation(
	...
	collectObservedRoutingRecommendations(
		value.observedRouting,
		recommendations,
		"outputRouter.observedRouting",
	);
	collectScriptTransformRecommendations(
		value.scriptTransform,
		recommendations,
	...
	if (Object.keys(value).length === 0) {
		recommendations.push(
			recommendation(
				"outputRouter",
				"remove",
				"Empty outputRouter object can be removed; built-in defaults apply.",
			),
		);
	}
}
	...
	if (!isRecord(value)) {
		recommendations.push(
			recommendation(
				...
				"fix",
				"Expected object with largeOutputBytes and largeOutputLines.",
			),
		);
		return;
	}
	...
	if (!isRecord(value)) {
		recommendations.push(
			recommendation(
				"outputRouter.vault",
				"fix",
				"Expected object with root and retention.",
			),
		);
		return;
	}
...
function collectScriptTransformRecommendations(value, recommendations, path) {
	...
	if (!isRecord(value)) {
		recommendations.push(
			recommendation(
				path,
				"fix",
				`Expected object; remove or rewrite invalid ${path} config.`,
			),
		);
		return;
	}
		...
		if (!SCRIPT_TRANSFORM_CONFIG_KEYS.has(key)) {
			recommendations.push(
				recommendation(
					`${path}.${key}`,
					"review",
					"Unrecognized scriptTransform key; review before migrating.",
				),
			);
		}
	}
	...
	addDefaultRecommendation(
		...
		`${path}.enabled`,
		value.enabled,
		DEFAULT_SCRIPT_TRANSFORM_CONFIG.enabled,
	);
	addDefaultRecommendation(
		recommendations,
		`${path}.sandbox`,
...
function collectObservedRoutingRecommendations(value, recommendations, path) {
		...
		if (!OBSERVED_ROUTING_CONFIG_KEYS.has(key)) {
			recommendations.push(
				recommendation(
					`${path}.${key}`,
					"review",
					"Unrecognized observedRouting key; review before migrating.",
				),
			);
		}
	}
	...
	addDefaultRecommendation(
		...
		`${path}.enabled`,
		value.enabled,
		DEFAULT_OBSERVED_ROUTING_CONFIG.enabled,
	);
	addDefaultRecommendation(
		recommendations,
	...
	collectObservedProducerRecommendations(
		value.web,
		`${path}.web`,
		recommendations,
	);
...
function collectObservedMcpRecommendations(value, recommendations, path) {
	...
	if (!isRecord(value)) {
		recommendations.push(
			recommendation(path, "fix", "Expected object with explicit servers."),
		);
		return;
	}
		...
		if (!OBSERVED_ROUTING_MCP_KEYS.has(key)) {
			recommendations.push(
				recommendation(
					`${path}.${key}`,
					"review",
					"Unrecognized observedRouting.mcp key; review before migrating.",
				),
			);
		}
	}
	...
	if (value.servers !== undefined && !isRecord(value.servers)) {
		recommendations.push(
			recommendation(
				`${path}.servers`,
				"fix",
				"Expected object keyed by MCP server id.",
			),
		);
	}
}
	...
	if (!isRecord(value)) {
		recommendations.push(
			recommendation(
				path,
				"fix",
				"Expected object with enabled and persistence.",
			),
		);
		return;
	}
...
function addLegacyMoveOrDefaultRecommendation(
	recommendations,
	path,
	value,
	defaultValue,
	...
	recommendations.push(
		recommendation(path, "move", `Move legacy ${path} to ${targetPath}.`),
	);
}
