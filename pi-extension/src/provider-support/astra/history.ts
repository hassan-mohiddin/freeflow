...
export interface Attempt {
  ...
  length: number;
  inputHash: string;
  update?: { at: number; prefixHash: string; effort: Effort };
}
...
const effort = (x: unknown): x is Effort => EFFORTS.includes(x as Effort);
const identity = (x: unknown): x is string => typeof x === "string" && x.length > 0 && x.length <= 1024;
export function parseAttempt(value: any): Attempt {
  if (
    !value ||
    value.version !== 1 ||
    !identity(value.id) ||
...
export function prefixHashes(input: readonly unknown[]): string[] {
  const result = [hash([])];
  for (const item of input) result.push(hash([result.at(-1), item]));
  return result;
}
export function envelopeHash(payload: any): string {
...
export function assemble(payload: any, key: string, generation: string, basis: string | null, records: Attempt[]) {
  ...
  const prior = compatible.reduce<Attempt | undefined>(
    (best, r) => (!best || r.length >= best.length ? r : best),
    undefined,
  );
  const baseline = prior?.baseline ?? applicable[0]?.baseline ?? (payload.reasoning.effort as Effort);
  const desired = payload.reasoning.effort as Effort;
  const byId = new Map(records.map((r) => [r.id, r]));
  const updates: NonNullable<Attempt["update"]>[] = [];
