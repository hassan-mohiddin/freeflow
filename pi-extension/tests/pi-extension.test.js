...
test("Pi layers local core overrides over repository defaults with source evidence", async () => {
	...
	try {
		...
		assert.equal(
			layers.local.parsed.processing.unsafeUnsandboxed.enabled,
			true,
		);
		const routerConfig = await readOutputRouterConfig(cwd);
		assert.equal(
			routerConfig.localConfig.processing.unsafeUnsandboxed.enabled,
			true,
		);
...
test("Pi restores layered session mode on resume and preserves it through compaction", async () => {
	...
	try {
		...
		assert.match(
			result.systemPrompt,
			/Resolved mode: `conversation` \(inactive because Skills are disabled\)/,
		);
...
test("Pi ordinary prompts and direct skill calls do not change mode state", async () => {
	...
	try {
		...
		const commitWorkCommand = commands.find(
			(command) => command.name === "commit-work",
		);
		assert.ok(sessionStart);
		assert.ok(beforeAgentStart);
		assert.ok(freeflowCommand);
		...
		assert.match(
			result.systemPrompt,
			/Task type, risk classification, and direct skill calls do not change mode/,
		);
		assert.deepEqual(entries, []);

		await freeflowCommand.definition.handler(
			"mode conversation",
			context(cwd),
		);
		assert.equal(entries.length, 1);
		await commitWorkCommand.definition.handler(
			"create a checkpoint",
			context(cwd),
		);
		...
		assert.equal(modeState.effectiveMode, "conversation");
		result = await beforeAgentStart(
			{ prompt: "/commit-work", systemPrompt: "base prompt" },
			context(cwd),
		);
		assert.match(result.systemPrompt, /Session mode override: `conversation`/);
		assert.match(result.systemPrompt, /Effective Freeflow mode: `conversation`/);
...
test("Pi layered core config inherits omitted values and built-in defaults", async () => {
	...
	try {
		...
		await writeFile(
			join(cwd, ".freeflow/local.json"),
			JSON.stringify({ interactionContract: false }, null, 2),
			"utf8",
		);
...
test("Pi requires repository activation even when local overrides exist", async () => {
	...
	try {
		...
		await writeFile(
			join(cwd, ".freeflow/local.json"),
			JSON.stringify(
				{ enabled: true, defaultMode: "strict-workflow" },
				null,
				2,
			),
			"utf8",
...
test("Pi fails closed when a local core override has an invalid type", async () => {
	...
	try {
		...
		await writeFile(
			join(cwd, ".freeflow/local.json"),
			JSON.stringify({ skills: { enabled: "no" } }, null, 2),
			"utf8",
		);
...
test("Pi skills toggle suppresses workflow skills while allowing enabled router tools", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-skills-disabled-router-on-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
...
test("Pi interaction contract can be disabled independently while Workflow stays enabled", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-interaction-disabled-skills-on-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
			join(cwd, ".freeflow/config.json"),
			JSON.stringify(
				{ defaultMode: "workflow", interactionContract: false },
				null,
				2,
			),
			"utf8",
...
test("Pi all-disabled capability state injects only Freeflow control-plane status", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-all-capabilities-disabled-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		assert.match(
			...
			/Freeflow modes are inactive because Skills are disabled/,
		);
		assert.match(
			modeCtx.notifications.at(-1).message,
			/Configured default mode: workflow \(repository default\)/,
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
	}
...
test("Pi /freeflow mode is the only mode command and opens a dedicated selector", async () => {
	...
	try {
		...
		modeCtx.ui.custom = async (factory) => {
			...
			assert.match(
				text,
				/strict-workflow\s+Stronger decision and evidence pressure for high-risk work/,
			);
		...
		assert.equal(
			...
			"freeflow: interaction · strict-workflow (session)",
		);
		...
		assert.equal(
			resetCtx.statuses.at(-1).value,
			"freeflow: interaction · workflow",
		);
		assert.match(
			resetCtx.notifications.at(-1).message,
			/reset to configured default: workflow \(repository default\)/,
		);
...
test("Pi statusline reports only effective Freeflow runtime state", async () => {
	...
	try {
		...
		assert.equal(
			await statusFor({ enabled: false, defaultMode: "workflow" }),
			"freeflow: off",
		);
		assert.equal(
			await statusFor({ defaultMode: "workflow", skills: { enabled: false } }),
...
test("Pi /freeflow command toggles master switch and blocks inactive settings rows", async () => {
	...
	try {
		...
		settingsCtx.ui.custom = async (factory, options) => {
			...
			assert.match(
				rootText,
				/Output Router\s+enabled \(22\) \(repository\) › inactive/,
			);
		...
		const afterInactiveEdit = JSON.parse(
...
test("Pi /freeflow settings marks default mode dormant when Skills are disabled", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-settings-skills-off-mode-dormant-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
			join(cwd, ".freeflow/config.json"),
			JSON.stringify(
				{ defaultMode: "workflow", skills: { enabled: false } },
				null,
				2,
			),
			"utf8",
		...
		settingsCtx.ui.custom = async (factory) => {
			...
			assert.match(
				rootText,
				/Session mode\s+default \(workflow · repository · inactive\).*inactive/,
			);
			assert.match(
				rootText,
				/Default mode\s+workflow \(repository · inactive\)/,
			);
			...
			assert.match(
				choiceText,
				/Freeflow Settings · Personal overrides › Default mode/,
			);
			assert.match(
				choiceText,
				/conversation\s+Discussion and read-only inspection/,
			);
			assert.match(
				choiceText,
				/workflow\s+Adaptive workflow for consequential work/,
			);
			assert.match(
				choiceText,
				/strict-workflow\s+Stronger decision and evidence pressure for high-risk work/,
		...
		await freeflowCommand.definition.handler("settings", settingsCtx);
		const repositoryAfter = JSON.parse(
			await readFile(join(cwd, ".freeflow/config.json"), "utf8"),
		);
		const localAfter = JSON.parse(
			await readFile(join(cwd, ".freeflow/local.json"), "utf8"),
		);
...
test("Pi /freeflow settings toggles the interaction contract independently", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-settings-interaction-contract-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		const configPath = join(cwd, ".freeflow/config.json");
		...
		settingsCtx.ui.custom = async (factory) => {
			...
			assert.match(
				...
				/Interaction Contract\s+enabled \(builtin\)/,
			);
			component.handleInput("\u001b[B");
			component.handleInput("\r");
			component.handleInput("\u001b[B");
		...
		enableCtx.ui.custom = async (factory) => {
			...
			assert.match(
				renderText(component),
				/Interaction Contract\s+disabled \(local\)/,
			);
			component.handleInput("\u001b[B");
			component.handleInput("\r");
			component.handleInput("\u001b[B");
		...
		await assert.rejects(
			readFile(join(cwd, ".freeflow/local.json"), "utf8"),
...
test("Pi settings refuse to overwrite an invalid local override", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-invalid-local-settings-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		assert.match(
			...
			/local\.json is invalid; repair or remove it/,
		);
...
test("Pi personal settings refuse to write a tracked local override", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-tracked-local-settings-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		await freeflowCommand.definition.handler("settings", ctx);
		assert.deepEqual(
			JSON.parse(await readFile(join(cwd, ".freeflow/local.json"), "utf8")),
			{},
		);
		assert.match(
			ctx.notifications.at(-1).message,
...
test("Pi /freeflow settings repo edits shared defaults and shows local effective sources", async () => {
	...
	try {
		...
		ctx.ui.custom = async (factory) => {
			...
			assert.match(
				rootText,
				/Default mode\s+workflow \(effective conversation · local\)/,
			);
...
test("Pi /freeflow settings groups capability settings", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-freeflow-grouped-settings-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
			join(cwd, ".freeflow/config.json"),
			JSON.stringify(
				{ defaultMode: "workflow", outputRouter: { enabled: true } },
				null,
				2,
			),
			"utf8",
		...
		settingsCtx.ui.custom = async (factory) => {
			...
			assert.match(
				routerText,
				/Freeflow Settings · Personal overrides › Output Router/,
			);
...
test("Pi before_agent_start keeps output-router disabled by default", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-router-default-disabled-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		assert.doesNotMatch(
			result.systemPrompt,
			/large native read\/bash outputs may be vaulted/,
		);
...
test("Pi before_agent_start keeps the per-turn system context to the compact interaction contract", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-interaction-contract-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		assert.match(
			result.systemPrompt,
			/enough shared\s+understanding for\s+the next sound action/,
		);
		assert.doesNotMatch(
			result.systemPrompt,
			/self-review|final assurance|standing authorization/i,
		);
		assert.doesNotMatch(result.systemPrompt, /## Loaded Mode Contract Skill/);
		assert.doesNotMatch(result.systemPrompt, /## Loaded Workflow Skill/);
		assert.doesNotMatch(result.systemPrompt, /## Loaded Decision Gate Skill/);
...
test("Pi loads the full Workflow skill as one persistent first-turn message", async () => {
	...
	try {
		...
		const workflowOwnerLine = workflowLines.find((line) =>
			line.startsWith("The active agent owns "),
		);
		const workflowSelfReviewLine = workflowLines.find((line) =>
			line.startsWith("Self-review is silent "),
		);
		assert.ok(workflowOwnerLine, "canonical Workflow owner sentinel missing");
		assert.ok(
			workflowSelfReviewLine,
			"canonical Workflow self-review sentinel missing",
		);
		...
		const later = await beforeAgentStart(
			{ prompt: "implement the feature", systemPrompt: "base prompt" },
			ctx,
		);
		...
		assert.equal(
			afterCompaction.message.customType,
...
test("Pi suppresses persisted Workflow context while Skills are disabled", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-workflow-bootstrap-disabled-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		const configPath = join(cwd, ".freeflow/config.json");
		...
		const disabled = await contextHandler(
			{ messages: [workflowMessage, userMessage] },
			context(cwd),
		);
		...
		const enabled = await contextHandler(
			{ messages: [workflowMessage, userMessage] },
			context(cwd),
		);
...
test("Pi session_start and session_compact keep the interaction contract on later turns", async () => {
	...
	try {
		...
		const afterFirst = await beforeAgentStart(
			{ systemPrompt: "base prompt" },
			context(cwd),
		);
		...
		const afterCompact = await beforeAgentStart(
			{ systemPrompt: "base prompt" },
			context(cwd),
		);
...
test("Pi capability config disables output-router context, active tools, and execution", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-output-router-disabled-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
			join(cwd, ".freeflow/config.json"),
			JSON.stringify(
				{ defaultMode: "workflow", outputRouter: { enabled: false } },
				null,
				2,
			),
			"utf8",
		...
		assert.match(
			...
			/freeflow_search\|disabled_by_config/,
		);
...
test("Pi output-router settings UI toggles multiple config values and reloads once", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-output-router-settings-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		assert.deepEqual(after.outputRouter.hints.generatedPathGlobs, [
			"graphify-out/**",
		]);
...
test("Pi output-router status summarizes master and subfeature state", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-output-router-status-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
...
test("Pi freeflow_status reports effective defaults without writing config", async () => {
	...
	try {
		...
		assert.equal(
			report.effectiveConfig.outputRouter.observedRouting.enabled,
			false,
		);
		assert.deepEqual(
			report.effectiveConfig.outputRouter.observedRouting.mcp.servers,
			{},
		);
		assert.equal(
			report.effectiveConfig.outputRouter.scriptTransform.enabled,
			false,
		);
		assert.equal(
			report.effectiveLocalConfig.processing.unsafeUnsandboxed.enabled,
			false,
		);
		...
		assert.ok(
			["available", "unavailable"].includes(
				report.scriptTransform.adapterStatus,
			),
		);
		assert.equal(report.scriptTransform.adapterContractVersion, 1);
		assert.deepEqual(report.scriptTransform.configuredLanguages, [
			"javascript",
			"python",
			"jq",
		]);
		const reportedLanguages = [
			...
			...report.scriptTransform.unavailableLanguages.map(
				(entry) => entry.language,
			),
		].sort();
		assert.deepEqual(reportedLanguages, ["javascript", "jq", "python"]);
		assert.ok(
			report.scriptTransform.requiredProofs.includes("network_access_denied"),
		);
		assert.ok(
			report.scriptTransform.candidateMechanisms.some(
				(candidate) =>
					candidate.id === "node-vm" && candidate.status === "rejected",
			),
		);
		assert.ok(
			report.scriptTransform.candidateMechanisms.some(
				(candidate) =>
					candidate.id === "os-sandbox-adapter" &&
					candidate.status === "candidate_unproven",
			),
		);
		...
		assert.equal(
			report.observedRouting.unsupportedPersistenceModes.includes("redacted"),
			true,
		);
...
test("Pi freeflow_status reports layered core sources and dormant mode", async () => {
	...
	try {
		...
		const expanded = renderText(
			statusTool.renderResult(result, { expanded: true }, testTheme),
		);
...
test("Pi freeflow_status fails closed and explains invalid local core config", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-status-invalid-local-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
			join(cwd, ".freeflow/config.json"),
			JSON.stringify({ defaultMode: "workflow" }),
			"utf8",
		);
		await writeFile(
			join(cwd, ".freeflow/local.json"),
			JSON.stringify({ skills: { enabled: "no" } }),
			"utf8",
		);
		...
		assert.ok(
			report.localConfigWarnings.some(
				(warning) =>
					warning.includes(".freeflow/local.json is invalid") &&
					warning.includes("runtime is inactive"),
			),
		);

		const collapsed = renderText(
			statusTool.renderResult(result, { expanded: false }, testTheme),
		);
...
test("Pi freeflow_status reports vault writability without creating directories", async () => {
	...
	try {
		...
		assert.equal(
			missingReport.vault.writability.status,
			"missing_ancestor_writable",
		);
...
test("Pi freeflow_status reports local unsafe processing opt-in without shared config", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-status-local-processing-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
...
test("Pi freeflow_status warns that shared config cannot enable unsafe processing", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-status-shared-processing-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		assert.equal(report.processing.unsafeUnsandboxed.enabled, false);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("processing config is ignored"),
			),
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
...
test("Pi freeflow_status reports configured observed routing", async () => {
	...
	try {
		...
		assert.equal(
			report.effectiveConfig.outputRouter.observedRouting.onRoutingFailure,
			"fail-open",
		);
		assert.deepEqual(
			report.effectiveConfig.outputRouter.observedRouting.mcp.servers.github,
			{ enabled: true, persistence: "exact" },
		);
		assert.deepEqual(
			report.effectiveConfig.outputRouter.observedRouting.mcp.servers.gmail,
			{ enabled: true, persistence: "metadata-only" },
		);
		...
		assert.deepEqual(
			report.effectiveConfig.outputRouter.observedRouting.fetch,
			{ enabled: false, persistence: "none" },
		);
		assert.deepEqual(
			report.effectiveConfig.outputRouter.observedRouting.codeSearch,
			{ enabled: true, persistence: "none" },
		);
		...
		assert.deepEqual(
			report.effectiveConfig.outputRouter.scriptTransform.languages,
			["python"],
		);
		assert.equal(report.scriptTransform.enabled, true);
		assert.ok(
			["available", "adapter_unavailable"].includes(
				report.scriptTransform.executionStatus,
			),
		);
		...
		assert.deepEqual(report.observedRouting.persistenceModes, [
			"exact",
			"metadata-only",
			"none",
		]);
...
test("Pi freeflow_status reports invalid config warnings and safe fallbacks", async () => {
	...
	try {
		...
		assert.equal(
			report.effectiveConfig.outputRouter.observedRouting.web.persistence,
			"metadata-only",
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.enabled"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.profile"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.scriptTransform.enabled"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.scriptTransform.sandbox"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.scriptTransform.languages"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.scriptTransform.network"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.observedRouting.enabled"),
			),
		);
		assert.ok(
			report.configWarnings.some(
				(warning) =>
					warning.includes(
						"outputRouter.observedRouting.mcp.servers.github.persistence",
					) && warning.includes("redacted"),
			),
		);
		assert.ok(
			report.configWarnings.some((warning) =>
				warning.includes("outputRouter.observedRouting.web.persistence"),
			),
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
...
test("Pi freeflow_status migration recommendations are non-destructive", async () => {
	...
	try {
		...
		assert.ok(
			report.migration.recommendations.some(
				(recommendation) =>
					recommendation.path === "outputRouter.postToolRouting",
			),
		);
		assert.ok(
			report.migration.recommendations.some(
				(recommendation) => recommendation.path === "capture",
			),
		);
		assert.ok(
			report.migration.recommendations.some(
				(recommendation) => recommendation.path === "providers",
			),
		);
		assert.equal(await readFile(configPath, "utf8"), configText);
	} finally {
...
test("Pi freeflow_search renders compact and expanded routed evidence UI", () => {
	...
	const collapsed = renderText(
		searchTool.renderResult(toolResult, { expanded: false }, testTheme),
	);
	...
	const expanded = renderText(
		searchTool.renderResult(toolResult, { expanded: true }, testTheme),
	);
	...
	assert.match(
		expanded,
		/exact search: action=retrieve source.kind=repo lineRange=523-527 path=docs\/example\.md/,
	);
	assert.match(expanded, /### Sandbox Permissions/);
	assert.match(expanded, /Recovery/);
	assert.match(
		expanded,
		/expand hint: freeflow_search action=expand evidenceId=ev_test/,
	);
});
...
test("Pi freeflow_run exposes declarative filter schema", () => {
	...
	assert.deepEqual(runTool.parameters.oneOf, [
		{ required: ["command"] },
		{ required: ["script"] },
	]);
	...
	assert.deepEqual(script.properties.language.enum, [
		"javascript",
		"python",
		"jq",
	]);
	...
	assert.deepEqual(filters.properties.stream.enum, [
		"stdout",
		"stderr",
		"combined",
	]);
	...
	assert.deepEqual(scriptFilter.properties.language.enum, [
		"javascript",
		"python",
		"jq",
	]);
	...
	assert.equal(
		scriptFilter.properties.limits.properties.timeoutMs.maximum,
		30000,
	);
...
test("Pi freeflow_run renders compact and expanded status, evidence, and vault UI", () => {
	...
	const call = renderText(
		runTool.renderCall(
			{ command: "npm test -- --runInBand", preserve: "important" },
			testTheme,
		),
	);
		...
		details: {
			result: {
				...
				routing: {
					...
					route: "run",
					reason:
						"Command failed; exact failure evidence was returned and raw output was vaulted before routing.",
				},
	...
	const collapsed = renderText(
		runTool.renderResult(toolResult, { expanded: false }, testTheme),
	);
	...
	assert.match(
		expanded,
		/raw command starting point: freeflow_search source.kind=vault outputId=ffout_test123/,
	);
...
test("Pi freeflow_run returns compact model-visible text with full structured details", async () => {
	...
	try {
		...
		assert.ok(
			Buffer.byteLength(visibleText, "utf8") <
				Buffer.byteLength(detailsText, "utf8"),
		);
	...
	try {
		...
		const pi = {
			...
			async exec() {
				execCalls += 1;
				throw new Error(
					"host exec should not be called for sandboxed script producers",
				);
			},
		};
		freeflowExtension(pi);
		...
		assert.equal(
			result.details.result.failure.kind,
...
test("Pi freeflow_batch returns compact summary and preserves child details", async () => {
	...
	try {
			...
			async exec(_bin, args) {
				...
				return {
					stdout:
						Array.from(
							{ length: 20 },
							(_, index) => `${prefix}_VISIBLE_BATCH_SENTINEL_${index + 1}`,
						).join("\n") + "\n",
					stderr: "",
					code: 0,
					killed: false,
		...
		const separateOne = await runTool.execute(
			"run-one",
			{ command: "fixture one" },
			undefined,
			undefined,
		...
		const separateTwo = await runTool.execute(
			"run-two",
			{ command: "fixture two" },
			undefined,
			undefined,
		...
		assert.ok(
			Buffer.byteLength(visibleText, "utf8") <
				Buffer.byteLength(separateVisible, "utf8"),
		);
		...
		assert.equal(
			payload.steps[0].result.importantLines[0].excerpt.includes(
				"ONE_VISIBLE_BATCH_SENTINEL_1",
			),
			true,
		);
		assert.equal(
			payload.steps[1].result.importantLines[0].excerpt.includes(
				"TWO_VISIBLE_BATCH_SENTINEL_1",
			),
			true,
		);

		const expanded = renderText(
			batchTool.renderResult(batch, { expanded: true }, testTheme),
		);
...
test("Pi freeflow_search applies configured generated path hints", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-generated-path-hints-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		...
		await writeFile(
			join(cwd, "target.md"),
			"PI_GENERATED_HINT_MARKER source truth",
			"utf8",
		);
		await writeFile(
		...
		assert.ok(
			Buffer.byteLength(broadVisible, "utf8") <
				Buffer.byteLength(JSON.stringify(broadPayload, null, 2), "utf8"),
		);
...
test("Pi observed routing vaults and labels configured MCP output before native safety net", async () => {
	...
	try {
		...
		assert.match(
			...
			/Freeflow routed this observed mcp result/,
		);
		...
		assert.equal(
			result.details.freeflowObservedRouting.producer.server,
			"github",
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
...
test("Pi observed routing fails open without losing MCP output", async () => {
	...
	try {
		...
		assert.equal(
			result.details.freeflowObservedRouting.routingStatus,
			"failed",
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
...
test("Pi post-tool safety net notes exact duplicate native output", async () => {
	const cwd = await mkdtemp(
		join(tmpdir(), "freeflow-pi-safety-net-duplicate-"),
	);
	try {
		await mkdir(join(cwd, ".freeflow"));
		await writeFile(
		...
		const firstOutputId = first.content[0].text.match(
			/outputId=(ffout_[a-f0-9]+)/,
		)?.[1];
		...
		assert.match(
			...
			new RegExp(
				`Duplicate: exact native output matches previous outputId=${firstOutputId}`,
			),
		);
		assert.equal(
			second.details.freeflowOutputRouter.duplicateOfOutputId,
			firstOutputId,
		);
		assert.match(
			second.content[0].text,
			/current raw output was vaulted as outputId=ffout_/,
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
	}
...
test("Pi keeps enabled capability context alongside the interaction contract", async () => {
	...
	try {
		...
		assert.doesNotMatch(
			result.systemPrompt,
			/Capture raw evidence before transformation/,
		);
	} finally {
		await rm(cwd, { recursive: true, force: true });
	}
