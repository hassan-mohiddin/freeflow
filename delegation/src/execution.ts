...
import type {
  ...
  CommitCheckpointValidationInput,
  ExecutionDecision,
  ExecutionMapMetadata,
  ExecutionPolicyReroute,
  PackageCheckpointState,
  WorkPackageMetadata,
  WorktreeMetadata,
} from "./types.js";
...
export function emptyExecutionMap(taskId: string, updatedAt: string): ExecutionMapMetadata {
  return {
    version: 1,
...
export function normalizeWorkPackageMetadata(input: WorkPackageMetadata): WorkPackageMetadata {
  ...
  const normalized: WorkPackageMetadata = {
    packageId,
    role: input.role,
    dependencies,
    expectedWriteScopes,
...
export function normalizeExecutionMap(input: ExecutionMapMetadata, updatedAt = input.updatedAt): ExecutionMapMetadata {
...
export function validateExecutionMap(input: ExecutionMapMetadata): ExecutionDecision {
  let executionMap: ExecutionMapMetadata;
  try {
...
export function validateOneWriterPerCheckout(packages: readonly WorkPackageMetadata[]): ExecutionDecision {
  const writers = new Map<string, WorkPackageMetadata>();
  for (const pkg of packages) {
    ...
    if (existing !== undefined && existing.packageId !== pkg.packageId) {
      return block(
        ...
        `checkout ${pkg.checkoutPath} already has writer package ${existing.packageId}; ${pkg.packageId} would violate one-writer-per-checkout`,
        "execution-parent",
        pkg.packageId,
      );
    }
  ...
  for (const dependencyId of pkg.dependencies) {
    ...
    if (!isPackageComplete(dependency)) {
      return block(
        ...
        `dependency ${dependencyId} is not complete for work package ${packageId}`,
        "execution-parent",
        packageId,
        dependencyId,
      );
  ...
  for (const earlierPackageId of order.slice(0, index)) {
    ...
    if (earlier.state !== "integrated") {
      return block(
        ...
        `work package ${packageId} cannot integrate before ${earlierPackageId} is integrated`,
        "execution-parent",
        packageId,
        earlierPackageId,
      );
...
export function buildWorktreeBranchName(input: { taskId: string; packageId: string; prefix?: string }): string {
...
export function validateCommitCheckpoint(input: CommitCheckpointValidationInput): ExecutionDecision {
  const packageId = validateSafeId(input.packageId, "package id");
...
export function isPackageComplete(pkg: WorkPackageMetadata): boolean {
...
function normalizeWorktreeMetadata(input: WorktreeMetadata, packageId: string): WorktreeMetadata {
  const metadata: WorktreeMetadata = {
    path: validateCheckoutPath(input.path, "worktree path"),
...
function normalizeCheckpointState(input: PackageCheckpointState, label: string): PackageCheckpointState {
  const status = input.status;
  if (!["not_required", "pending", "passed", "failed", "skipped"].includes(status)) {
    throw new Error(`${label} has unsupported status: ${status}`);
  }
  const normalized: PackageCheckpointState = { required: Boolean(input.required), status };
...
function normalizeCommitCheckpoints(checkpoints: readonly CommitCheckpointMetadata[], packageId: string): CommitCheckpointMetadata[] {
  ...
  return checkpoints.map((checkpoint) => {
    ...
    const normalized: CommitCheckpointMetadata = {
      checkpointId,
      packageId,
      planned: true,
...
function requiredCheckpointSatisfied(
  kind: "review" | "verification",
  checkpoint: PackageCheckpointState,
...
function allow(reason: string, packageId?: string, checkpointId?: string): ExecutionDecision {
  const decision: ExecutionDecision = { allowed: true, status: "allowed", code: "allowed", reason };
  if (packageId !== undefined) decision.packageId = packageId;
  if (checkpointId !== undefined) decision.checkpointId = checkpointId;
  return decision;
}

function block(
  code: ExecutionDecision["code"],
  reason: string,
  ...
  if (packageId !== undefined) decision.packageId = packageId;
  if (checkpointId !== undefined) decision.checkpointId = checkpointId;
  if (filePath !== undefined) decision.filePath = filePath;
  return decision;
}
...
function validateCheckoutPath(value: string, label: string): string {
...
function normalizePathList(values: readonly string[], label: string): string[] {
  return [...new Set((values ?? []).map((value, index) => validatePathLike(value, `${label} ${index + 1}`)))];
}

function normalizeFileList(values: readonly string[], label: string): string[] {
