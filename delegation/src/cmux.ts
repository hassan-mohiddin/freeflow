...
const REQUIRED_CMUX_COMMANDS = ["new-pane", "send", "send-key", "read-screen", "close-surface"] as const;
...
export class CmuxAdapter {
  ...
  constructor(runner: CmuxCommandRunner, options: { cwd?: string; timeoutMs?: number } = {}) {
    this.runner = runner;
  ...
  ensureReady(input: Omit<EnsureDelegationReadyInput, "runner">): Promise<DelegationPreflightResult> {
    const readyInput: EnsureDelegationReadyInput = { runner: this.runner, ...input };
  ...
  async newPane(input: NewPaneInput = {}): Promise<CmuxNewPaneOutcome> {
  ...
  async send(input: CmuxSendInput): Promise<CmuxCommandOutcome> {
  ...
  async sendKey(input: CmuxSendInput & { key: string }): Promise<CmuxCommandOutcome> {
  ...
  async readScreen(input: CmuxReadScreenInput): Promise<CmuxCommandOutcome> {
  ...
  async closeSurface(input: CmuxCloseSurfaceInput): Promise<CmuxCommandOutcome> {
  ...
  private runCmux(command: string[]): Promise<CmuxRunResult> {
    return this.runner.run(command, cmuxRunOptions({ cwd: this.defaultCwd, timeoutMs: this.defaultTimeoutMs }));
  }
}

export async function ensureDelegationReady(input: EnsureDelegationReadyInput): Promise<DelegationPreflightResult> {
...
export function buildCmuxNewPaneCommand(input: NewPaneInput = {}): string[] {
...
export function buildCmuxSendCommand(input: CmuxSendInput): string[] {
...
export function buildCmuxSendKeyCommand(input: CmuxSendInput & { key: string }): string[] {
...
export function buildCmuxReadScreenCommand(input: CmuxReadScreenInput): string[] {
...
export function buildCmuxCloseSurfaceCommand(input: CmuxCloseSurfaceInput): string[] {
...
export function parseCmuxRefs(output: string): CmuxRefs {
  const raw = output.trim();
  const refs: CmuxRefs = { raw };
  collectRefsFromJson(raw, refs);
  const refRe = /\b(window|workspace|pane|surface):([A-Za-z0-9._-]+)/g;
  let match = refRe.exec(raw);
  while (match !== null) {
    setRef(refs, match[1] as CmuxRefKind, `${match[1]}:${match[2]}`);
  ...
  while (match !== null) {
    setRef(refs, match[1] as CmuxRefKind, normalizeRef(match[1] as CmuxRefKind, match[2] ?? ""));
    match = looseRe.exec(raw);
  }
  return refs;
}
...
): Promise<{ ok: boolean; check: DelegationPreflightCheck }> {
...
function isRunOk(result: CmuxRunResult): boolean {
...
function assertRunOk(label: string, result: CmuxRunResult, command: string[]): void {
  if (isRunOk(result)) {
    return;
  }
...
function summarizeRunFailure(result: CmuxRunResult): string {
...
async function checkStoreWritability(storeRoot: string): Promise<DelegationPreflightCheck> {
...
function collectRefsFromJson(raw: string, refs: CmuxRefs): void {
...
function visitJsonForRefs(value: unknown, refs: CmuxRefs): void {
...
function refKindFromKey(key: string): CmuxRefKind | undefined {
...
function setRef(refs: CmuxRefs, kind: CmuxRefKind, ref: string): void {
  if (ref.length === 0) {
    return;
  }
...
function normalizeRef(kind: CmuxRefKind, value: string): string {
...
function appendOptionalRef(command: string[], flag: string, value: string | undefined): void {
  if (value !== undefined) {
...
function validateExecutableName(value: string, label: string): string {
...
function validateSimpleToken(value: string, label: string): string {
  const trimmed = value.trim();
  if (trimmed.length === 0 || trimmed.includes("\0") || /\s/.test(trimmed)) {
