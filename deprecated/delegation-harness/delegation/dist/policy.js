...
export function evaluatePolicy(input) {
    ...
    const profile = input.profile ?? role;
    let definition;
    try {
    ...
    if (toolName === undefined || !definition.activeTools.includes(toolName)) {
        ...
        return block("capability_gap", `tool ${requested} is not active for profile ${profile}`, role, profile, definition.defaultPolicy.suggestedReroute, { kind: "capability_gap", detail: `reroute to a profile with ${requested}` });
    }
...
export function commandMatchesAllowedList(command, allowedCommands = []) {
    const normalized = normalizeCommand(command);
    return allowedCommands.some((allowed) => normalizeCommand(allowed) === normalized);
}
export function isCommandBlockedAsDestructive(command) {
...
export function isCredentialOrEnvDumpCommand(command) {
    return commandInspectionStrings(command).some((candidate) => [
        /^(?:env|printenv|set)(?:\s|$)/,
        /^export\s+-p(?:\s|$)/,
...
export function isPublishOrDeployCommand(command) {
    return commandInspectionStrings(command).some((candidate) => [
        /(?:^|[;&|]\s*)npm\s+publish\b/,
        /(?:^|[;&|]\s*)pnpm\s+publish\b/,
...
export function isBroadGitStageCommand(command) {
    return commandInspectionStrings(command).some((candidate) => /(?:^|[;&|]\s*)git\s+add\s+(?:\.|-A|--all)(?:\s|$)/.test(candidate));
}
export function isGitWorktreeMutationCommand(command) {
...
function evaluateWrite(path, taskPolicy, role, profile, definition) {
    if (role === "planning-parent" && isLikelyProductCodePath(path, taskPolicy?.cwd)) {
        return block("product_code_write_requires_scope", `planning-parent cannot edit product/runtime implementation: ${path}`, role, profile, "execution-parent", { kind: "policy_block", detail: "route implementation to execution-parent or a scoped worker" });
    }
...
function evaluateCommand(command, taskPolicy, role, profile, definition) {
    ...
    if (definition.defaultPolicy.commandPolicy === "allowed-list" && allowedCommands.length === 0) {
        return block("command_not_allowed", `profile ${profile} requires ALLOWED_COMMAND entries before running commands`, role, profile, definition.defaultPolicy.suggestedReroute, { kind: "capability_gap", detail: "request parent to reroute or add an allowed command in a new packet" });
    }
    return { allowed: true, status: "allowed", role, profile, reason: "command passed policy guards" };
}
function hasValidatedCommitCheckpointApproval(taskPolicy) {
...
function inferToolName(intent, definition) {
...
function explicitToolName(intent) {
    if (intent.kind === "tool") {
        return intent.toolName;
    }
    return intent.toolName;
}
function isValidIntentShape(intent) {
    if (intent.kind === "tool") {
...
function blockFromProfileError(error, role, profile) {
...
function block(code, reason, role, profile, suggestedReroute, request) {
    const decision = {
        allowed: false,
        status: "blocked",
        code,
...
export function isPathInsideAnyScope(path, scopes, cwd) {
    return scopes.some((scope) => isPathInsideScope(path, scope, cwd));
}
export function isPathInsideScope(path, scope, cwd) {
...
export function isLikelyProductCodePath(path, cwd) {
...
function relativePolicyPath(path, cwd) {
...
function hasUninspectableShellWrapper(command) {
    const normalized = normalizeCommand(command);
...
function commandInspectionStrings(command) {
    const seen = new Set();
...
function unescapeShellPayload(payload) {
