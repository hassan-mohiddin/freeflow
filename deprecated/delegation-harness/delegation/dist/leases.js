...
export function activeLeasesForAgent(view, agentId) {
    ...
    return leaseIds.map((leaseId) => normalized.leasesById[leaseId]).filter((lease) => lease !== undefined && lease.state === "active");
}
...
export function authorizeDelegationLease(input) {
    ...
    if (input.intent.kind === "write") {
        for (const lease of matchingIdentity) {
            if (!lease.actions.includes("edit"))
                continue;
            for (const scope of lease.writeScopes) {
                if (requiresNarrowParentScope(input.role) && isBroadWriteScope(scope, input.cwd))
                    continue;
        ...
        return leaseBlock("write_scope_violation", `no single active lease authorizes edit of ${input.intent.path} for ${taskId}/${agentId}/${input.role}`, input.role === "orchestrator" ? "orchestrator" : "parent");
    }
    ...
    for (const lease of matchingIdentity) {
        if (!lease.actions.includes("run_allowlisted"))
            continue;
    ...
    return leaseBlock("command_not_allowed", `no single active lease authorizes exact command for ${taskId}/${agentId}/${input.role}: ${input.intent.command}`, input.role === "orchestrator" ? "orchestrator" : "parent");
}
