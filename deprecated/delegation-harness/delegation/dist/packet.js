...
import { CURRENT_DELEGATION_IDENTITY_SCHEMA_VERSION, CURRENT_DELEGATION_PROFILE_SCHEMA_VERSION, CURRENT_DELEGATION_PROTOCOL_VERSION, } from "./identity.js";
...
export function validateTaskPacketIdentity(text, expected) {
    ...
    const role = roleProfileMatch[1];
    const profile = roleProfileMatch[2];
...
function normalizeTaskPacket(input) {
    ...
    const missingIdentity = ["assignmentId", "attemptId", "identitySchemaVersion", "profileSchemaVersion", "protocolVersion"]
...
function packetIdentityField(text, pattern, label) {
    return packetIdentityMatch(text, pattern, label)[1] ?? "";
}
function packetIdentityMatch(text, pattern, label) {
...
function currentVersion(value, expected, label) {
    const actual = value;
    if (actual !== expected) {
...
function validateRequiredSafeId(value, label) {
...
function normalizeTools(tools, activeTools, profile) {
...
function normalizeSourcePointers(pointers) {
...
function normalizePacketList(values, label) {
...
function validatePacketField(value, label) {
...
function defaultPolicySummary(profile, commandPolicy) {
    const summary = [
        `profile=${profile}`,
