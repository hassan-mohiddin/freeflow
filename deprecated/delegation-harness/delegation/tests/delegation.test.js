...
import {
  ...
  parseModelText,
  parseProtocolRow,
  planningReportPlanArtifactPath,
...
test("planning-parent static policy blocks implementation but preserves scoped planning artifacts", () => {
  assertBlocked(
    evaluatePolicy({
      role: "planning-parent",
      intent: { kind: "write", path: "/repo/src/runtime.ts" },
      taskPolicy: { cwd: "/repo", writeScopes: ["src/**"] },
    }),
  ...
  assertAllowed(
    evaluatePolicy({
      role: "planning-parent",
      intent: { kind: "write", path: "/repo/docs/plans/phase5.md" },
      taskPolicy: { cwd: "/repo", writeScopes: ["docs/plans/**"] },
    }),
  );
});
...
test("task packet compiler renders readable Markdown with scoped lifecycle return guidance", () => {
  const packet = compileTaskPacket({
    taskId: "TASK-9",
    agentId: "worker-9",
    assignmentId: "worker-9",
    attemptId: "attempt-worker-9",
...
test("task packet compiler falls back to legacy return instructions when lifecycle tools are unavailable", () => {
  const packet = compileTaskPacket({
    taskId: "TASK-10A",
    agentId: "worker-10A",
    assignmentId: "worker-10A",
    attemptId: "attempt-worker-10A",
    identitySchemaVersion: 1,
    profileSchemaVersion: 1,
    protocolVersion: 1,
    role: "worker",
...
test("task packet compiler rejects malformed required input without side effects", () => {
  const base = {
    taskId: "TASK-10",
    agentId: "worker-10",
    assignmentId: "worker-10",
    attemptId: "attempt-worker-10",
    identitySchemaVersion: 1,
    profileSchemaVersion: 1,
    protocolVersion: 1,
    role: "worker",
