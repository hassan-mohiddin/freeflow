...
import {
  type DelegationAlertPriority,
  type DelegationAlertState,
  type DelegationAssignmentState,
  type DelegationExecutionAuthorizationEvidence,
  type DelegationPaneState,
  type DelegationRouteAction,
  type DelegationRouteApplication,
  type DelegationRouteApplicationState,
  type DelegationRouteBreadth,
  type DelegationRouteDecision,
  type DelegationRouteDecisionKind,
  type DelegationRouteRequest,
  type DelegationRole,
  type DelegationRoleAssessmentStatus,
  type DelegationRouteActionKind,
  type DelegationRouteRiskFlag,
  type DelegationTaskWorkflowState,
} from "./types.js";
...
export const DELEGATION_TASK_WORKFLOW_STATES = [
  ...
  "failed",
  "cancelled",
] as const satisfies readonly DelegationTaskWorkflowState[];
...
export function normalizeDelegationRouteRequest(input: DelegationRouteRequest): DelegationRouteRequest {
  const normalized: DelegationRouteRequest = {
...
export function normalizeDelegationRouteDecision(input: DelegationRouteDecision): DelegationRouteDecision {
  const routeId = validateSafeId(input.routeId, "route id");
  const reasonCodes = uniqueNonEmptyStrings(input.reasonCodes, "reason code");

  switch (input.kind) {
    case "inline_allowed": {
      const normalized: DelegationRouteDecision = { kind: "inline_allowed", routeId, reasonCodes };
...
export function normalizeDelegationRouteApplication(input: DelegationRouteApplication): DelegationRouteApplication {
  const normalized: DelegationRouteApplication = {
...
export function normalizeExecutionAuthorizationEvidence(
  ...
  return {
    ...
    taskState: oneOf(input.taskState, ["ready_for_execution"] as const, "authorization task state"),
    ...
    approvedBy: oneOf(input.approvedBy, ["user", "orchestrator"] as const, "approver"),
  };
}

function routeImplementationIntent(request: DelegationRouteRequest, routeId: string): DelegationRouteDecision {
  ...
  if (request.role === "planning-parent") {
    ...
    return blockedDecision(
      routeId,
      "Planning-parent cannot perform implementation before stored execution authorization exists.",
      ["planning_parent_implementation_blocked", ...missingAuthorizationReasonCodes(request)],
      "orchestrator",
    );
  }
  ...
  if (roleIn(request.role, NON_IMPLEMENTING_LEAF_ROLES)) {
    return blockedDecision(
      routeId,
      "This leaf role cannot perform implementation edits; route the work through execution-parent.",
      ["leaf_implementation_blocked"],
      "execution-parent",
    );
  }
...
function routeSpawnIntent(request: DelegationRouteRequest, routeId: string): DelegationRouteDecision {
  if (roleIn(request.role, LEAF_DELEGATION_ROLES)) {
    return blockedDecision(
      routeId,
      "Leaf agents cannot spawn children or perform parent-control routing.",
      ["leaf_spawn_blocked"],
      "execution-parent",
    );
  }
...
export function deriveDelegationInlineLease(input: DelegationRouteRequest, routeId?: string) {
  const request = normalizeDelegationRouteRequest(input);
...
function routeIdFor(request: DelegationRouteRequest): string {
...
function canonicalStringArray(values: readonly string[] | undefined): string[] {
  return [...new Set(values ?? [])].sort();
}

function missingAuthorizationReasonCodes(request: DelegationRouteRequest): string[] {
...
function isImplementationAction(kind: DelegationRouteActionKind): boolean {
...
function hasAnyRiskFlag(request: DelegationRouteRequest, riskFlags: readonly DelegationRouteRiskFlag[]): boolean {
...
function normalizeRouteAction(input: DelegationRouteAction): DelegationRouteAction {
  const normalized: DelegationRouteAction = {
