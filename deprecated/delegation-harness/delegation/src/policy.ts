...
export function isPathInsideAnyScope(path: string, scopes: readonly string[], cwd: string | undefined): boolean {
  return scopes.some((scope) => isPathInsideScope(path, scope, cwd));
}

export function isPathInsideScope(path: string, scope: string, cwd: string | undefined): boolean {
