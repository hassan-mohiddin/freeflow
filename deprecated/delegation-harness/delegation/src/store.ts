...
import type {
  ...
  DelegationExecutionEnvelope,
  DelegationIndex,
  DelegationIndexTaskEntry,
  DelegationLayoutAllocation,
  DelegationLayoutPolicy,
  DelegationLayoutState,
  DelegationLease,
  DelegationLeaseEvent,
  DelegationLeaseState,
  DelegationProfile,
  DelegationRegistry,
  ...
  DelegationRouteRequest,
  DelegationState,
  DelegationTaskMetadata,
  DelegationWaitState,
  DelegationWakeAttempt,
  DelegationWakeAttemptOutcome,
  ExecutionDecision,
...
const CURRENT_EXECUTION_ENVELOPE_SCHEMA_VERSION = 1 as const;
...
interface StoredTerminalAcceptedOutcome {
  schemaVersion: 1;
...
interface StoredTerminalReconciliation {
  schemaVersion: 1;
...
interface StoredTerminalRejectedOutcome {
  schemaVersion: 1;
...
interface StoredPlanningReportPublication {
  schemaVersion: 1;
...
export class DelegationStore {
  ...
  async publishTerminalOutcome(
    ...
    const lockPath = terminalOutcomeAttemptPaths(
      taskPathSet,
      currentIdentity.assignmentId,
    ...
    try {
      ...
      if (
        ...
        rejectionReason = await planningParentTerminalEvidenceRejectionReason(
          taskPaths(this.root, safeTaskId),
          safeTaskId,
          submittedAgentId,
          identity,
          normalizedEvidenceResult.evidence,
        );
      }
      ...
      if (source === undefined) throw new Error("validated terminal source is unavailable");
      ...
      if (await fileExists(paths.acceptedJsonPath)) {
        ...
        const claim = await readJson<StoredTerminalClaim>(paths.claimPath);
      ...
      if (claimRequired) {
        const claim: StoredTerminalClaim = {
          schemaVersion: 1,
          recordType: "terminal.claim",
      ...
      const accepted: StoredTerminalAcceptedOutcome = {
        schemaVersion: 1,
        recordType: "terminal.accepted",
        disposition: "accepted",
        outcomeId,
  ...
  private async reconcileTerminalOutcome(
    ...
    const failures: Array<{ effect: (typeof effects)[number]; reason: string }> = [];
    let agentState: DelegationState | undefined;
    let endedLeaseIds: string[] = [];
    let eventId: string | undefined;
    let alertResult: QueueParentAlertResult | undefined;
    for (const effect of effects) {
      ...
      try {
        if (effect === "result_projection") {
          ...
          const ended = await this.endActiveAssignmentLeases(
            accepted.taskId,
            accepted.agentId,
            accepted.status === "cancelled" ? "revoked" : "exhausted",
            `terminal outcome ${accepted.outcomeId}`,
          );
          ...
          } satisfies StoredTerminalReconciliation);
        }
      } catch (error) {
  ...
  private async projectAcceptedTerminalResult(accepted: StoredTerminalAcceptedOutcome): Promise<void> {
    ...
    if (accepted.role === "execution-parent") {
      ...
      if (report === undefined) throw new Error("execution-parent accepted evidence has no normalized report");
  ...
  private async projectAcceptedTerminalStatus(accepted: StoredTerminalAcceptedOutcome): Promise<void> {
  ...
  async publishPlanningReport(
    ...
    return this.withExecutionAuthorizationLock(safeTaskId, async () => {
      ...
      let stored: StoredPlanningReportPublication;
      if (await fileExists(evidencePaths.jsonPath)) {
        stored = await readJson<StoredPlanningReportPublication>(evidencePaths.jsonPath);
      ...
      if (stored.reportStatus !== undefined) eventData.reportStatus = stored.reportStatus;
      if (stored.planArtifactPath !== undefined) eventData.planArtifactPath = stored.planArtifactPath;
      if (stored.errors !== undefined) eventData.errors = stored.errors as unknown as JsonValue;
  ...
  async readAgentResult(taskId: string, agentId: string): Promise<AgentResultRecord> {
    ...
    try {
      ...
      if (await fileExists(terminalPaths.acceptedJsonPath)) {
        ...
        const claim = await readJson<StoredTerminalClaim>(terminalPaths.claimPath);
        ...
        try {
          const [projectedRaw, projectedJson, reconciliation] = await Promise.all([
            ...
            readJson<Record<string, unknown>>(paths.resultJson),
            readJson<StoredTerminalReconciliation>(terminalPaths.reconciledJsonPath),
          ]);
      ...
      if (await terminalOutcomeEvidenceExistsForAgent(this.root, safeTaskId, safeAgentId)) throw error;
    }
  ...
  async appendRouteDecision(
    ...
    const record: DelegationRouteDecisionRecord = {
      taskId: safeTaskId,
  ...
  async readRouteDecisions(taskId: string): Promise<DelegationRouteDecisionRecord[]> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const records = await readJsonLines<DelegationRouteDecisionRecord>(taskPaths(this.root, safeTaskId).routesJsonl);
  ...
  async recordRouteApplication(input: DelegationRouteApplication): Promise<RecordRouteApplicationResult> {
    ...
    const toRecord: DelegationRouteApplication = { ...application };
  ...
  async readExecutionApprovalRequest(taskId: string): Promise<ExecutionApprovalRequest> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const paths = taskPaths(this.root, safeTaskId);
  ...
  async approveAndAuthorizeExecution(
    ...
    return this.withExecutionAuthorizationLock(safeTaskId, async () => {
      ...
      let authorization: DelegationEvent;
      try {
  ...
  private async recordPlanApprovedLocked(safeTaskId: string, input: PlanApprovedInput): Promise<DelegationEvent> {
    const paths = taskPaths(this.root, safeTaskId);
    const events = await readJsonLines<DelegationEvent>(paths.eventsJsonl);
    ...
    if (existing !== undefined) {
      if (taskEventContentMatches(existing, candidate)) return existing;
  ...
  async readExecutionAuthorization(taskId: string): Promise<DelegationExecutionAuthorizationEvidence | undefined> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const paths = taskPaths(this.root, safeTaskId);
    if (!(await fileExists(paths.taskJson))) return undefined;
    const task = await this.readTask(safeTaskId);
    if (task.taskId !== safeTaskId) return undefined;
    const events = await readJsonLines<DelegationEvent>(paths.eventsJsonl);
    ...
    if (authorizationIndex < 0 || executionId === undefined) return undefined;
    try {
      const envelopePath = executionEnvelopeFilePath(paths.executionEnvelopesDir, executionId);
  ...
  async hasExecutionAuthorization(taskId: string): Promise<boolean> {
    return (await this.readExecutionAuthorization(taskId)) !== undefined;
  }

  async appendLeaseEvent(taskId: string, input: AppendLeaseEventInput): Promise<DelegationLeaseEvent> {
    const safeTaskId = validateSafeId(taskId, "task id");
    await this.initTask({ taskId: safeTaskId });
  ...
  async transitionLease(
    ...
    const eventInput: AppendLeaseEventInput = {
      timestamp,
  ...
  async endActiveAssignmentLeases(
    taskId: string,
    agentId: string,
    state: "exhausted" | "revoked",
    reason: string,
  ): Promise<EndActiveAssignmentLeasesResult> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const safeAgentId = validateSafeId(agentId, "agent id");
    await this.initTask({ taskId: safeTaskId });
    let events = await this.readLeaseEvents(safeTaskId);
  ...
  async rebuildActiveLeaseView(taskId: string): Promise<DelegationActiveLeaseView> {
    const safeTaskId = validateSafeId(taskId, "task id");
    await this.initTask({ taskId: safeTaskId });
    const events = await this.readLeaseEvents(safeTaskId);
  ...
  async readActiveLeaseView(taskId: string): Promise<DelegationActiveLeaseView> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const paths = taskPaths(this.root, safeTaskId);
    const view = normalizeDelegationActiveLeaseView(await readJson<DelegationActiveLeaseView>(paths.activeLeasesJson));
    const events = await this.readLeaseEvents(safeTaskId);
  ...
  async recordLayoutAllocation(allocation: DelegationLayoutAllocation): Promise<DelegationLayoutAllocation> {
  ...
  async readLayoutState(taskId: string): Promise<DelegationLayoutState> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const paths = taskPaths(this.root, safeTaskId);
    if (!(await fileExists(paths.layoutJson))) {
      return { version: 1, taskId: safeTaskId, allocations: [], updatedAt: this.now() };
    }
    const state = await readJson<DelegationLayoutState>(paths.layoutJson);
    if (state.version !== 1) {
  ...
  async recordWakeAttempt(taskId: string, input: RecordWakeAttemptInput): Promise<DelegationWakeAttempt> {
    const safeTaskId = validateSafeId(taskId, "task id");
    const attempt: DelegationWakeAttempt = {
    ...
    try {
      const existing = (await readJsonLines<DelegationWakeAttempt>(path))
  ...
  private async ensureParentAlertQueue(taskId: string): Promise<void> {
    const safeTaskId = validateSafeId(taskId, "task id");
    await this.withParentAlertLock(safeTaskId, async () => {
      ...
      const queue: ParentAlertQueue = { version: 1, taskId: safeTaskId, alerts: [], updatedAt: this.now() };
  ...
  private async queueParentAlertRecord(
    ...
    return this.withParentAlertLock(safeTaskId, async () => {
      ...
      if (agentId !== undefined) candidate.agentId = agentId;
      if (parentAgentId !== undefined) candidate.parentAgentId = parentAgentId;
  ...
  private async withParentAlertLock<T>(taskId: string, operation: () => Promise<T>): Promise<T> {
  ...
  private async projectTaskReadyAfterAuthorization(taskId: string): Promise<void> {
  ...
  private async withLeaseLogLock<T>(taskId: string, operation: () => Promise<T>): Promise<T> {
  ...
  private async withExecutionAuthorizationLock<T>(taskId: string, operation: () => Promise<T>): Promise<T> {
  ...
  private async updateRegistryState(
    taskId: string,
    agentId: string,
    state: DelegationState,
    updatedAt = this.now(),
...
async function terminalOutcomeEvidenceExistsForAgent(root: string, taskId: string, agentId: string): Promise<boolean> {
...
async function writeImmutableText(path: string, text: string): Promise<void> {
  const directory = dirname(path);
  await mkdir(directory, { recursive: true });
  const tempPath = `${path}.tmp-${process.pid}-${randomUUID()}`;
  let handle;
  try {
    ...
    try {
      ...
      if (existing !== text) throw new Error(`immutable file conflict: ${path}`);
    }
    try {
...
interface ObservedFileLockOwner {
  record?: FileLockOwnerRecord;
  malformedFresh: boolean;
}

async function acquireBoundedFileLock(lockPath: string): Promise<() => Promise<void>> {
  ...
  try {
    ...
    const maxTicket = initialOwners.reduce((maximum, owner) => {
      if (owner.record === undefined || !processIsAlive(owner.record.pid)) return maximum;
    ...
    while (Date.now() - startedAt <= ALERT_LOCK_TIMEOUT_MS) {
      ...
      const blocked = owners.some((owner) => {
        if (owner.malformedFresh) return true;
        const other = owner.record;
        if (other === undefined || other.token === token || !processIsAlive(other.pid)) return false;
        if (other.choosing || other.ticket <= 0) return true;
...
async function readFileLockOwners(ownersDir: string): Promise<ObservedFileLockOwner[]> {
  const entries = await readdir(ownersDir, { withFileTypes: true });
  const owners: ObservedFileLockOwner[] = [];
...
function parseFileLockOwnerRecord(text: string): FileLockOwnerRecord | undefined {
  try {
    ...
    return record as FileLockOwnerRecord;
  } catch {
    return undefined;
  }
}
...
function stateForAlertOutcome(outcome: ParentAlertOutcome): DelegationState {
  ...
  if (outcome === "progress" || outcome === "info") return "running";
  return outcome;
}
...
function mergeUnreadParentAlert(existing: ParentAlert, candidate: ParentAlert, timestamp: string): ParentAlert {
  ...
  if (preserveExistingEvidence) {
    ...
    if (existing.eventType !== undefined) merged.eventType = existing.eventType;
    if (existing.evidence !== undefined) merged.evidence = existing.evidence;
    if (existing.data !== undefined) merged.data = existing.data;
  ...
  if (
    ...
    merged.data = { ...existingData, ...candidateData, sourceAlerts: sources } as JsonValue;
...
function mergeEscalationSourceLinks(left: unknown, right: unknown): Array<Record<string, JsonValue>> {
  const byAlertId = new Map<string, Record<string, JsonValue>>();
  for (const value of [...(Array.isArray(left) ? left : []), ...(Array.isArray(right) ? right : [])]) {
    const record = jsonRecord(value);
    if (record === undefined || typeof record.alertId !== "string") continue;
    const normalized: Record<string, JsonValue> = {};
    for (const key of ["alertId", "agentId", "outcome", "priority", "evidence"]) {
      const child = record[key];
      if (child !== undefined && isJsonValue(child)) normalized[key] = child;
    }
...
function strongerAlertPriority(left: DelegationAlertPriority, right: DelegationAlertPriority): DelegationAlertPriority {
  return alertPriorityRank(left) <= alertPriorityRank(right) ? left : right;
}

function alertPriorityRank(priority: DelegationAlertPriority): number {
...
function leaseLifecycleEventId(leaseId: string, state: DelegationLeaseState, ordinal: number): string {
  return validateSafeId(`lease-evt-${leaseId}-${state}-${ordinal}`, "lease event id");
}

function leaseAuthorityMatches(left: DelegationLease, right: DelegationLease): boolean {
  return JSON.stringify(leaseAuthorityContent(left)) === JSON.stringify(leaseAuthorityContent(right));
}

function leaseAuthorityContent(lease: DelegationLease): Record<string, unknown> {
  return {
    leaseId: lease.leaseId,
    taskId: lease.taskId,
...
async function resolveTerminalPublicationIdentity(
  manifest: AgentManifest,
  status: AgentStatus,
  paths: DelegationTaskPaths,
): Promise<ResolvedDelegationAssignmentAttemptIdentity> {
  try {
...
interface TerminalOutcomeAttemptPaths {
  attemptDir: string;
  lockPath: string;
  claimPath: string;
  acceptedRawPath: string;
  acceptedJsonPath: string;
  reconciledJsonPath: string;
  rejectedDir: string;
}

interface NormalizedTerminalEvidenceResult {
  evidence?: JsonValue;
  error?: string;
}

interface NormalizedTerminalSourceResult {
  source?: TerminalOutcomePublicationSource;
...
function terminalOutcomeAttemptPaths(
  paths: DelegationTaskPaths,
  assignmentId: string,
  attemptId: string,
): TerminalOutcomeAttemptPaths {
...
function normalizeTerminalOutcomeSource(input: TerminalOutcomePublicationSource): TerminalOutcomePublicationSource {
...
function normalizeTerminalOutcomeSourceForPublication(input: unknown): NormalizedTerminalSourceResult {
  const diagnostic = diagnosticJsonValue(input);
  try {
    return {
      source: normalizeTerminalOutcomeSource(input as TerminalOutcomePublicationSource),
      diagnostic,
    };
  } catch (error) {
...
function diagnosticJsonValue(value: unknown): JsonValue {
  try {
...
function normalizeTerminalOutcomeEvidence(input: JsonValue): NormalizedTerminalEvidenceResult {
  try {
...
function terminalOutcomeRejectionReason(input: {
  manifest: AgentManifest;
  identity: ResolvedDelegationAssignmentAttemptIdentity;
  submittedAssignmentId: string;
  submittedAttemptId: string;
  submittedRole: string;
  submittedStatus: string;
  evidenceResult: NormalizedTerminalEvidenceResult;
}): string | undefined {
  ...
  if (!TERMINAL_ROLES.includes(input.submittedRole as (typeof TERMINAL_ROLES)[number])) {
  ...
  if (!TERMINAL_STATUSES.includes(input.submittedStatus as (typeof TERMINAL_STATUSES)[number])) {
  ...
  if (evidence === undefined) return "terminal evidence must be an object";
  if (!nonEmptyJsonString(evidence.summary)) return "terminal evidence summary is required";
  ...
  if (input.submittedRole === "verifier") {
    ...
    for (const check of evidence.checks) {
      ...
      if (
        ...
        !TERMINAL_CHECK_STATUSES.includes(record.status as (typeof TERMINAL_CHECK_STATUSES)[number])
      ) {
  ...
  if (input.submittedRole === "reviewer" && evidence.findings !== undefined) {
    if (!Array.isArray(evidence.findings)) return "reviewer findings must be an array";
    for (const finding of evidence.findings) {
      const record = jsonObjectValue(finding);
      if (
        record === undefined ||
        typeof record.severity !== "string" ||
        !TERMINAL_FINDING_SEVERITIES.includes(record.severity as (typeof TERMINAL_FINDING_SEVERITIES)[number]) ||
  ...
  if (input.submittedRole === "execution-parent") {
    ...
    if (
      evidence.reportName !== "execution-report" ||
      !nonEmptyJsonString(evidence.reportStatus) ||
      report === undefined ||
...
async function planningParentTerminalEvidenceRejectionReason(
  paths: DelegationTaskPaths,
  taskId: string,
  agentId: string,
  identity: ResolvedDelegationAssignmentAttemptIdentity,
...
function normalizeCanonicalJsonValue(value: unknown, label: string): JsonValue {
  ...
  for (const key of Object.keys(value as Record<string, unknown>).sort()) {
    output[key] = normalizeCanonicalJsonValue((value as Record<string, unknown>)[key], `${label}.${key}`);
  }
  return output;
}
...
function nonEmptyJsonString(value: JsonValue | undefined): value is string {
...
async function abandonStaleTerminalClaim(
  paths: TerminalOutcomeAttemptPaths,
  claim: StoredTerminalClaim,
  ...
  let preservedRawPath: string | undefined;
  ...
  if (await fileExists(abandonmentPath)) {
    const existing = await readJson<Record<string, unknown>>(abandonmentPath);
    if (
...
function validateStoredTerminalClaim(
  claim: StoredTerminalClaim,
  taskId: string,
  agentId: string,
  identity: ResolvedDelegationAssignmentAttemptIdentity,
...
async function readValidTerminalAcceptedOutcome(
  paths: TerminalOutcomeAttemptPaths,
  taskId: string,
  agentId: string,
  identity: ResolvedDelegationAssignmentAttemptIdentity,
): Promise<StoredTerminalAcceptedOutcome> {
  const accepted = await readJson<StoredTerminalAcceptedOutcome>(paths.acceptedJsonPath);
  const rawText = await readFile(paths.acceptedRawPath, "utf8");
  ...
  if (
    ...
    !TERMINAL_ROLES.includes(accepted.role as (typeof TERMINAL_ROLES)[number]) ||
    !TERMINAL_STATUSES.includes(accepted.status as (typeof TERMINAL_STATUSES)[number]) ||
...
function terminalEvidenceSummary(evidence: JsonValue): string {
  const summary = jsonObjectValue(evidence)?.summary;
  if (!nonEmptyJsonString(summary)) throw new Error("accepted terminal evidence summary is unavailable");
  return summary;
}

function terminalAlertData(accepted: StoredTerminalAcceptedOutcome): JsonValue {
  const evidence = jsonObjectValue(accepted.evidence);
  if (evidence === undefined) throw new Error("accepted terminal evidence must be an object");
  const data: Record<string, JsonValue> = {
    terminalOutcomeId: accepted.outcomeId,
    role: accepted.role,
    resultStatus: accepted.status,
  };
  for (const key of ["filesChanged", "findings", "checks", "completionClaimSupported"] as const) {
    if (evidence[key] !== undefined) data[key] = evidence[key];
  }
  return data;
}

function terminalEffectEventId(outcomeId: string, scope: "agent" | "task"): string {
  return validateSafeId(`${scope}-result-${outcomeId}`, "terminal event id");
}

function terminalReconciliationPath(accepted: StoredTerminalAcceptedOutcome): string {
  return join(dirname(accepted.jsonPath), "terminal.reconciled.json");
}

function validateStoredTerminalReconciliation(
  reconciliation: StoredTerminalReconciliation,
  accepted: StoredTerminalAcceptedOutcome,
): void {
  if (
...
function terminalResultProjection(accepted: StoredTerminalAcceptedOutcome, rawText: string): JsonValue {
  const evidence = jsonObjectValue(accepted.evidence);
  if (evidence === undefined) throw new Error("accepted terminal evidence must be an object");
  ...
  if (suppliedProjection !== undefined) {
    ...
    if (projection === undefined) throw new Error("terminal resultProjection must be an object");
    return {
      ...projection,
      terminalOutcomeId: accepted.outcomeId,
...
async function ensureDelegationEvent(path: string, event: DelegationEvent): Promise<void> {
  const release = await acquireBoundedFileLock(`${path}.terminal.lock`);
  try {
    const events = await readJsonLines<DelegationEvent>(path);
    const existing = events.find((candidate) => candidate.eventId === event.eventId);
    if (existing !== undefined) {
...
async function recordTerminalRejection(input: {
  ...
  if (await fileExists(jsonPath)) {
    const stored = await readJson<StoredTerminalRejectedOutcome>(jsonPath);
  ...
  const stored: StoredTerminalRejectedOutcome = {
    schemaVersion: 1,
    recordType: "terminal.rejected",
    disposition: "rejected",
    rejectionId,
...
function planningReportPublicationId(
  taskId: string,
  disposition: "accepted" | "rejected",
  contentHash: string,
  source: PlanningReportPublicationSource,
...
function planningReportPublicationEvidencePaths(
  paths: DelegationTaskPaths,
  disposition: "accepted" | "rejected",
  publicationId: string,
): { rawPath: string; jsonPath: string } {
...
function lastEventIndexOfType(events: readonly DelegationEvent[], type: string): number {
...
function stringDataField(event: DelegationEvent, field: string): string | undefined {
...
function recordDataField(event: DelegationEvent, field: string): Record<string, JsonValue> | undefined {
  const data = event.data;
  if (data === null || typeof data !== "object" || Array.isArray(data)) return undefined;
  const value = data[field];
...
function stringArrayDataField(event: DelegationEvent, field: string): { valid: boolean; value?: string[] } {
  const data = event.data;
...
function normalizeExecutionEnvelope(input: DelegationExecutionEnvelope): DelegationExecutionEnvelope {
  ...
  if (!isStableTimestamp(input.createdAt)) throw new Error("execution envelope createdAt must be a timestamp");
  return {
    schemaVersion: CURRENT_EXECUTION_ENVELOPE_SCHEMA_VERSION,
...
interface PlanningReadyPublicationBinding {
  planningReady: DelegationEvent;
  accepted: DelegationEvent;
  publicationId: string;
  contentHash: string;
  rawPath: string;
  jsonPath: string;
  reportStatus: "ready" | "ready_with_open_questions";
  planArtifactPath: string;
  source: Record<string, JsonValue>;
}

function planningReadyPublicationBinding(
  events: readonly DelegationEvent[],
  planningReadyIndex: number,
  taskId: string,
): PlanningReadyPublicationBinding {
...
async function readValidAcceptedPlanningPublication(
  paths: DelegationTaskPaths,
  accepted: DelegationEvent,
): Promise<StoredPlanningReportPublication> {
  try {
    ...
    const stored = await readJson<StoredPlanningReportPublication>(expectedPaths.jsonPath);
    const parsed = parseProtocolText(rawText);
...
async function validatePlanningReadyPublicationEvidence(
  paths: DelegationTaskPaths,
  binding: PlanningReadyPublicationBinding,
): Promise<void> {
  const stored = await readValidAcceptedPlanningPublication(paths, binding.accepted);
  if (
    stored.publicationId !== binding.publicationId ||
...
function assertPlanningReadyPublicationIsCurrent(
  events: readonly DelegationEvent[],
  planningReadyIndex: number,
  taskId: string,
): void {
  ...
  if (latestAcceptedIndex > planningReadyIndex) {
    const latestStatus = stringDataField(events[latestAcceptedIndex] as DelegationEvent, "reportStatus");
...
function executionApprovalRequestFromEvents(
  taskId: string,
  paths: DelegationTaskPaths,
  events: readonly DelegationEvent[],
  ...
  if (planArtifactPath === undefined) throw new Error("latest planning-ready event has no plan artifact identity");
...
function validateExecutionPredecessors(
  events: readonly DelegationEvent[],
  taskId: string,
  planningId: string,
  approvalId: string,
...
function reconstructExecutionAuthorization(
  taskId: string,
  paths: DelegationTaskPaths,
  events: readonly DelegationEvent[],
  envelopeInput: DelegationExecutionEnvelope,
  authorizationIndex: number,
): DelegationExecutionAuthorizationEvidence | undefined {
  try {
    ...
    const predecessors = validateExecutionPredecessors(
      events,
      taskId,
      envelope.planningReportReadyEventId,
      envelope.planApprovedEventId,
      envelope.planArtifactPath,
    );
...
function validateLeaseEventSequence(events: readonly DelegationLeaseEvent[]): void {
  const eventIds = new Set<string>();
  const latest = new Map<string, DelegationLeaseEvent>();
...
function assertLeaseEventCanFollow(
  precedingEvents: Iterable<DelegationLeaseEvent>,
  candidate: DelegationLeaseEvent,
): void {
  let previous: DelegationLeaseEvent | undefined;
  for (const event of precedingEvents) {
    if (event.leaseId === candidate.leaseId) previous = event;
  }
...
function latestLeaseById(events: readonly DelegationLeaseEvent[]): Map<string, DelegationLeaseEvent> {
  const latest = new Map<string, DelegationLeaseEvent>();
  for (const event of events) {
    latest.set(event.leaseId, event);
  }
...
function activeLeaseViewsMatch(left: DelegationActiveLeaseView, right: DelegationActiveLeaseView): boolean {
  return JSON.stringify(activeLeaseViewPolicyContent(left)) === JSON.stringify(activeLeaseViewPolicyContent(right));
}

function activeLeaseViewPolicyContent(view: DelegationActiveLeaseView): Record<string, unknown> {
  return {
...
function normalizeAlertPriority(priority: DelegationAlertPriority): DelegationAlertPriority {
...
function normalizeWakeAttemptOutcome(outcome: DelegationWakeAttemptOutcome): DelegationWakeAttemptOutcome {
...
function normalizeWakeAttempt(taskId: string, input: DelegationWakeAttempt): DelegationWakeAttempt {
  const attempt: DelegationWakeAttempt = {
    attemptId: validateSafeId(input.attemptId, "wake attempt id"),
    taskId,
...
function uniqueSafeIds(values: readonly string[], label: string): string[] {
