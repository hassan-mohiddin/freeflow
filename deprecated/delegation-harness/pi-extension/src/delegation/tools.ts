...
import {
  ...
  deriveDelegationInlineLease,
  deriveRoutedAttemptId,
  deriveRoutedRecoveryAttemptId,
...
async function executeRequestExecutionAuthorization(params: any, ctx: any) {
  ...
  try {
    ...
    const commitState = (error as any)?.commitState;
...
async function executeRoute(params: any, ctx: any) {
  const taskId = validateSafeId(requireString(params.taskId, "taskId"), "task id");
  ...
  if (targetFiles !== undefined) request.targetFiles = targetFiles;
  const writeScopes = stringArrayParam(params.writeScopes, "writeScopes");
  if (writeScopes !== undefined) request.writeScopes = writeScopes;
  const riskFlags = stringArrayParam(params.riskFlags, "riskFlags");
  if (riskFlags !== undefined) request.riskFlags = riskFlags;
...
async function executeApplyRoute(pi: any, params: any, signal: AbortSignal | undefined, ctx: any) {
  const taskId = validateSafeId(requireString(params.taskId, "taskId"), "task id");
  ...
  if (!routeRecord.ok) return routeRecord.result;
...
async function applyChildRouteSpawnReuse(
  ...
  if (!requestEvidence.ok) return requestEvidence.result;
  ...
  if (!reusable.ok) return reusable.result;
  ...
  await store.registerAgent({
    ...
    state: "starting",
    retention: "auto",
    layoutPolicy: normalizeLayoutPolicy(undefined, role) as any,
  });
...
async function executeFinish(params: any, ctx: any) {
  ...
  if (!validation.ok) {
    ...
    if (rejected.status !== "rejected") throw new Error("invalid delegate_finish evidence unexpectedly committed");
...
async function finishParentReport(params: any, validation: any, target: any, store: any, role: string) {
  ...
  let report: any;
  let reportPaths: { rawPath: string; jsonPath: string };
  let planArtifactPath: string | undefined;
  let planningReadyEventId: string | undefined;
  let planningPublicationId: string | undefined;
  if (role === "planning-parent") {
    ...
    if (report === undefined) throw new Error("accepted planning publication has no parsed report");
  ...
  if (role === "planning-parent") evidence.planningPublicationId = planningPublicationId;
  else evidence.report = JSON.parse(JSON.stringify(report));
  for (const key of ["filesChanged", "checks", "findings", "completionClaimSupported", "recommendation", "data"]) {
    if (params[key] !== undefined) evidence[key] = JSON.parse(JSON.stringify(params[key]));
  }
  const publication = await store.publishTerminalOutcome(target.taskId, {
    agentId: target.agentId,
    assignmentId: target.identity.assignmentId,
...
function buildPlanningReportText(params: any, validation: any): any {
  ...
  const rows: Array<[string, string | undefined]> = [
    ...
    ...(planArtifactPath === undefined ? [] : [["PLAN_ARTIFACT_PATH", planArtifactPath] as [string, string]]),
...
async function executeRecordReport(params: any, ctx: any) {
  ...
  if (reportName === "planning-report") {
    ...
    let source: any = { transport: "delegate_record_report" };
  ...
  if (reportName === "execution-report") {
    ...
    if (envTaskId === undefined || envAgentId === undefined) {
      return typedError(
        "delegate_record_report",
        "delegated_lifecycle_identity_required",
        "execution-report publication is available only to the current delegated execution parent",
        { taskId, reportName },
      );
    }
    const target = await lifecycleTarget({ taskId, agentId: envAgentId }, ctx, store, "delegate_record_report");
...
function compactWakeDisposition(result: any): any {
...
function sortParentAlerts(alerts: any[]): any[] {
...
function alertPriorityRank(priority: unknown): number {
  return priority === "P0" ? 0 : priority === "P1" ? 1 : priority === "P2" ? 2 : 3;
}

function alertPriority(alert: any): string {
  ...
  if (
    ...
    return {
      ...
      result: typedError(
        operation,
        "delegated_lifecycle_identity_required",
        `${operation} is available only to the current delegated assignment; parent recovery uses a distinct parent-control operation`,
      ),
    };
  }
...
function routeActionFromParams(value: unknown): any {
...
async function lookupStoredExecutionAuthorization(store: any, taskId: string, callerProvided: boolean): Promise<any> {
  try {
...
async function readStoredRouteDecision(store: any, taskId: string, routeId: string): Promise<any> {
  let records;
  try {
  ...
  const record = records.filter((candidate: any) => candidate.routeId === routeId).at(-1);
  if (record === undefined) {
    return {
      ok: false,
      result: typedError(
...
async function readStoredRouteApplication(store: any, taskId: string, routeId: string): Promise<any> {
  try {
    const applications = await store.readRouteApplications(taskId);
    return { ok: true, application: applications.find((application: any) => application.routeId === routeId) };
  } catch (error) {
    return {
      ok: false,
...
async function legacyParentRouteAttention(store: any, input: any): Promise<any> {
...
async function alreadyAppliedRouteResult(
  ...
  if (!completeness.ok) return completeness.result;
...
function routeApplicationRequiresLayout(decision: any): boolean {
  return decision?.kind === "route_required";
}

function routeApplicationIdFor(routeId: string): string {
  return validateSafeId(`apply-${routeId}`, "route application id");
}

function parentRouteAgentId(role: string): string {
  return validateSafeId(`${role}-1`, "parent route agent id");
}

function isChildRouteRole(role: string): boolean {
  return role === "worker" || role === "reviewer" || role === "verifier" || role === "researcher";
}

function childRouteProfile(role: string): string {
  if (role === "verifier") return "check-runner";
  if (role === "reviewer" || role === "researcher") return "read-only";
  return "worker";
}

function childRouteAssignmentId(role: string, routeId: string): string {
  return validateSafeId(`${role}-${routeId}`, "child route assignment id");
}

function childPreferredLayoutGroup(role: string): "execution" | "review" | "scratch" {
  if (role === "worker") return "execution";
  if (role === "reviewer" || role === "verifier") return "review";
  return "scratch";
}

function childWaitingFor(role: string): string {
  if (role === "worker") return "WORKER_RESULT";
  if (role === "reviewer") return "REVIEW_RESULT";
  if (role === "verifier") return "VERIFICATION_RESULT";
...
async function findReusableDirectSpawn(store: any, input: any): Promise<any> {
  ...
  if (input.windowRef !== undefined && manifest.windowRef !== input.windowRef) mismatches.push("windowRef");
...
function directSpawnExistingStateError(store: any, input: any, code: string, reason: string): any {
  return {
    ok: false,
...
async function findReusableChildRouteAgent(
  ...
  try {
    ...
    validateTaskPacketIdentity(await readFile(canonicalPacketPath, "utf8"), {
      ...
      attemptId: identity.attemptId,
      role: role as any,
      profile: expectedProfile as any,
      identitySchemaVersion: identity.schemaVersion,
      profileSchemaVersion: identity.profileSchemaVersion,
      protocolVersion: identity.protocolVersion,
    });
...
function childRouteOutOfScope(role: string): string[] {
...
function childRoutePolicy(compiled: any): any {
  return {
    writeScope: compiled.writeScopes,
    allowedCommands: compiled.allowedCommands,
    tools: compiled.tools,
...
function assignmentLeaseFor(input: {
  ...
  if (input.routeId !== undefined) lease.routeId = input.routeId;
  return lease;
}
...
async function routedWorkerCommandAuthority(
  ...
  let candidates = (executionMap.packages ?? []).filter((pkg: any) => {
    if (pkg.role !== "worker") return false;
    if (pkg.agentId !== undefined && pkg.agentId !== agentId) return false;
    const packageCheckout = isAbsolute(pkg.checkoutPath) ? resolve(pkg.checkoutPath) : resolve(cwd, pkg.checkoutPath);
    if (resolve(cwd) !== packageCheckout) return false;
  ...
  candidates.sort((left: any, right: any) => left.packageId.localeCompare(right.packageId));
  if (candidates.length !== 1) {
    return {
      ...
      candidatePackageIds: candidates.map((pkg: any) => pkg.packageId),
    };
  }
...
function compactManifestCmuxRefs(manifest: any): any {
  return {
    windowRef: manifest.windowRef,
    workspaceRef: manifest.workspaceRef,
...
function childRouteApplicationAgentId(application: any): string | undefined {
...
function compactLayoutAllocation(allocation: any): any | undefined {
...
function childMaterializationFor(decision: any, allocation: any, application: any): any | undefined {
...
function nextApplyRouteAction(decision: any, application: any, allocation: any): string {
  if (decision.kind === "inline_allowed") {
...
function applyRoutePaths(store: any, taskId: string): any {
...
function routeActionGuidance(decision: any): string {
...
function stringArrayParam(value: unknown, label: string): string[] | undefined {
...
class StartupTransactionError extends Error {
  ...
  constructor(code: string, failureMessage: string, cause: unknown, extra: any = {}) {
...
async function failStartupTransactionBestEffort(
  store: any,
  taskId: string,
  agentId: string,
  message: string,
...
): Promise<{ authorityCleanupErrors: string[]; failurePersistenceErrors: string[] }> {
  const authorityCleanupErrors: string[] = [];
  const failurePersistenceErrors: string[] = [];
...
function compactDelegationToolText(toolName: string, result: any): string {
  ...
  if (toolName === "delegate_route") appendRouteRows(lines, result);
  if (toolName === "delegate_apply_route") appendApplyRouteRows(lines, result);
...
function appendRouteRows(lines: string[], result: any): void {
  ...
  if (Array.isArray(decision.reasonCodes)) {
    ...
    if (decision.reasonCodes.length > 10) lines.push(row("reason_codes_more", decision.reasonCodes.length - 10));
  }
  if (decision.question) lines.push(row("question", truncateLine(decision.question, 240)));
  if (decision.reason) lines.push(row("blocked_reason", truncateLine(decision.reason, 240)));
  if (result?.actionGuidance) lines.push(row("next_action", truncateLine(result.actionGuidance, 260)));
}
...
function appendPathRows(lines: string[], paths: any): void {
  ...
  if (paths.routes) lines.push(row("routes", paths.routes));
  if (paths.routeApplications) lines.push(row("route_applications", paths.routeApplications));
  if (paths.layout) lines.push(row("layout_path", paths.layout));
}
...
function safeConfirmationText(value: unknown): string {
