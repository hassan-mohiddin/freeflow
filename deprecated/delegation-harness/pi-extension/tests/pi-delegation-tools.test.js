...
import { compileTaskPacket, createDelegationStore, parseModelText, parseProtocolText, resolveProfileForRole } from "../../delegation/dist/index.js";
...
function storedChildRequest(taskId, routeId, role = "worker", overrides = {}) {
  return {
    ...
    action: overrides.action ?? { kind: role === "worker" ? "implement" : role === "reviewer" ? "review" : role === "verifier" ? "verify" : "research", breadth: "single_file", description: `Run stored ${role} route ${routeId}.` },
    targetFiles: overrides.targetFiles ?? ["src/target.ts"],
    ...(overrides.writeScopes !== undefined ? { writeScopes: overrides.writeScopes } : role === "worker" ? { writeScopes: ["src/**"] } : {}),
...
test("delegate_request_execution_authorization binds only host-confirmed canonical planning evidence", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    const recorded = await tools.get("delegate_record_report").execute("record-planning-ready", { taskId, reportName: "planning-report", rawText: planningReport }, undefined, undefined, ctx(repoRoot));
    ...
    assert.equal(resolveProfileForRole("orchestrator").activeTools.includes("delegate_request_execution_authorization"), true);
    assert.equal(resolveProfileForRole("planning-parent").activeTools.includes("delegate_request_execution_authorization"), false);
    assert.equal(resolveProfileForRole("execution-parent").activeTools.includes("delegate_request_execution_authorization"), false);
    const confirmations = [];
    ...
    const routed = await tools.get("delegate_route").execute("route-after-owner-confirm", {
      taskId,
    ...
    const replacementReport = planningReport.replace(planArtifactPath, replacementPlan).replace(planArtifactPath, replacementPlan);
    await tools.get("delegate_record_report").execute("record-planning-ready-v2", { taskId, reportName: "planning-report", rawText: replacementReport }, undefined, undefined, ctx(repoRoot));
    const reauthorized = await requestAuthorization.execute("authorize-owner-v2", { taskId }, undefined, undefined, toolCtx);
    ...
    const blockedReport = replacementReport.replace("STATUS|ready", "STATUS|blocked").replace("RISKS|none", "RISKS|planning blocker");
    const blocked = await tools.get("delegate_record_report").execute("record-planning-blocked", { taskId, reportName: "planning-report", rawText: blockedReport }, undefined, undefined, ctx(repoRoot));
    ...
    const rerouted = await tools.get("delegate_route").execute("route-after-planning-blocked", {
      taskId,
      agentId: "orchestrator",
      role: "orchestrator",
      action: { kind: "implement", breadth: "broad" },
    ...
    const unavailable = await requestAuthorization.execute("authorize-blocked", { taskId }, undefined, undefined, toolCtx);
...
test("delegate_request_execution_authorization fails closed without owner confirmation or on stale evidence", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    const noUi = await requestAuthorization.execute("authorize-no-ui", { taskId: "TASK-NO-UI" }, undefined, undefined, ctx(repoRoot));
    ...
    const declined = await requestAuthorization.execute("authorize-declined", { taskId: "TASK-DECLINED" }, undefined, undefined, declineCtx);
    ...
    const stale = await requestAuthorization.execute("authorize-stale", { taskId: "TASK-STALE-CONFIRM" }, undefined, undefined, staleCtx);
    ...
    const staleEvents = (await readFile(store.pathsForTask("TASK-STALE-CONFIRM").eventsJsonl, "utf8")).trim().split("\n").map((line) => JSON.parse(line));
    assert.equal(staleEvents.some((event) => event.type === "plan.approved"), false);
  }));
});
...
test("delegate_request_execution_authorization uses RPC host confirmation and fails closed without an RPC responder", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    rpcCtx.mode = "rpc";
    rpcCtx.hasUI = true;
    rpcCtx.ui.confirm = async () => { rpcPrompts += 1; return true; };

    const confirmed = await requestAuthorization.execute("authorize-rpc", { taskId: "TASK-RPC-AUTH" }, undefined, undefined, rpcCtx);
    ...
    const unavailable = await requestAuthorization.execute("authorize-rpc-unavailable", { taskId: "TASK-RPC-NO-RESPONDER" }, undefined, undefined, unavailableCtx);
...
test("delegate_request_execution_authorization sanitizes owner-visible stored identity", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    toolCtx.mode = "tui";
    toolCtx.hasUI = true;
    toolCtx.ui.confirm = async (_title, message) => { displayed = message; return false; };

    await tools.get("delegate_request_execution_authorization").execute("authorize-sanitize", { taskId }, undefined, undefined, toolCtx);
...
test("delegate_request_execution_authorization rejects delegated child callers before prompting", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await withProcessEnv({ ...DELEGATION_ENV_CLEAR, FREEFLOW_DELEGATION_TASK_ID: "TASK-CHILD-AUTH", FREEFLOW_DELEGATION_AGENT_ID: "planning-parent-1" }, async () => {
      const { tools } = loadExtension(() => ok());
      let prompted = false;
      const childCtx = ctx(repoRoot);
      childCtx.mode = "tui";
      childCtx.hasUI = true;
      childCtx.ui.confirm = async () => { prompted = true; return true; };
      const result = await tools.get("delegate_request_execution_authorization").execute("authorize-child", { taskId: "TASK-CHILD-AUTH" }, undefined, undefined, childCtx);
...
test("delegate_route routes orchestrator broad implementation to planning-parent and stores the decision", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await route.execute("route-planning", {
...
test("delegate_route uses stored execution authorization to route orchestrator broad implementation to execution-parent", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const approval = await store.recordPlanApproved("TASK-ROUTE-AUTH", { eventId: "evt.plan.approved", planningReportReadyEventId: ready.eventId, planArtifactPath: "docs/plans/approved-plan.md", approvedBy: "user" });
    await store.recordExecutionAuthorized("TASK-ROUTE-AUTH", { eventId: "evt.execution.authorized", planningReportReadyEventId: ready.eventId, planApprovedEventId: approval.eventId, planArtifactPath: "docs/plans/approved-plan.md" });
    const { tools } = loadExtension(() => ok());
    const route = tools.get("delegate_route");

    const result = await route.execute("route-execution", {
    ...
    const coherentlyTamperedRaw = (await readFile(accepted.data.rawPath, "utf8"))
      .replace("GOAL|Authorize the accepted plan.", "GOAL|Coherently altered route evidence.");
    ...
    const rerouted = await route.execute("route-after-planning-evidence-tamper", {
      taskId: "TASK-ROUTE-AUTH",
      agentId: "orchestrator",
      role: "orchestrator",
      action: { kind: "implement", breadth: "broad" },
...
test("delegate_route treats hasApprovedPlan as a hint, not execution authorization", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await route.execute("route-plan-hint", {
    ...
    assert.deepEqual(storedDecision.request.action, { kind: "implement", breadth: "broad", description: "Implement stored route request evidence." });
    ...
    assert.deepEqual(storedDecision.request.targetFiles, ["pi-extension/src/delegation/tools.ts", "delegation/src/store.ts"]);
...
test("delegate_route attaches a deterministic issued lease to tiny inline implementation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const routed = await tools.get("delegate_route").execute("route-inline-lease", {
...
test("delegate_apply_route applies stored inline route idempotently without cmux or layout panes", async () => {
  await withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    await store.appendRouteDecision("TASK-APPLY-INLINE", {
      kind: "inline_allowed",
    ...
    const applied = await applyRoute.execute("apply-inline", { taskId: "TASK-APPLY-INLINE", routeId: "route-inline-1" }, undefined, undefined, ctx(repoRoot));
    const reapplied = await applyRoute.execute("apply-inline-again", { taskId: "TASK-APPLY-INLINE", routeId: "route-inline-1" }, undefined, undefined, ctx(repoRoot));
    ...
    assert.deepEqual((await store.readActiveLeaseView("TASK-APPLY-INLINE")).activeLeaseIdsByAgent, { "execution-parent-1": ["lease-route-inline-1"] });
...
test("delegate_apply_route applies planning-parent route through stored right-top layout and ignores caller target hints", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-planning", {
      taskId: "TASK-APPLY-PLANNING",
      routeId: "route-planning-parent-1",
      targetRole: "execution-parent",
    }, undefined, undefined, ctx(repoRoot));
    const routeApplicationsBefore = await readFile(store.pathsForTask("TASK-APPLY-PLANNING").routeApplicationsJsonl, "utf8");
    const layoutBefore = await readFile(store.pathsForTask("TASK-APPLY-PLANNING").layoutJson, "utf8");
    const reapplied = await applyRoute.execute("apply-planning-again", {
      taskId: "TASK-APPLY-PLANNING",
      routeId: "route-planning-parent-1",
      targetRole: "worker",
    }, undefined, undefined, ctx(repoRoot));
    ...
    assert.equal(calls.length, 0);
    assert.match(result.content[0].text, /apply_route\|route-planning-parent-1\|route_required\|target=planning-parent\|state=applied/);
    assert.match(result.content[0].text, /layout\|right-top\|allocation=layout-planning-parent-1-planning-parent\|focus=preserved/);
    ...
    assert.equal(await readFile(store.pathsForTask("TASK-APPLY-PLANNING").routeApplicationsJsonl, "utf8"), routeApplicationsBefore);
...
test("legacy running parent application exposes an additive routed-recovery candidate without resending startup", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await writeFile(store.pathsForAgent(taskId, agentId).manifestJson, `${JSON.stringify(legacyManifest, null, 2)}\n`, "utf8");
    ...
    const result = await applyRoute.execute("recover-legacy-parent", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for execution-parent without stored authorization even with caller hints", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-exec-no-auth", {
      ...
      executionAuthorization: {
        ...
        taskState: "ready_for_execution",
        taskId: "TASK-APPLY-EXEC-NO-AUTH",
        executionMapPath: join(repoRoot, ".freeflow", "delegation", "tasks", "TASK-APPLY-EXEC-NO-AUTH", "execution-map.json"),
        planArtifactPath: "docs/plans/fake.md",
        approvedBy: "user",
      },
    }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route rejects a causally invalid stored authorization before startup mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await tools.get("delegate_apply_route").execute("apply-exec-invalid-auth", {
      taskId,
      routeId: "route-execution-parent-invalid-auth",
    }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route applies execution-parent with stored authorization and records right-top layout", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const approval = await store.recordPlanApproved("TASK-APPLY-EXEC-AUTH", { eventId: "evt.plan.approved", planningReportReadyEventId: ready.eventId, planArtifactPath: "docs/plans/approved-plan.md", approvedBy: "orchestrator" });
    await store.recordExecutionAuthorized("TASK-APPLY-EXEC-AUTH", { eventId: "evt.execution.authorized", planningReportReadyEventId: ready.eventId, planApprovedEventId: approval.eventId, planArtifactPath: "docs/plans/approved-plan.md" });
    ...
    const result = await applyRoute.execute("apply-exec-auth", { taskId: "TASK-APPLY-EXEC-AUTH", routeId: "route-execution-parent-auth" }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route spawns a stored worker route once and reuses the recorded application", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await appendChildRouteDecision(store, taskId, routeId, "worker", {
      action: { kind: "implement", breadth: "multi_file", description: "Implement the stored worker route from request evidence." },
    ...
    const applied = await applyRoute.execute("apply-worker", {
      taskId,
      routeId,
      targetRole: "planning-parent",
      kind: "inline_allowed",
    ...
    const reapplied = await applyRoute.execute("apply-worker-again", {
      taskId,
      routeId,
      targetRole: "execution-parent",
    }, undefined, undefined, ctx(repoRoot));
    ...
    assert.match(applied.content[0].text, /apply_route\|route-worker-spawn\|route_required\|target=worker\|state=applied/);
    assert.match(applied.content[0].text, /spawned\|worker-route-worker-spawn/);
    assert.match(applied.content[0].text, /materialization\|spawned\|assignment=worker-route-worker-spawn\|role=worker\|running=true/);
    assert.match(reapplied.content[0].text, /route_application\|already_applied\|id=apply-route-worker-spawn\|waitingFor=WORKER_RESULT/);
    assert.match(reapplied.content[0].text, /action\|existing_child_route_application_reused_no_cmux_new_pane_or_task_packet/);
...
test("routed worker commands come only from one deterministic execution-map package", async () => {
  for (const mode of ["unique", "ambiguous"]) {
    await withTempRepo(async (repoRoot) => {
      ...
      const packages = mode === "unique"
        ? [packageFor("P1", ["src/**"], assignmentId)]
        : [packageFor("P1", ["src/**"]), packageFor("P2", ["src/target.ts"])];
      await writeFile(store.pathsForTask(taskId).executionMapJson, `${JSON.stringify({ version: 1, taskId, packages, integrationOrder: [], updatedAt: "2026-07-09T00:00:00.000Z" }, null, 2)}\n`, "utf8");

      const { tools } = loadExtension(cmuxLifecycleExec());
      const applied = await tools.get("delegate_apply_route").execute(`apply-${mode}`, { taskId, routeId }, undefined, undefined, ctx(repoRoot));
      const manifest = await store.readAgentManifest(taskId, assignmentId);
...
test("delegate_apply_route reuses a pre-existing active deterministic child assignment", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-worker-reuse", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
    ...
    assert.equal(calls.some((call) => call.command[0] === "cmux" && call.command[1] === "new-pane"), false);
    assert.equal(calls.some((call) => call.command[0] === "cmux" && call.command[1] === "send"), false);
    const applications = await store.readRouteApplications(taskId);
    assert.equal(applications.length, 1);
    ...
    assert.match(await readFile(store.pathsForAgent(taskId, assignmentId).taskPacketRaw, "utf8"), /- attempt: attempt-route-worker-reuse/);
  });
});
...
test("delegate_apply_route revokes active authority when a post-activation event write fails", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await tools.get("delegate_apply_route").execute("apply-post-activation-store-fail", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
    ...
    assert.deepEqual((await store.readLeaseEvents(taskId)).map((event) => event.state), ["issued", "active", "revoked"]);
...
test("delegate_apply_route fails closed on child route cmux pane creation and startup failures", async () => {
  for (const [failAt, expectedCode] of [["new-pane", "cmux_new_pane_failed"], ["surface-ref", "cmux_surface_ref_missing"], ["send", "child_pi_start_failed"]]) {
    await withTempRepo(async (repoRoot) => {
      const taskId = `TASK-APPLY-WORKER-${String(failAt).toUpperCase().replace(/[^A-Z]/g, "-")}`;
      ...
      const result = await applyRoute.execute(`apply-${failAt}`, { taskId, routeId }, undefined, undefined, ctx(repoRoot));
      ...
      assert.deepEqual(leaseEvents.map((event) => event.state), ["issued", "active", "revoked"], failAt);
...
test("delegate_apply_route fails closed for mismatched active reuse state, profile, and parent", async () => {
  const cases = [
    ...
    },
    {
      ...
      async mutate(store, taskId, assignmentId) {
        ...
        await writeFile(manifestPath, `${JSON.stringify({ ...manifest, attemptId: "attempt-wrong" }, null, 2)}\n`, "utf8");
      },
    },
    {
  ...
  for (const entry of cases) {
    await withTempRepo(async (repoRoot) => {
      ...
      const result = await applyRoute.execute(`apply-${entry.name}`, { taskId, routeId }, undefined, undefined, ctx(repoRoot));
      ...
      assert.equal(calls.some((call) => call.command[0] === "cmux" && call.command[1] === "new-pane"), false, entry.name);
      assert.equal(calls.some((call) => call.command[0] === "cmux" && call.command[1] === "send"), false, entry.name);
    });
  }
});
...
test("delegate_apply_route fails closed for mismatched active reuse identity before mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-identity-mismatch", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for child route missing stored request evidence before cmux or mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-worker-no-request", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for worker route missing stored writeScopes before cmux or mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-worker-no-scope", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
  ...
  await withTempRepo(async (repoRoot) => {
    ...
    for (const role of ["reviewer", "researcher", "verifier"]) {
      ...
      await appendChildRouteDecision(store, taskId, routeId, role, {
        action: { kind: role === "reviewer" ? "review" : role === "verifier" ? "verify" : "research", breadth: "single_file", description: `Run ${role} from stored route request.` },
        targetFiles: ["src/target.ts"],
        writeScopes: ["caller/hint/ignored/**"],
        riskFlags: ["unknown"],
      });

      const result = await applyRoute.execute(`apply-${role}`, { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for existing materialization-only child route application", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("reapply-materialization-only", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed when reapplying a child route with missing layout allocation evidence", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const reapplied = await applyRoute.execute("reapply-worker-missing-layout", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed when reapplying a child route with malformed layout state", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await store.recordRouteApplication({ applicationId: `apply-${routeId}`, routeId, taskId, state: "applied", decisionKind: "route_required", layoutAllocationId: allocation.allocationId, spawned: [assignmentId], waitingFor: "REVIEW_RESULT" });
    ...
    const reapplied = await applyRoute.execute("reapply-reviewer-bad-layout", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed when reapplying a child route with mismatched allocation role and assignment", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await store.recordRouteApplication({ applicationId: `apply-${routeId}`, routeId, taskId, state: "applied", decisionKind: "route_required", layoutAllocationId: allocation.allocationId, spawned: [assignmentId], waitingFor: "WORKER_RESULT" });
    const { tools, calls } = loadExtension(() => {
      throw new Error("corrupt already-applied child allocation must not call cmux");
    });
    const applyRoute = tools.get("delegate_apply_route");

    const reapplied = await applyRoute.execute("reapply-worker-corrupt-allocation", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed when reapplying a child route with non-canonical prompt and report paths", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const reapplied = await applyRoute.execute("reapply-reviewer-corrupt-paths", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route applies read-only child overflow after two existing read-only allocations", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const first = await applyRoute.execute("apply-researcher-overflow", { taskId, routeId: "route-researcher-overflow" }, undefined, undefined, ctx(repoRoot));
    const second = await applyRoute.execute("apply-verifier-overflow", { taskId, routeId: "route-verifier-overflow" }, undefined, undefined, ctx(repoRoot));
    const third = await applyRoute.execute("apply-reviewer-overflow", { taskId, routeId: "route-reviewer-overflow" }, undefined, undefined, ctx(repoRoot));
    ...
    assert.match(third.content[0].text, /layout\|right-surface-overflow\|allocation=layout-reviewer-route-reviewer-overflow-reviewer\|focus=preserved/);
    assert.equal(calls.filter((call) => call.command[0] === "cmux" && call.command[1] === "new-pane").length, 3);
    const layout = await store.readLayoutState(taskId);
    assert.deepEqual(layout.allocations.map((allocation) => allocation.slot).sort(), ["right-bottom", "right-bottom", "right-surface-overflow"].sort());
...
test("delegate_apply_route fails closed on malformed layout state before child route application mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-bad-layout", { taskId, routeId }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route leaves integrator routes unsupported without mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-integrator", { taskId, routeId: "route-integrator" }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route declines ask-user and blocked route decisions without mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const askUser = await applyRoute.execute("apply-ask-user", { taskId: "TASK-APPLY-DECLINE", routeId: "route-ask-user" }, undefined, undefined, ctx(repoRoot));
    const blocked = await applyRoute.execute("apply-blocked", { taskId: "TASK-APPLY-DECLINE", routeId: "route-blocked" }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for missing stored route decision without mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-missing", { taskId: "TASK-APPLY-MISSING", routeId: "route-missing" }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for malformed stored route decision log without mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-bad-route", { taskId: "TASK-APPLY-BAD-ROUTE", routeId: "route-bad" }, undefined, undefined, ctx(repoRoot));
...
test("delegate_apply_route fails closed for malformed stored route application log before layout mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await applyRoute.execute("apply-bad-application", { taskId: "TASK-APPLY-BAD-APPLICATION", routeId: "route-planning-bad-app-log" }, undefined, undefined, ctx(repoRoot));
    ...
    await assert.rejects(() => store.readRouteApplications("TASK-APPLY-BAD-APPLICATION"), /Expected property name|Unexpected token|not valid JSON/);
  });
});
...
test("delegate_spawn reuses an identical complete running assignment without opening a second pane", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    assert.deepEqual((await store.readLeaseEvents(params.taskId)).map((event) => event.state), ["issued", "active"]);
  ...
  for (const entry of [
    {
      ...
      async arrange(store, repoRoot, spawn) {
        await spawn.execute("spawn-mismatch-seed", {
          taskId: "TASK-SPAWN-MISMATCHED",
          agentId: "worker-1",
          parentAgentId: "execution-parent-1",
          role: "worker",
    ...
    },
    {
      ...
      async arrange(store, repoRoot, spawn) {
        await spawn.execute("spawn-terminal-seed", {
          taskId: "TASK-SPAWN-TERMINAL",
          agentId: "worker-1",
          parentAgentId: "execution-parent-1",
          role: "worker",
    ...
    await withTempRepo(async (repoRoot) => {
      ...
      const result = await spawn.execute(`spawn-${entry.name}-duplicate`, entry.params(repoRoot), undefined, undefined, ctx(repoRoot));
...
test("delegate_spawn revokes authority before best-effort failure persistence when running status write fails", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await tools.get("delegate_spawn").execute("spawn-post-activation-store-fail", {
      taskId,
      agentId,
      role: "worker",
      cwd: repoRoot,
...
test("delegate_spawn revokes its active assignment lease on startup failure", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const result = await tools.get("delegate_spawn").execute("spawn-start-failure", {
    ...
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    assert.deepEqual((await store.readLeaseEvents("TASK-SPAWN-LEASE-FAIL")).map((event) => event.state), ["issued", "active", "revoked"]);
...
test("delegate_finish stores direct results, queues inbox alerts, and ack tools clear them", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const finishManifest = await store.registerAgent({ taskId: "TASK-FINISH", agentId: "worker-1", role: "worker", cwd: repoRoot, writeScope: repoRoot, state: "running", surfaceRef: "surface:3", parentAgentId: "execution-parent-1" });
    ...
    const finished = await withProcessEnv(delegatedEnv(store, finishManifest), () => finish.execute("finish-worker", {
      taskId: "TASK-FINISH",
      agentId: "worker-1",
      status: "completed",
    ...
    assert.match(finished.content[0].text, /alert\|P2\|completed\|state=queued\|agent=worker-1\|.*Implemented focused worker slice\./);
    ...
    assert.equal(JSON.parse(await readFile(finished.details.result.paths.acceptedJson, "utf8")).outcomeId, finished.details.result.terminalOutcomeId);

    const retry = await withProcessEnv(delegatedEnv(store, finishManifest), () => finish.execute("finish-worker-retry", {
      taskId: "TASK-FINISH",
      agentId: "worker-1",
      status: "completed",
      summary: "Implemented focused worker slice.",
    ...
    assert.equal((await store.readParentAlerts("TASK-FINISH")).filter((alert) => alert.eventType === "agent-result").length, 1);
...
test("root callers cannot impersonate child lifecycle identity through delegate_finish parameters", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    const { tools } = loadExtension(() => { throw new Error("delegate_finish must not call cmux"); });

    const result = await tools.get("delegate_finish").execute("finish-root-impersonation", {
      taskId: manifest.taskId,
      agentId: manifest.agentId,
      status: "completed",
    ...
    const attention = await tools.get("delegate_attention").execute("attention-root-impersonation", {
      taskId: manifest.taskId,
      agentId: manifest.agentId,
      message: "Root must not send child attention.",
    }, undefined, undefined, ctx(repoRoot));
    const progress = await tools.get("delegate_progress").execute("progress-root-impersonation", {
      taskId: manifest.taskId,
      agentId: manifest.agentId,
    ...
    await assert.rejects(readFile(store.pathsForAgent(manifest.taskId, manifest.agentId).resultJson, "utf8"), /ENOENT/);
  }));
});
...
test("delegate_finish rejects a superseded environment attempt before result mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await withProcessEnv({
      ...
      const result = await tools.get("delegate_finish").execute("finish-old-attempt", {
        status: "completed",
      ...
      await assert.rejects(readFile(store.pathsForAgent("TASK-FINISH-ATTEMPT", "worker-1").resultJson, "utf8"), /ENOENT/);
...
test("delegate_finish rejects stored packet identity mismatch before result mutation", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    await withProcessEnv(delegatedEnv(store, manifest), async () => {
      const result = await tools.get("delegate_finish").execute("finish-packet-mismatch", {
        taskId: "TASK-FINISH-PACKET",
        agentId: "worker-1",
        status: "completed",
      ...
      await assert.rejects(readFile(store.pathsForAgent("TASK-FINISH-PACKET", "worker-1").resultJson, "utf8"), /ENOENT/);
...
test("known unversioned running assignment can finish only under a synthetic attempt", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const { schemaVersion, identitySchemaVersion, profileSchemaVersion, protocolVersion, assignmentId, attemptId, attemptSource, ...legacyManifest } = manifest;
    await writeFile(store.pathsForAgent("TASK-FINISH-LEGACY", "worker-legacy").manifestJson, `${JSON.stringify(legacyManifest, null, 2)}\n`, "utf8");
    const { tools } = loadExtension(() => { throw new Error("delegate_finish must not call cmux"); });
    ...
    await withProcessEnv({
      ...
      const withoutLease = await tools.get("delegate_finish").execute("finish-legacy-without-lease", {
        status: "completed",
      ...
      await assert.rejects(readFile(store.pathsForAgent("TASK-FINISH-LEGACY", "worker-legacy").resultJson, "utf8"), /ENOENT/);

      await store.ensureLeaseActive("TASK-FINISH-LEGACY", {
      ...
      const result = await tools.get("delegate_finish").execute("finish-legacy", {
        status: "completed",
      ...
      const stored = JSON.parse(await readFile(store.pathsForAgent("TASK-FINISH-LEGACY", "worker-legacy").resultJson, "utf8"));
...
test("planning-parent delegate_finish routes rejected and corrected reports through semantic publication", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    const manifest = await store.registerAgent({ taskId, agentId: "planning-parent-1", role: "planning-parent", profile: "planning-parent", cwd: repoRoot, writeScope: join(repoRoot, "docs"), state: "running", surfaceRef: "surface:plan", parentAgentId: "orchestrator" });
    await writeCurrentTaskPacket(store, manifest);
    const acceptedSeed = await store.publishPlanningReport(taskId, {
      rawText: [
        "PLANNING_REPORT", "STATUS|ready", "GOAL|Seed accepted plan.",
        "PLAN_ARTIFACT_PATH|docs/plans/seed.md", "ARTIFACT_PATHS|docs/plans/seed.md", "REVIEW_STATUS|passed",
        "SETTLED_DECISIONS|seed", "OPEN_QUESTIONS|none", "EXECUTION_AUTONOMY|bounded", "USER_CHECKPOINTS|authorization",
        "EXECUTION_GUIDANCE|follow seed", "RISKS|none", "EVIDENCE|tests", "END_PLANNING_REPORT",
      ].join("\n"),
      source: { transport: "delegate_record_report" },
    });
    ...
    const rejected = await withProcessEnv(delegatedEnv(store, manifest), () => finish.execute("finish-planning-ambiguous", {
      taskId,
    ...
    const rejectedEvents = (await readFile(store.pathsForTask(taskId).eventsJsonl, "utf8")).trim().split("\n").map((line) => JSON.parse(line)).filter((event) => event.type === "planning_report.rejected");
    assert.equal(rejectedEvents.length, 1);
    assert.match(rejectedEvents[0].data.rawPath, /planning-report-publications\/rejected/);

    const corrected = await withProcessEnv(delegatedEnv(store, manifest), () => finish.execute("finish-planning-corrected", {
      taskId,
      agentId: "planning-parent-1",
      status: "completed",
      reportStatus: "ready",
...
test("planning-parent delegate_finish blocked publication invalidates earlier authorization", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    ...
    const seed = await store.publishPlanningReport(taskId, {
      rawText: [
        "PLANNING_REPORT", "STATUS|ready", "GOAL|Seed authorized plan.",
        "PLAN_ARTIFACT_PATH|docs/plans/seed.md", "ARTIFACT_PATHS|docs/plans/seed.md", "REVIEW_STATUS|passed",
        "SETTLED_DECISIONS|seed", "OPEN_QUESTIONS|none", "EXECUTION_AUTONOMY|bounded", "USER_CHECKPOINTS|authorization",
        "EXECUTION_GUIDANCE|follow seed", "RISKS|none", "EVIDENCE|tests", "END_PLANNING_REPORT",
      ].join("\n"),
    ...
    assert.equal((await store.readExecutionAuthorization(taskId)).planningReportReadyEventId, seed.planningReadyEventId);
    const { tools } = loadExtension(() => ok());

    const blocked = await withProcessEnv(delegatedEnv(store, manifest), () => tools.get("delegate_finish").execute("finish-planning-blocked", {
      taskId,
      agentId: "planning-parent-1",
      status: "blocked",
...
test("delegate_finish stores parent reports with canonical task report records", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const planningManifest = await store.registerAgent({ taskId: "TASK-PARENT-FINISH", agentId: "planning-parent-1", role: "planning-parent", profile: "planning-parent", cwd: repoRoot, writeScope: join(repoRoot, "docs"), state: "running", surfaceRef: "surface:plan", parentAgentId: "orchestrator" });
    const executionManifest = await store.registerAgent({ taskId: "TASK-PARENT-FINISH", agentId: "execution-parent-1", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, writeScope: repoRoot, state: "running", surfaceRef: "surface:exec", parentAgentId: "orchestrator" });
    ...
    const planning = await withProcessEnv(delegatedEnv(store, planningManifest), () => finish.execute("finish-planning", {
    ...
    assert.equal((await store.readExecutionApprovalRequest("TASK-PARENT-FINISH")).planArtifactPath, "docs/plans/fix.md");
    ...
    const execution = await withProcessEnv(delegatedEnv(store, executionManifest), () => finish.execute("finish-execution", {
      taskId: "TASK-PARENT-FINISH",
      agentId: "execution-parent-1",
      status: "completed_with_risks",
...
test("delegate_finish enforces verifier top-level status and accepts check pass rows", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const verifierManifest = await store.registerAgent({ taskId: "TASK-VERIFY", agentId: "verifier-1", role: "verifier", profile: "verifier", cwd: repoRoot, state: "running", surfaceRef: "surface:3" });
    await writeCurrentTaskPacket(store, verifierManifest);
    const { tools } = loadExtension(() => ok());
    const finish = tools.get("delegate_finish");

    const invalid = await withProcessEnv(delegatedEnv(store, verifierManifest), () => finish.execute("finish-invalid", { taskId: "TASK-VERIFY", agentId: "verifier-1", status: "PASS", summary: "all checks pass" }, undefined, undefined, ctx(repoRoot)));
    ...
    const valid = await withProcessEnv(delegatedEnv(store, verifierManifest), () => finish.execute("finish-valid", {
      taskId: "TASK-VERIFY",
      agentId: "verifier-1",
      status: "completed",
...
test("delegate_attention keeps every child-selectable level at P1 with queued next-turn wake only", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const { tools, calls, userMessages } = loadExtension(() => { throw new Error("delegate_attention must not call cmux"); });
    ...
    for (const level of levels) {
      ...
      const attentionManifest = await store.registerAgent({ taskId: "TASK-ATTENTION-MATRIX", agentId, role: "worker", cwd: repoRoot, writeScope: repoRoot, state: "running", parentAgentId: "orchestrator" });
      await writeCurrentTaskPacket(store, attentionManifest);
      const result = await withProcessEnv(delegatedEnv(store, attentionManifest), () => attention.execute(`attention-${level}`, {
    ...
    assert.deepEqual((await store.readParentAlerts("TASK-ATTENTION-MATRIX", { unreadOnly: true })).map((alert) => alert.priority), ["P1", "P1", "P1", "P1", "P1"]);
  }));
});
...
test("delegate_finish derives worker reviewer verifier priority from structured result evidence", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const { tools, calls, userMessages } = loadExtension(() => { throw new Error("delegate_finish must not call cmux"); });
    const finish = tools.get("delegate_finish");
    const cases = [
      ...
      { id: "review-nonblocking", role: "reviewer", status: "completed", findings: [{ severity: "non_blocking", problem: "minor" }], expected: "P2" },
      { id: "review-blocking", role: "reviewer", status: "completed", findings: [{ severity: "blocking", problem: "must fix" }], expected: "P1" },
      { id: "verify-pass", role: "verifier", status: "completed", checks: [{ name: "tests", status: "pass" }], completionClaimSupported: true, expected: "P2" },
      { id: "verify-fail", role: "verifier", status: "completed", checks: [{ name: "tests", status: "fail" }], completionClaimSupported: true, expected: "P1" },
      { id: "verify-not-run", role: "verifier", status: "completed_with_risks", checks: [{ name: "tests", status: "not_run" }], completionClaimSupported: true, expected: "P2" },
      { id: "verify-unsupported", role: "verifier", status: "completed", checks: [{ name: "tests", status: "pass" }], completionClaimSupported: false, expected: "P1" },
    ];
    for (const item of cases) {
      const matrixManifest = await store.registerAgent({ taskId: "TASK-FINISH-MATRIX", agentId: item.id, role: item.role, profile: item.role, cwd: repoRoot, ...(item.role === "worker" ? { writeScope: repoRoot } : {}), state: "running", parentAgentId: "orchestrator" });
      await writeCurrentTaskPacket(store, matrixManifest);
      const result = await withProcessEnv(delegatedEnv(store, matrixManifest), () => finish.execute(`finish-${item.id}`, {
        ...
        ...(item.completionClaimSupported === undefined ? {} : { completionClaimSupported: item.completionClaimSupported }),
      }, undefined, undefined, ctx(repoRoot)));
      ...
      const storedAlert = (await store.readParentAlerts("TASK-FINISH-MATRIX", { unreadOnly: true, agentId: item.id }))[0];
...
test("explicitly closed parent escalates one deduped P0 to stored ancestor or orchestrator root without direct wake", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    await store.registerAgent({ taskId: "TASK-ESCALATE", agentId: "grandparent-1", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "running", parentAgentId: "orchestrator" });
    await store.registerAgent({ taskId: "TASK-ESCALATE", agentId: "parent-1", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "closed", parentAgentId: "grandparent-1" });
    const escalatedWorkerManifest = await store.registerAgent({ taskId: "TASK-ESCALATE", agentId: "worker-1", role: "worker", cwd: repoRoot, writeScope: repoRoot, state: "running", parentAgentId: "parent-1" });
    await writeCurrentTaskPacket(store, escalatedWorkerManifest);
    const { tools, calls, userMessages } = loadExtension(() => { throw new Error("closed-parent escalation must not call cmux"); });
    const attention = tools.get("delegate_attention");

    const result = await withProcessEnv(delegatedEnv(store, escalatedWorkerManifest), () => attention.execute("attention-escalate", { taskId: "TASK-ESCALATE", agentId: "worker-1", level: "blocked", message: "worker blocked" }, undefined, undefined, ctx(repoRoot)));
    ...
    assert.equal((await store.readParentAlerts("TASK-ESCALATE", { unreadOnly: true })).filter((alert) => alert.eventType === "parent-unavailable-escalation").length, 1);
    const secondWorkerManifest = await store.registerAgent({ taskId: "TASK-ESCALATE", agentId: "worker-2", role: "worker", cwd: repoRoot, writeScope: repoRoot, state: "running", parentAgentId: "parent-1" });
    await writeCurrentTaskPacket(store, secondWorkerManifest);
    const secondResult = await withProcessEnv(delegatedEnv(store, secondWorkerManifest), () => attention.execute("attention-escalate-second-source", { taskId: "TASK-ESCALATE", agentId: "worker-2", level: "blocked", message: "second worker blocked" }, undefined, undefined, ctx(repoRoot)));
    const afterSecondSource = await store.readParentAlerts("TASK-ESCALATE", { unreadOnly: true });
    const coalescedEscalations = afterSecondSource.filter((alert) => alert.eventType === "parent-unavailable-escalation");
    ...
    assert.equal((await store.readWakeAttempts("TASK-ESCALATE")).filter((attempt) => attempt.priority === "P0" && attempt.alertIds.includes(coalescedEscalations[0].alertId)).length, 1);

    await store.registerAgent({ taskId: "TASK-ESCALATE-ROOT", agentId: "parent-root", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "closed" });
    const rootWorkerManifest = await store.registerAgent({ taskId: "TASK-ESCALATE-ROOT", agentId: "worker-root", role: "worker", cwd: repoRoot, writeScope: repoRoot, state: "running", parentAgentId: "parent-root" });
    await writeCurrentTaskPacket(store, rootWorkerManifest);
    await withProcessEnv(delegatedEnv(store, rootWorkerManifest), () => attention.execute("attention-escalate-root", { taskId: "TASK-ESCALATE-ROOT", agentId: "worker-root", level: "blocked", message: "root-owned worker blocked" }, undefined, undefined, ctx(repoRoot)));
    const rootEscalation = (await store.readParentAlerts("TASK-ESCALATE-ROOT", { unreadOnly: true })).find((alert) => alert.eventType === "parent-unavailable-escalation");
...
test("running missing malformed and unknown parent state never escalate", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "parent-running", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "running", parentAgentId: "orchestrator" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "worker-running", role: "worker", cwd: repoRoot, state: "running", parentAgentId: "parent-running" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "worker-missing", role: "worker", cwd: repoRoot, state: "running", parentAgentId: "parent-missing" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "parent-malformed", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "closed", parentAgentId: "orchestrator" });
    await writeFile(store.pathsForAgent("TASK-NO-ESCALATE", "parent-malformed").statusJson, "{bad-json", "utf8");
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "worker-malformed", role: "worker", cwd: repoRoot, state: "running", parentAgentId: "parent-malformed" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "parent-unknown", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "closed", parentAgentId: "orchestrator" });
    await writeFile(store.pathsForAgent("TASK-NO-ESCALATE", "parent-unknown").statusJson, JSON.stringify({ taskId: "TASK-NO-ESCALATE", agentId: "parent-unknown", state: "lost", updatedAt: "now" }), "utf8");
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "worker-unknown", role: "worker", cwd: repoRoot, state: "running", parentAgentId: "parent-unknown" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "parent-identity", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "closed", parentAgentId: "orchestrator" });
    await writeFile(store.pathsForAgent("TASK-NO-ESCALATE", "parent-identity").statusJson, JSON.stringify({ taskId: "OTHER-TASK", agentId: "parent-identity", state: "closed", updatedAt: "2026-07-10T00:00:00.000Z" }), "utf8");
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "worker-identity", role: "worker", cwd: repoRoot, state: "running", parentAgentId: "parent-identity" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "parent-profile", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, state: "closed", parentAgentId: "orchestrator" });
    await store.updateAgentManifest("TASK-NO-ESCALATE", "parent-profile", { profile: "planning-parent" });
    await store.registerAgent({ taskId: "TASK-NO-ESCALATE", agentId: "worker-profile", role: "worker", cwd: repoRoot, state: "running", parentAgentId: "parent-profile" });
    const noEscalateWorkerIds = ["worker-running", "worker-missing", "worker-malformed", "worker-unknown", "worker-identity", "worker-profile"];
    ...
    const { tools, calls, userMessages } = loadExtension(() => { throw new Error("unknown parent state must not call cmux"); });
    const attention = tools.get("delegate_attention");
    for (const agentId of noEscalateWorkerIds) {
      const manifest = await store.readAgentManifest("TASK-NO-ESCALATE", agentId);
      await withProcessEnv(delegatedEnv(store, manifest), () => attention.execute(`attention-${agentId}`, { taskId: "TASK-NO-ESCALATE", agentId, message: `${agentId} needs parent` }, undefined, undefined, ctx(repoRoot)));
    }
...
test("delegate_result auto-closes passing verifier panes after parent consumes result", async () => {
  await withProcessEnv(DELEGATION_ENV_CLEAR, async () => withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const autoCloseManifest = await store.registerAgent({ taskId: "TASK-AUTOCLOSE", agentId: "verifier-1", role: "verifier", profile: "verifier", cwd: repoRoot, state: "running", surfaceRef: "surface:3", retention: "auto" });
    ...
    await withProcessEnv(delegatedEnv(store, autoCloseManifest), () => finish.execute("finish-verifier", {
...
test("delegated delegate_record_report binds current planning-parent attempt and rejects scope or role forgery", async () => {
  await withTempRepo(async (repoRoot) => {
    const store = createDelegationStore({ root: join(repoRoot, ".freeflow", "delegation") });
    const planning = await store.registerAgent({ taskId: "TASK-DIRECT-REPORT-SOURCE", agentId: "planning-parent-1", role: "planning-parent", profile: "planning-parent", cwd: repoRoot, writeScope: join(repoRoot, "docs"), state: "running", parentAgentId: "orchestrator" });
    await writeCurrentTaskPacket(store, planning);
    const execution = await store.registerAgent({ taskId: "TASK-DIRECT-REPORT-ROLE", agentId: "execution-parent-1", role: "execution-parent", profile: "execution-parent", cwd: repoRoot, writeScope: repoRoot, state: "running", parentAgentId: "orchestrator" });
    await writeCurrentTaskPacket(store, execution);
    const rawText = [
      "PLANNING_REPORT", "STATUS|ready", "GOAL|Bind delegated source.",
      "PLAN_ARTIFACT_PATH|docs/plans/source-bound.md", "ARTIFACT_PATHS|docs/plans/source-bound.md", "REVIEW_STATUS|passed",
      "SETTLED_DECISIONS|bind attempt", "OPEN_QUESTIONS|none", "EXECUTION_AUTONOMY|bounded", "USER_CHECKPOINTS|authorization",
      "EXECUTION_GUIDANCE|follow plan", "RISKS|none", "EVIDENCE|tests", "END_PLANNING_REPORT",
    ].join("\n");
    const { tools } = loadExtension(() => ok());
    ...
    await withProcessEnv({
      ...
      const accepted = await record.execute("record-current-planning-attempt", { taskId: planning.taskId, reportName: "planning-report", rawText }, undefined, undefined, ctx(repoRoot));
      assert.equal(accepted.details.result.reportStatus, "ready");
      const events = (await readFile(store.pathsForTask(planning.taskId).eventsJsonl, "utf8")).trim().split("\n").map((line) => JSON.parse(line));
      ...
      const crossTask = await record.execute("record-cross-task", { taskId: "TASK-OTHER", reportName: "planning-report", rawText }, undefined, undefined, ctx(repoRoot));
    ...
    await withProcessEnv({
      ...
      const stale = await record.execute("record-stale-planning-attempt", { taskId: planning.taskId, reportName: "planning-report", rawText }, undefined, undefined, ctx(repoRoot));
    ...
    await withProcessEnv({
      ...
      const wrongRole = await record.execute("record-execution-parent-planning", { taskId: execution.taskId, reportName: "planning-report", rawText }, undefined, undefined, ctx(repoRoot));
...
test("delegate_record_report stores parsed reports and malformed report evidence", async () => {
  await withTempRepo(async (repoRoot) => {
    ...
    const rootRejected = await withProcessEnv(DELEGATION_ENV_CLEAR, () => record.execute("record-report-root", { taskId: "TASK-P4", reportName: "execution-report", rawText: report }, undefined, undefined, ctx(repoRoot)));
    ...
    const okResult = await withProcessEnv(delegatedEnv(store, executionManifest), () => record.execute("record-report", { taskId: "TASK-P4", reportName: "execution-report", rawText: report }, undefined, undefined, ctx(repoRoot)));
    ...
    const conflicting = await record.execute("record-conflicting-ready", { taskId: "TASK-P4-CONFLICT", reportName: "planning-report", rawText: conflictingReady }, undefined, undefined, ctx(repoRoot));
    ...
    assert.notEqual(conflicting.details.result.paths.raw, conflictStore.pathsForTask("TASK-P4-CONFLICT").planningReportRaw);
    await assert.rejects(() => conflictStore.readExecutionApprovalRequest("TASK-P4-CONFLICT"), /no valid planning-ready event/);

    const validReady = conflictingReady
      .replace("PLAN_ARTIFACT_PATH|docs/plans/first.md\nPLAN_ARTIFACT_PATH|docs/plans/second.md", "PLAN_ARTIFACT_PATH|docs/plans/accepted.md")
      .replace("ARTIFACT_PATHS|docs/plans/first.md,docs/plans/second.md", "ARTIFACT_PATHS|docs/plans/accepted.md");
    const accepted = await record.execute("record-valid-after-conflict", { taskId: "TASK-P4-CONFLICT", reportName: "planning-report", rawText: validReady }, undefined, undefined, ctx(repoRoot));
    assert.equal(accepted.details.result.reportStatus, "ready");
    ...
    await record.execute("record-conflicting-after-valid", { taskId: "TASK-P4-CONFLICT", reportName: "planning-report", rawText: conflictingReady }, undefined, undefined, ctx(repoRoot));
