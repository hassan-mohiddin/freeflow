...
export function renderDelegationResult(toolName, result, { expanded } = {}, theme) {
    ...
    if (payload.layout?.slot) {
        lines.push(themeFg(theme, "accent", `layout: ${payload.layout.slot} • focus preserved`));
    }
    ...
    if (payload.decision) {
        lines.push("", themeFg(theme, "toolTitle", "Route decision"));
        pushField(lines, theme, "kind", payload.decision.kind);
        pushField(lines, theme, "target", payload.decision.targetRole);
        pushField(lines, theme, "suggested reroute", payload.decision.suggestedReroute);
        pushField(lines, theme, "route id", payload.decision.routeId);
        pushField(lines, theme, "reason codes", listText(payload.decision.reasonCodes));
        pushField(lines, theme, "guidance", payload.actionGuidance ?? payload.nextAction);
    }
    if (payload.routeApplication || payload.layout) {
        lines.push("", themeFg(theme, "toolTitle", "Route application"));
        pushField(lines, theme, "state", payload.routeApplication?.state ?? payload.status);
        pushField(lines, theme, "application", payload.routeApplication?.applicationId);
        pushField(lines, theme, "waiting for", payload.routeApplication?.waitingFor);
        pushField(lines, theme, "spawned", listText(payload.spawned));
        pushField(lines, theme, "reused", listText(payload.reused));
        pushField(lines, theme, "layout slot", payload.layout?.slot);
        pushField(lines, theme, "layout allocation", payload.layout?.allocationId);
        pushField(lines, theme, "focus", payload.layout?.preserveFocus ? "preserved" : undefined);
    }
    if (payload.delivery) {
    ...
    if (payload.result) {
        ...
        if (Array.isArray(payload.result.errors) && payload.result.errors.length > 0) {
            pushField(lines, theme, "errors", payload.result.errors.map((error) => error.message).join("; "));
        }
    }
    ...
    if (payload.executionMap) {
        ...
        if (Array.isArray(payload.executionMap.packages)) {
            for (const pkg of payload.executionMap.packages.slice(0, 12)) {
                ...
                if (pkg.checkoutPath)
                    lines.push(`    ${themeFg(theme, "muted", "checkout:")} ${themeFg(theme, "accent", shortenMiddle(pkg.checkoutPath, 110))}`);
                if (Array.isArray(pkg.commitCheckpoints) && pkg.commitCheckpoints.length > 0) {
                    lines.push(`    ${themeFg(theme, "muted", "commit checkpoints:")} ${pkg.commitCheckpoints.map((checkpoint) => `${checkpoint.checkpointId}:${checkpoint.status}`).join(", ")}`);
                }
            }
        }
    }
