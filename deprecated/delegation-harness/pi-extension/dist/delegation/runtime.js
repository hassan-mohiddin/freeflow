...
import { activeLeasesForAgent, authorizeDelegationLease, createDelegationStore, defaultDenySummaryForProfile, delegationRootForRepo, defaultReturnProtocolForRole, evaluatePolicy, findActiveLegacyAssignmentLease, isChildLifecycleDelegationTool, isDelegationTool, isParentControlDelegationTool, isReadRecoveryDelegationTool, parseModelText, priorityForParentAlert, resolveAssignmentAttemptIdentity, resolveProfileForRole, returnProtocolForActiveTools, validateSafeId, validateTaskPacketIdentity, } from "../../../delegation/dist/index.js";
...
export async function appendUnreadDelegationAlertSummary(ctx, systemPrompt) {
    ...
    try {
        ...
        if (detection.mode === "delegated") {
            ...
            scopedAlerts.push(...alerts.map((alert) => normalizeSummaryAlert(alert)).filter(Boolean));
        }
        else {
            ...
            for (const taskId of taskIds) {
                try {
                    ...
                    scopedAlerts.push(...alerts
                        .filter((alert) => isRootSummaryAlert(alert))
                        .map((alert) => normalizeSummaryAlert(alert))
                        .filter(Boolean));
                }
                catch {
...
export async function handleDelegatedAssistantMessageEnd(event, ctx) {
    ...
    if (setup.identity === undefined || setup.blockers.length > 0 || setup.terminalIdentityBlocker !== undefined) {
        const rawSnapshotPath = await store.writeAgentModelText(setup.detection.taskId, setup.detection.agentId, `rejected-attempt-${assistantRawFileName(rawText)}`, rawText);
...
function validateConsequentialManifestIdentity(setup) {
    if (setup.detection.mode !== "delegated")
        return undefined;
    ...
    for (const [field, expected, actual] of [
        ...
        if (expected !== actual)
            mismatches.push(`${field} env=${expected} manifest=${actual}`);
    }
...
function leaseViewErrorClass(error) {
    const message = messageFrom(error).toLowerCase();
    if (message.includes("enoent") || message.includes("no such file"))
        return "missing";
    if (message.includes("stale active lease view"))
        return "stale";
    if (message.includes("does not match lease log") || message.includes("agent mismatch") || message.includes("task id mismatch"))
        return "forged";
    return "malformed";
}
async function queueLeasePolicyAttentionBestEffort(setup, event, errorClass, error) {
    if (!canAddressAgentStore(setup.detection))
        return;
    try {
        const store = createDelegationStore({ root: setup.detection.storeRoot });
...
async function appendKickoffEvent(store, setup, status, paths, resultPaths) {
    await appendStoreEvents(store, setup.detection.taskId, setup.detection.agentId, {
...
async function recordParsedTerminalOutcome(store, setup, parsed, rawText, planningPublications) {
...
function normalizeParsedResultAlertEvidence(result, role) {
    const checks = [];
    const findings = Array.isArray(result?.blockers)
        ? result.blockers
            .filter((blocker) => blocker?.kind !== "capability_gap")
            .map((blocker) => ({
            severity: "blocking",
...
async function recordMalformedAssistantOutput(store, setup, parsed, resultPaths, _state) {
...
async function recordMissingRequiredOutput(store, setup, parsed, resultPaths, _state, stopReason) {
...
async function recordRejectedRuntimeTerminalEvidence(store, setup, rawText, evidence) {
    const publication = await store.publishTerminalOutcome(setup.detection.taskId, {
        agentId: setup.detection.agentId,
        assignmentId: setup.identity.assignmentId,
        attemptId: setup.identity.attemptId,
...
function parentAlertOutcomeForEvent(state, data) {
    ...
    if (state === "cancelled")
        return "cancelled";
    if (state === "attention" || state === "waiting_for_parent")
        return "attention";
    return undefined;
}
function hasCapabilityGap(data) {
    const blockers = Array.isArray(data?.blockers) ? data.blockers : [];
    const requests = Array.isArray(data?.requests) ? data.requests : [];
    return blockers.some((blocker) => blocker?.kind === "capability_gap")
        || requests.some((request) => request?.attributes?.kind === "capability_gap" || request?.action === "capability_gap")
        || data?.code === "capability_gap";
}
function evidenceForAlert(data) {
    const evidence = {};
    if (typeof data?.rawPath === "string")
        evidence.rawPath = data.rawPath;
    if (typeof data?.jsonPath === "string")
        evidence.jsonPath = data.jsonPath;
...
function isRootSummaryAlert(alert) {
    if (!alert || typeof alert !== "object")
        return false;
    if (alert.parentAgentId === "orchestrator")
        return true;
    if (alert.outcome === "user_attention")
        return true;
    return alert.parentAgentId === undefined && alert.agentId === undefined;
}
function normalizeSummaryAlert(alert) {
    if (!alert || typeof alert !== "object" || alert.readAt !== undefined)
        return undefined;
    try {
...
function sortSummaryAlerts(alerts) {
...
function summaryPriorityRank(priority) {
    return priority === "P0" ? 0 : priority === "P1" ? 1 : priority === "P2" ? 2 : 3;
}
function renderUnreadAlertSummaryRow(alert) {
    return [
        "UNTRUSTED_ALERT",
        alert.priority,
...
function sanitizeAlertSummaryField(value, maxLength) {
...
async function readDelegationIndexTaskIds(root) {
    let text;
    try {
...
async function recordSentSummaryAttemptsBestEffort(store, shown, parentAgentId) {
