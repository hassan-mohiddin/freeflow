...
export function operationIdentity(name, definitions) {
  const content = definitions
    .map((definition) => (typeof definition === "function" ? definition.toString() : JSON.stringify(definition)))
    .join("\n---\n");
...
function turnValue(turn, priorSections, index) {
  ...
  if (response.omittedBytes > 0)
    omissions.push({
      reason: "bounded-response-excerpt",
      span: `/evidence/turns/${index}/final_response`,
      omittedBytes: response.omittedBytes,
    });

  const reduced = compactPatchSections(turn.diff);
  if (reduced.omittedBytes > 0)
    omissions.push({
      reason: "unmodified-diff-context-and-transport-headers",
      span: `/evidence/turns/${index}/diff`,
      omittedBytes: reduced.omittedBytes,
    });
  const diff = [];
  for (const section of reduced.sections) {
    ...
    if (prior?.hash === hash) {
      ...
      omissions.push({
        reason: "duplicate-diff-section",
        span: `/evidence/turns/${index}/diff`,
        omittedBytes: byteLength(content),
      });
    } else {
      const excerpt = bounded(content, DIFF_LIMIT);
      diff.push(`FILE ${section.path}\n${excerpt.text}`);
      if (excerpt.omittedBytes > 0)
        omissions.push({
          reason: "bounded-diff-excerpt",
          span: `/evidence/turns/${index}/diff`,
  ...
  return {
    value: [
      ...
      `NATURAL_PROMPT\n${turn.natural_prompt ?? ""}`,
      `RESPONSE_EXCERPT\n${response.text}`,
      `CHANGED_PATHS ${(turn.changed_paths ?? []).join(",")}`,
      `DIFF_CHANGE_EXCERPTS\n${diff.join("\n")}`,
    ].join("\n"),
    omissions,
...
function oneShotValue(evidence) {
  ...
  const files = changedFiles
    .map((file, index) => {
      const excerpt = bounded(
        file.status === "deleted" || file.content === null ? "<deleted>" : (file.content ?? "<unavailable>"),
        FILE_LIMIT,
      );
      if (excerpt.omittedBytes > 0)
        omissions.push({
          reason: "bounded-file-excerpt",
    ...
    .join("\n");
  const unavailableFiles = (evidence.changed_file_unavailable ?? [])
    .map((file) => `FILE ${file.path} UNAVAILABLE ${file.reason}`)
    .join("\n");
  const hasFiles = changedFiles.length > 0;
  const compact = compactPatchSections(evidence.diff);
  const availablePaths = new Set(
    changedFiles.filter((file) => (file.status ?? "available") === "available").map((file) => file.path),
  );
  const allDiffDuplicated =
    compact.sections.length > 0 && compact.sections.every((section) => availablePaths.has(section.path));
  let diffValue = "";
  if (allDiffDuplicated && evidence.diff) {
    omissions.push({
      reason: "diff-duplicated-by-changed-file-content",
      span: "/evidence/diff",
      omittedBytes: byteLength(evidence.diff),
    });
  } else {
    if (compact.omittedBytes > 0)
      omissions.push({
        reason: "unmodified-diff-context-and-transport-headers",
        span: "/evidence/diff",
        omittedBytes: compact.omittedBytes,
      });
    diffValue = compact.sections
      .map((section) => {
  ...
  const values = [
    `NATURAL_PROMPT\n${evidence.natural_prompt ?? ""}`,
    `RESPONSE_EXCERPT\n${response.text}`,
    `CHANGED_PATHS ${(evidence.changed_paths ?? []).join(",")}`,
  ];
...
function objectiveAssertionValue(assertion) {
  return stableJson({
    ...
    ...(assertion.evidence === undefined ? {} : { evidence: assertion.evidence }),
  });
}
  ...
  return {
    ...
    fields: {
      ...
      detail: omissions
        .map(
          (omission) =>
            `${OMISSION_CODES[omission.reason] ?? omission.reason},${omission.span},${omission.omittedBytes}`,
        )
        .join(";"),
      op,
...
const TURN_OP = operationIdentity("t", [
  turnValue,
  turnCollectionValue,
  compactPatchSections,
  bounded,
  { RESPONSE_LIMIT, DIFF_LIMIT },
]);
const ONE_SHOT_OP = operationIdentity("s", [
  oneShotValue,
  compactPatchSections,
  bounded,
  { RESPONSE_LIMIT, DIFF_LIMIT, FILE_LIMIT },
]);
...
function validateSelectedTurns(evidence) {
  ...
  if (!Array.isArray(selected) || JSON.stringify(selected) !== JSON.stringify(rendered)) {
    throw new Error(
      `Selected turn IDs must exactly match ordered rendered turns: selected=${JSON.stringify(selected)} rendered=${JSON.stringify(rendered)}`,
    );
  }
  for (const criterion of evidence.criteria) {
    if (JSON.stringify(criterion.turn_ids ?? []) !== JSON.stringify(selected))
      throw new Error(`Criterion ${criterion.id} turn IDs must match selected turn IDs`);
  }
}

export function reduceSemanticPacket(packet, { bundle = "semantic-packet", sourcePath = "semantic-packet.json" } = {}) {
  if (packet?.schema_version !== 1 || !packet?.evidence || !Array.isArray(packet.evidence.criteria))
    throw new Error("Invalid canonical semantic packet");
  ...
  const records = [
    {
      ...
      schema: "CEV1",
      fields: {
        l: evidence.label ?? "opaque",
        o: String(evidence.objective_checks_passed === true),
        t: (evidence.selected_turn_ids ?? []).join(","),
      },
    },
    {
      type: "S",
  ...
  for (let index = 0; index < (evidence.objective_assertions ?? []).length; index += 1) {
    ...
    records.push({
      ...
      fields: {
        name: `o.${assertion.id}`,
        value: objectiveAssertionValue(assertion),
        source: "p",
        span: `/evidence/objective_assertions/${index}`,
        op: OBJECTIVE_OP,
        recovery: "exact-source",
      },
    });
  ...
  if (Array.isArray(evidence.turns)) {
    ...
    records.push({
      ...
      fields: {
        name: "t",
        value: turnCollectionValue(turnValues),
        source: "p",
        span: "/evidence/turns",
        op: TURN_OP,
        recovery: "exact-source",
  ...
  return {
    ...
    parity: {
      criterion_ids: evidence.criteria.map((criterion) => criterion.id),
      selected_turn_ids: evidence.selected_turn_ids ?? [],
      source_sha256: sourceHash,
      omissions: omissions.length,
      omitted_bytes: omissions.reduce((sum, omission) => sum + omission.omittedBytes, 0),
