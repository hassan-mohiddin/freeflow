...
const RECORD_KEYS = [
  "authority", "convertible_to_current_result", "id", "indexed_on", "indexing_revision", "limitations",
  "readiness_eligible", "referenced_artifacts", "related_current_skills", "reported_date", "reported_eval_ids",
  "reported_outcome", "source_report", "superseded_by", "supersedes",
];
...
function assertFixedAuthority(value, label) {
  if (value.authority !== AUTHORITY || value.readiness_eligible !== false || value.convertible_to_current_result !== false) {
...
function assertSortedUnique(values, label, itemCheck = () => {}) {
  ...
  for (let index = 0; index < values.length; index += 1) {
    ...
    if (index > 0 && compareText(values[index - 1], values[index]) >= 0) fail(`${label} must be unique and lexically sorted`);
  }
}
...
function assertSafeRelativePath(path, label) {
  ...
  if (normalized !== path || normalized === "." || normalized.startsWith("../") || normalized.includes("/../")) fail(`${label} is not normalized`);
}
...
export function extractArtifactReferences(source) {
  ...
  for (const match of source.matchAll(pattern)) {
    ...
    if (!sourceToken || /[*?${}\[\]<>\\#]/.test(sourceToken) || sourceToken.split("/").some((segment) => segment === "." || segment === "..")) continue;
...
function assertSchemaDocument(schema) {
  ...
  if (schema.type !== "object" || schema.additionalProperties !== false) fail("historical schema root must be a closed object");
  ...
  if (schema.properties?.readiness_eligible?.const !== false || schema.properties?.convertible_to_current_result?.const !== false) fail("historical schema non-authority booleans are invalid");
  if (schema.$defs?.record?.additionalProperties !== false || schema.$defs?.artifact?.additionalProperties !== false) fail("historical schema record shapes must be closed");
}
...
function assertContext(context, source, label) {
  ...
  if (sha256(context.excerpt) !== context.sha256 || !source.includes(context.excerpt)) fail(`${label} excerpt is not exact source evidence`);
...
function assertRelationShape(relation, label) {
  ...
  assertExactKeys(relation.evidence, ["source_report_path", "source_report_sha256", "excerpt", "excerpt_sha256"], [], `${label}.evidence`);
...
function sameRelationEvidence(left, right) {
  return left?.source_report_path === right?.source_report_path
...
async function validateRecord({ root, record, index, sourceByPath, recordsById, ignoredRunsRoot, counters, position }) {
  ...
  if (record.indexing_revision !== index.indexing_revision || record.indexed_on !== index.indexed_on) fail(`${record.id} indexing identity differs from index`);
  ...
  if (historicalRecordId(record.source_report.path) !== record.id) fail(`${record.id} is not derived from source report path`);
  ...
  assertSortedUnique(record.related_current_skills, `${record.id}.related_current_skills`, (skill, label) => assertString(skill, label, SKILL_PATTERN));
  ...
  assertSortedUnique(record.reported_eval_ids, `${record.id}.reported_eval_ids`, (evalId, label) => assertString(evalId, label, EVAL_ID_PATTERN));
  for (const evalId of record.reported_eval_ids) if (!source.includes(evalId)) fail(`${record.id} eval ID is not stated in source: ${evalId}`);
  ...
  if (sha256(record.reported_outcome.excerpt) !== record.reported_outcome.sha256 || !source.includes(record.reported_outcome.excerpt)) fail(`${record.id} outcome excerpt is not exact source evidence`);
  ...
  if (record.referenced_artifacts.length !== extracted.length) fail(`${record.id} concrete artifact extraction is incomplete`);
  for (let artifactIndex = 0; artifactIndex < record.referenced_artifacts.length; artifactIndex += 1) {
    ...
    const expectedKeys = artifact.status === "present" ? ["kind", "path", "sha256", "source_tokens", "status"] : ["path", "source_tokens", "status"];
    assertExactKeys(artifact, expectedKeys, [], `${record.id}.referenced_artifacts[${artifactIndex}]`);
    assertSortedUnique(artifact.source_tokens, `${record.id}.referenced_artifacts[${artifactIndex}].source_tokens`, (token, label) => assertString(token, label));
    assertSafeRelativePath(artifact.path, `${record.id}.referenced_artifacts[${artifactIndex}].path`);
    if (artifact.path !== expectedReference?.path || JSON.stringify(artifact.source_tokens) !== JSON.stringify(expectedReference?.source_tokens)) fail(`${record.id} artifact source-token normalization mismatch`);
    const observed = await inspectArtifact(root, artifact.path, ignoredRunsRoot);
    if (artifact.status !== observed.status || artifact.kind !== observed.kind || artifact.sha256 !== observed.sha256) fail(`${record.id} artifact status or hash mismatch: ${artifact.path}`);
  ...
  if (JSON.stringify(record.limitations) !== JSON.stringify(derivedLimitations(record))) fail(`${record.id} limitations do not match observable state`);
  ...
  for (const relationName of ["supersedes", "superseded_by"]) {
    ...
    for (let relationIndex = 0; relationIndex < relations.length; relationIndex += 1) {
      ...
      if (previous && compareText(previous, relation.record_id) >= 0) fail(`${record.id}.${relationName} must be unique and sorted by record_id`);
      previous = relation.record_id;
      const target = recordsById.get(relation.record_id);
      if (!target || target.id === record.id) fail(`${record.id}.${relationName} has invalid target ${relation.record_id}`);
      ...
      if (sha256(evidenceSource) !== relation.evidence.source_report_sha256 || !evidenceSource.includes(relation.evidence.excerpt) || sha256(relation.evidence.excerpt) !== relation.evidence.excerpt_sha256) fail(`${record.id}.${relationName} evidence is not exact source text`);
      const opposite = relationName === "supersedes" ? "superseded_by" : "supersedes";
      const reciprocal = target[opposite]?.find((item) => item.record_id === record.id && sameRelationEvidence(item.evidence, relation.evidence));
...
export async function auditHistoricalEvidence({ root }) {
  ...
  const [schema, index] = await Promise.all([readJson(schemaPath, "historical schema"), readJson(indexPath, "historical registry")]);
  assertSchemaDocument(schema);
  assertExactKeys(index, ["schema_version", "index_id", "indexing_revision", "indexed_on", "scope", "authority", "readiness_eligible", "convertible_to_current_result", "records"], [], "historical registry");
  if (index.schema_version !== 1 || index.index_id !== "freeflow-historical-evidence") fail("historical registry identity is invalid");
  ...
  assertExactKeys(index.scope, ["included_roots", "excluded_roots", "recursive", "file_extension"], [], "historical registry.scope");
  if (JSON.stringify(index.scope.included_roots) !== JSON.stringify(INCLUDED_ROOTS) || JSON.stringify(index.scope.excluded_roots) !== JSON.stringify(EXCLUDED_ROOTS) || index.scope.recursive !== true || index.scope.file_extension !== ".md") fail("historical registry scope is not the fixed initial scope");

  const reportPaths = (await Promise.all(INCLUDED_ROOTS.map((path) => discoverMarkdown(absoluteRoot, path)))).flat().sort(compareText);
  const sourceByPath = new Map();
  for (const reportPath of reportPaths) sourceByPath.set(reportPath, await readFile(resolve(absoluteRoot, reportPath), "utf8"));
  if (!Array.isArray(index.records) || index.records.length !== reportPaths.length) fail(`historical registry must contain exactly ${reportPaths.length} in-scope reports`);
  const recordPaths = index.records.map((record) => record?.source_report?.path);
  if (JSON.stringify(recordPaths) !== JSON.stringify(reportPaths)) fail("historical registry records must be complete and sorted by source report path");
  ...
  for (let position = 0; position < index.records.length; position += 1) {
    await validateRecord({ root: absoluteRoot, record: index.records[position], index, sourceByPath, recordsById, ignoredRunsRoot, counters, position });
  }
