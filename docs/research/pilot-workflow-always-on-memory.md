# Pilot Workflow Always-On Memory

Date: 2026-05-25

## Decision

Pilot should install a small always-on runtime contract for each host agent, then keep full workflow behavior in progressive skills.

Always-load invariants. Progressively load procedure. Mechanically enforce only after evals prove wording fails.

## Why

Pilot needs to survive cold starts, fresh conversations, and compaction. The agent should remember Pilot's core behavior before it decides whether to invoke a skill.
...
- read default mode from `.pilot-workflow/config.json`
...
Do not use `.codex/rules/*.rules` for Pilot behavior. Codex rules are shell approval/security policy. They are loaded by the runtime, but not as model memory.
...
Claude should use `CLAUDE.md` plus an imported Pilot core file.
...
Put the recommended core block in `.claude/rules/pilot-core.md`.
...
```text
pilot-mode-contract.md
pilot-interview-gate.md
pilot-workflow.md
pilot-capture-decisions.md
```
...
pilot-mode-contract.md
pilot-interview-gate.md
pilot-source-truth.md
pilot-capture-memory.md
```

`pilot-source-truth.md` is worth naming separately if splitting happens. Source-truth conflict failures have shown high behavioral risk in Pilot evals.
...
`setup-pilot-workflow` should eventually support host-specific setup:

- Codex target: update `AGENTS.md`.
- Claude target: update `CLAUDE.md` and optionally create `.claude/rules/pilot-core.md` imported by `CLAUDE.md`.
...
2. Claude setup creates `CLAUDE.md` import plus `.claude/rules/pilot-core.md` only when Claude is the clear target.
3. Both files exist and target is ambiguous: setup asks before editing.
4. User asks for four rule files: setup recommends one `pilot-core.md` unless user confirms split.
