...
The Pi extension registers direct Freeflow commands, keeps `/workflow` mode changes session-scoped, loads mode-contract, workflow, interview-gate, discover, and output-router context on session start and compact, injects full skill context once after that boundary, then keeps later turns to a short mode/priority refresh plus compact output-router tool reminder. It does not enforce policy, block tools, grant permissions, or create repo-local hooks.
