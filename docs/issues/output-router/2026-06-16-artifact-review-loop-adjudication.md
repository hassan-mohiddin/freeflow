...
During review of `docs/specs/output-router/freeflow-output-router-design.md`, a reviewer subagent returned findings. The parent agent treated the findings too much like a patch queue and moved toward revising the spec before first adjudicating the findings with the user.
