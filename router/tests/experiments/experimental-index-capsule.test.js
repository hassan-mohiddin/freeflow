...
test("experimental index capsule exports existing names", async () => {
...
test("index benchmark imports capsule while product runtime avoids local index experiment", async () => {
  const indexBenchmarks = await readFile("router/src/benchmarks/index-benchmarks.ts", "utf8");
  assert.match(indexBenchmarks, /\.\.\/experiments\/local-index\.js/);
  assert.doesNotMatch(indexBenchmarks, /experimental-local-index\.js/);

  for (const path of ["router/src/tools/retrieve.ts", "router/src/tools/run.ts"]) {
