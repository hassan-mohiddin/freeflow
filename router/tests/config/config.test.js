...
test("normalizeFreeflowConfig keeps minimal setup config valid without dumping optional defaults", () => {
  ...
  assert.equal(result.config.outputRouter.enabled, false);
...
test("normalizeFreeflowConfig ignores removed capture/provider config and rejects invalid active values", () => {
  ...
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.enabled")));
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.sandbox")));
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.languages")));
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.network")));
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.limits.timeoutMs")));
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.limits.maxInputBytes")));
  assert.ok(result.warnings.some((warning) => warning.includes("outputRouter.scriptTransform.limits.maxOutputBytes")));
...
test("normalizeRouterConfig accepts nested thresholds, vault, and hints", () => {
  const result = normalizeRouterConfig({
    thresholds: { largeOutputBytes: 12_000, largeOutputLines: 120 },
    vault: { root: ".freeflow/vault", retention: { strategy: "ttl", ttlDays: 14 } },
    hints: { generatedPathGlobs: ["graphify-out/**"], noisyCommandPatterns: ["npm test"] },
  });

  assert.deepEqual(result.warnings, []);
  assert.deepEqual(result.config.thresholds, { largeOutputBytes: 12_000, largeOutputLines: 120 });
  assert.equal(result.config.vault.root, ".freeflow/vault");
  assert.deepEqual(result.config.vault.retention, { strategy: "ttl", ttlDays: 14 });
