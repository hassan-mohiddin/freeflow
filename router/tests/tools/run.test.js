...
import { parseCommandOutput } from "../../dist/routing/parsers.js";
...
test("git output parser preserves exact status and diffstat lines", async () => {
  await withTempVault(async (vault) => {
    const stdout = [
      " FAIL.md | 2 ++",
      " router/src/tools/run.ts | 41 ++++++++++++++++++++----",
      " router/src/config/schema.ts | 8 ++++++--",
    ...
    assert.match(result.importantLines?.[0].excerpt ?? "", /router\/src\/tools\/run\.ts/);
    assert.match(result.importantLines?.[0].excerpt ?? "", /router\/src\/config\/schema\.ts/);
