...
export function assembleTextEvidence(options) {
    ...
    if (entries.length <= caps.maxLines &&
...
export function lineEntries(text) {
    return splitLines(text).map((line, index) => ({ line, lineNumber: index + 1 }));
}
export function nonEmptyLineEntries(text) {
...
function buildBoundedExcerpt(entries, maxExcerptBytes, maxLineBytes) {
    const parts = [];
...
function truncateLine(line, maxBytes) {
