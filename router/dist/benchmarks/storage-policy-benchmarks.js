...
import { commandOutputFingerprints, createVault, findExactDuplicateCommandOutput, readOutputText, storeCommandOutput, storeMetadataOutput, } from "../vault/vault.js";
...
export async function runStoragePolicyBenchmarks(options = {}) {
...
export function renderStoragePolicyBenchmarkReport(report) {
    ...
    lines.push("## Decision Boundary", "", "This benchmark intentionally stops before changing defaults. Any storage default change still needs a product/safety decision because metadata-only storage changes exact recovery semantics and privacy surface.", "");
    return `${lines.join("\n")}\n`;
}
export async function writeStoragePolicyBenchmarkReports(report, markdownReportPath = defaultReportPath(), options = {}) {
...
async function runPolicy(policy, iterations) {
...
async function runPolicyFixture(options) {
    ...
    const decisionInput = { fixture: options.fixture, combined, rawCombinedBytes };
...
function summarizeStoragePolicyReport(policies) {
...
function createStoragePolicyFixtures() {
