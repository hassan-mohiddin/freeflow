...
import {
  ...
  escapeMarkdownTableCell as escapeMarkdown,
...
interface ContextModeToolResult {
  content?: Array<{ text?: string }>;
}

interface ContextModeClient {
  connect(transport: unknown): Promise<void>;
  callTool(request: { name: string; arguments: Record<string, unknown> }): Promise<ContextModeToolResult>;
  close(): Promise<void>;
}

interface ContextModeClientConstructor {
  new (options: { name: string; version: string }): ContextModeClient;
}

interface ContextModeTransportConstructor {
  new (options: { command: string; args: string[]; env: NodeJS.ProcessEnv }): unknown;
}

interface LoadedContextModeRuntime {
...
export async function runContextModeRealDeepBenchmark(
  options: RunContextModeRealDeepBenchmarkOptions = {},
): Promise<ContextModeRealDeepReport> {
  ...
  const rows: DeepBenchmarkRow[] = [];

  let cm: ContextModeClient | undefined;
  ...
  try {
    ...
    const runner: HostCommandRunner = {
      async run(request) {
        const shellOptions: { cwd?: string; timeoutMs?: number } = {};
    ...
    async function callCM(name: string, args: Record<string, unknown>): Promise<Observation> {
      if (!cm) {
    ...
    function scriptCode(rel: string, code: string): string {
      return `import { readFileSync } from 'node:fs';\nconst FILE_CONTENT = readFileSync(${JSON.stringify(rel)}, 'utf8');\n${code}\n`;
    }

    async function writeScenarioScript(id: string, rel: string, code: string): Promise<string> {
    ...
    for (const scenario of executeScenarios) {
      ...
      const processingReducerModes: Record<string, { mode: string; reducer: string }> = {
        ...
        "playwright-summary": { mode: "freeflow:process-browser-snapshot-reducer", reducer: "browser-snapshot" },
        "git-log-summary": { mode: "freeflow:process-git-log-reducer", reducer: "git-log" },
      };
...
export function renderContextModeRealDeepBenchmarkReport(report: ContextModeRealDeepReport): string {
  const lines: string[] = [];
  ...
  for (const summary of report.summaries) {
    lines.push(
      `| ${escapeMarkdown(summary.mode)} | ${summary.scenarios} | ${summary.correct}/${summary.scenarios} | ${summary.totalFactsFound}/${summary.totalFacts} | ${summary.totalRawBytes} | ${summary.totalVisibleBytes} | ${summary.weightedReductionPct}% | ${summary.avgLatencyMs}ms | ${summary.exactRecovery} | ${summary.metadataOnly} |`,
    );
  }
  lines.push("");
  ...
  for (const row of report.rows) {
    lines.push(
      `| ${escapeMarkdown(row.fixture)} | ${escapeMarkdown(row.category)} | ${escapeMarkdown(row.mode)} | ${escapeMarkdown(row.capability)} | ${row.correct ? "yes" : "no"} | ${row.factsFound}/${row.factsTotal} | ${row.rawBytes} | ${row.visibleBytes} | ${row.reductionPct}% | ${escapeMarkdown(row.recovery)} | ${row.latencyMs} | ${escapeMarkdown(row.notes)} |`,
    );
  }
  lines.push("");
...
export async function writeContextModeRealDeepBenchmarkReports(
  report: ContextModeRealDeepReport,
...
export function baselineFailureClassesDetected(report: Pick<ContextModeRealDeepReport, "failureClusters">): boolean {
...
function buildReport(input: {
  generatedAt: string;
  availability: ContextModeAvailability;
  freeflowRepo: string;
  benchmarkRoot: string;
  storeRoot: string;
  freeflowVaultRoot: string;
  sessionId: string;
  rows: DeepBenchmarkRow[];
  contextModeStatsPreview: string;
}): ContextModeRealDeepReport {
...
function realBenchmarkMethodology(): string[] {
  return [
...
function resolveContextModeAvailability(options: RunContextModeRealDeepBenchmarkOptions): ContextModeAvailability {
...
async function setupProject(input: { projectRoot: string; fixtureSource: string; outsideRoot: string }): Promise<void> {
  ...
  await writeFile(
    path.join(input.projectRoot, "repo-fixture/graphify-out/graph.html"),
...
function createExecuteScenarios(): ExecuteScenario[] {
  return [
    {
      id: "vitest-summary",
      category: "dev-output",
...
function createSearchScenarios(): SearchScenario[] {
  return [
    {
...
function requiredScenario(scenarios: readonly ExecuteScenario[], id: string): ExecuteScenario {
...
function summarizeRows(rows: readonly DeepBenchmarkRow[]): BenchmarkModeSummary[] {
  const groups = new Map<string, BenchmarkModeSummary>();
...
function failureClusters(rows: readonly DeepBenchmarkRow[]): FailureClusters {
  return {
...
function buildBaselineChecks(clusters: FailureClusters): BaselineChecks {
...
function detectedFailureKeys(clusters: FailureClusters): Set<string> {
...
function contentText(result: ContextModeToolResult): string {
...
function factCount(text: string, facts: readonly string[]): number {
...
function recoveryOutputId(result: unknown): string | undefined {
...
function safeShellSync(command: string): string {
  try {
...
function safeReadPackageVersion(repo: string): string {
  try {
    const packageJson = JSON.parse(readFileSync(path.join(repo, "package.json"), "utf8")) as { version?: unknown };
...
async function runCli(): Promise<void> {
  ...
  const options: RunContextModeRealDeepBenchmarkOptions = {};
...
function cliStringOption(args: readonly string[], name: string): string | undefined {
  const prefix = `${name}=`;
