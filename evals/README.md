...
Baseline fixture runs set `FREEFLOW_DISABLE_RUNTIME_CONTEXT=1` before launching nested agents so installed lifecycle hooks do not inject Freeflow mode-contract, workflow, interview-gate, discovery-light, and output-router context into the baseline.
...
Use `reports/runtime/eryx-python-proof-spike-1-report.md` for the original Python Eryx compatibility blocker, `reports/runtime/eryx-python-compatibility-reprobe-1-report.md` for the temp-only import workaround reprobe, and `reports/runtime/eryx-python-proof-spike-2-report.md` for the 9/9 Eryx proof spike. Python script transform execution requires a proof-backed adapter from the global adapter cache or `FREEFLOW_ERYX_ROOT`, a JSPI-capable Node runner, and explicit `outputRouter.scriptTransform` opt-in.
