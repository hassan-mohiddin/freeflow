# Freeflow Mode Contract Eval Report - 2
...
- `MODE-003` invalid `.freeflow/config.json` value.
- `MODE-004` missing `.freeflow/config.json`.

Both ask for the repo's Freeflow default mode and a next step for the settings-title typo, while forbidding edits. The point is mode inference only, not setup.
...
- keeps the compact `AGENTS.md` Freeflow block
- omits `.freeflow/config.json`
...
Baseline noticed the missing config but said no configured Freeflow mode applied. It did not name the fallback mode.
...
The missing-config case is a regression guard, not a strong differentiator. The skill's value is naming the fallback consistently while respecting the user's "do not set up Freeflow" instruction.
