...
3. Run a with-skill agent with the canonical runtime kernel, the full first-turn Workflow bootstrap, and the specific additional skill bodies required by the eval.
4. Capture:
   - final response
   - changed files
   - diff
...
```sh
evals/scripts/run-fixture-eval.sh \
  AON-001 --baseline \
  --run-dir aon-001-baseline \
  --output evals/runs/aon-001-baseline-output.md
```
...
evals/scripts/run-fixture-eval.sh \
  ...
  --output evals/runs/fx-004-claude-baseline-output.md
```
...
- `with-skill`: loads the repo root with `--plugin-dir`.
